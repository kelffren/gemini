 ...
 * owner: Map Forge CI
 * purpose: lock the visually approved Royal Capital primary road widths across representative deterministic seeds
 */
...
for(const seed of SEEDS){
  ...
  results.push({seed,roadCount:map.roads.length,arterialCount:arterials.length,collectorCount:collectors.length,arterialWidths:[...new Set(arterials.map(road=>road.width))],collectorWidths:[...new Set(collectors.map(road=>road.width))],quality:map.quality.total});
}
console.log(JSON.stringify({ok:true,expectedArterialWidth:EXPECTED_ARTERIAL_WIDTH,expectedCollectorWidth:EXPECTED_COLLECTOR_WIDTH,seeds:SEEDS,results},null,2));
