...
test('mobile frame editor performs pinch, ghost, auto-match, masks, patches and clean export',async({page},testInfo)=>{
  ...
  await page.locator('.kfb-slot').nth(0).click();await expect(page.locator('.kfe')).toBeVisible();await shot(page,testInfo,'03-editor-frame');await page.getByRole('button',{name:/GHOST/}).click();await expect(page.getByRole('button',{name:/GHOST/})).toHaveClass(/on/);await shot(page,testInfo,'04-editor-ghost');await page.locator('.kfe-close').click();
