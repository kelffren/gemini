...
for(const seed of SEEDS){
  ...
  assert.equal(map.metadata.generatorVersion,'1.10.0',`${seed}: boundary-owner gateway guard targets generator 1.10.0`);
  ...
  if(seed===PRIMARY_SEED){assert.ok(exits.length>=3,`${seed}: boundary-owner fallback must frame at least three world exits`);assert.ok(exits.some(scene=>scene.exitId==='exit_northwest'&&scene.members.every(member=>member.family==='rock')),`${seed}: northwest forest exit must retain its authored stone markers`);assert.ok(exits.some(scene=>scene.exitId==='exit_south'&&scene.district==='farms'&&scene.members.every(member=>member.family==='flower')),`${seed}: south exit must use the actual boundary district and visible farm planters`);}
