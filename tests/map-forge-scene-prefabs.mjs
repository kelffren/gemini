...
for(const seed of SEEDS){
  ...
  assert.equal(map.metadata.generatorVersion,'1.10.0',`${seed}: mining edge gateway fallback guard targets generator 1.10.0`);
  ...
  if(seed===PRIMARY_SEED){assert.ok(exits.length>=3,`${seed}: fallback semantic pairs must frame at least three world exits`);assert.ok(exits.some(scene=>scene.exitId==='exit_northwest'&&scene.members.every(member=>member.family==='rock')),`${seed}: northwest forest exit must fall back from unavailable shrubs to authored stone markers`);assert.ok(exits.some(scene=>scene.exitId==='exit_southwest'&&scene.members.every(member=>member.family==='rock')),`${seed}: southwest mining exit must fall back to authored stone markers after mine props are committed to its landmark`);}
