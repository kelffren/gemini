...
test('V6.69 evergreen branch boots plaza without uncaught errors', async ({ page }) => {
  ...
  expect(snapshot.abilityAim.version).toBe('kelo-ability-aim-v1.1.0-pointer-owner');
...
test('consolidated ability aim owns pointer drag lifecycle and keeps action-slot contract', async ({ page }) => {
  await page.goto('/?guest=1',{waitUntil:'domcontentloaded'});
  ...
  expect(ended.pointerLifecycle.endHandled-moved.pointerLifecycle.endHandled).toBe(1);
});
