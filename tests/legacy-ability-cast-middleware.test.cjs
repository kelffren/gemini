 ...
 * purpose: congela retirada de monkey-patches, orden LIFO y separación completa del registro de cast respecto a KeloAbilityAim
 */
...
for(const [name,text] of [['engine-l',plaza],['engine-m',world]]){
  ...
  assert.ok(text.includes('window.KeloLegacyAbilityCast'),`${name} must use KeloLegacyAbilityCast`);
  assert.ok(text.includes('registerMiddleware('),`${name} must register through KeloLegacyAbilityCast`);
  assert.ok(!text.includes('window.KeloAbilityAim'),`${name} direct KeloAbilityAim dependency must stay retired`);
}
