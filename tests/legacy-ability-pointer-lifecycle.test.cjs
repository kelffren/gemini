 ...
 * purpose: impide que engine-g recupere hotbar/aim/listeners/render y exige lifecycle/cast registry legacy propiedad de KeloAbilityAim
 */
...
assert.ok(owner.includes('function registerCastMiddleware(owner,fn)'),'KeloAbilityAim must own the legacy cast middleware registry');
