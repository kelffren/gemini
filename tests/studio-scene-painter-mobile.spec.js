...
test('Scene Painter survives mobile lazy boot, Grid 3x2, one Undo and shell mutation stress',async({page})=>{
  ...
  try{
    ...
    const diag=await page.evaluate(async()=>window.__scenePainterDiag?.());
    throw new Error(`SCENE_PAINTER_READY_TIMEOUT ${JSON.stringify(diag)} :: ${error?.message||error}`);
  }
