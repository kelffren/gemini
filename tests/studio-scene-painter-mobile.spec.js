...
test('Scene Painter survives mobile lazy boot, Grid 3x2, one Undo and shell mutation stress',async({page})=>{
  ...
  await page.evaluate(build=>{window.__scenePainterStudio=async()=>{for(const tag of [build,`${build}-retry`]){try{const mod=await import(`./src/studio/integration/world-studio-bridge.mjs?v=${tag}`);const s=mod.getKeloStudioLive?.()?.studio;if(s)return s;}catch{}}return null;};},WORLD_BUILD);
