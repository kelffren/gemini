 ...
 * area: MOUNTS / RUNTIME
 * owner: KeloMounts
 * purpose: estado equip/mounted, equipo/outfit, atributos y bridge a KeloMovement sin segundo loop
 ...
 * consumes: KeloMountCatalog, KeloMountEquipmentCatalog, KeloStatModifiers, KeloAppearance, KeloMovement, KeloEvents, KeloEquipment
 ...
 * do-not: no tocar STATE.equipped stones; no game loop; no renderer; no reglas por mountId; no usar KeloPlayerStats para atributos
 */
(function(root){'use strict';
if(root.KeloMounts)return;
const Catalog=root.KeloMountCatalog,EquipCatalog=root.KeloMountEquipmentCatalog,Stats=root.KeloStatModifiers,Appearance=root.KeloAppearance;
if(!Catalog||!EquipCatalog||!Stats){console.error('KeloMounts missing catalog/stat-modifier dependency');return;}
const VERSION='mount-runtime-v1.0.2',SCHEMA_VERSION=1;
...
function state(){
 ...
 if(!Object.keys(s.owned).length){s.owned['mount.training_horse']={id:'mount.training_horse',unlockedAt:Date.now()};s.equippedMountId='mount.training_horse';}
 for(const id of ['equipment.mount.war_saddle','equipment.mount.swift_horseshoes'])if(!s.equipmentInventory.includes(id))s.equipmentInventory.push(id);
...
function rehydratePersistedStats(){Stats.markDirty();if(typeof localPlayer!=='undefined'&&root.KeloEquipment?.recalculate){root.KeloEquipment.recalculate(localPlayer,true);return true;}return false;}
installStatsSource();state();installMovement();const didRehydratePersistedStats=rehydratePersistedStats();
...
root.KELO_MOUNT_AUDIT=Object.freeze({version:VERSION,stoneSlotsUntouched:true,mountAbilitySlots:3,usesKeloMovementHooks:!!movementBefore,usesKeloStatModifiers:true,authorityBoundary:true,secondLoop:false,rehydratesPersistedStats:true,rehydratedOnBoot:didRehydratePersistedStats});
