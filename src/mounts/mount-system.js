...
const VERSION='mount-runtime-v1.0.0',SCHEMA_VERSION=1;
...
function equipmentModifiersFor({target,targetId,context}){const s=state();if(!s?.equippedMountId)return[];const mountId=s.equippedMountId;const out=[];for(const item of equipmentItems(mountId))for(const raw of item.modifiers||[]){const m=clone(raw);if(m.target==='mount')m.targetId=mountId;out.push(m);}return out;}
...
installStatsSource();state();installMovement();
...
root.KELO_MOUNT_AUDIT=Object.freeze({version:VERSION,stoneSlotsUntouched:true,mountAbilitySlots:3,usesKeloMovementHooks:!!movementBefore,usesKeloStats:true,authorityBoundary:true,secondLoop:false});
