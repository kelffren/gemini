...
export function createStudioAssetPreviewService({assetCatalog,atlasContract,devicePixelRatio=globalThis.devicePixelRatio||1}={}){
  ...
  async function decodeRemoteBlob(blob){const objectUrl=URL.createObjectURL(blob),image=new Image();image.decoding='async';await new Promise((resolve,reject)=>{image.onload=resolve;image.onerror=()=>reject(new Error('REMOTE_PREVIEW_DECODE_FAILED'));image.src=objectUrl;});return{image,objectUrl,bytes:blob.size,mime:blob.type};}
  async function fetchRemoteImage(url,controller){const response=await fetch(url,{mode:'cors',credentials:'omit',cache:'no-store',signal:controller.signal});if(!response.ok)throw new Error(`REMOTE_PREVIEW_HTTP_${response.status}`);const declared=Number(response.headers.get('content-length')||0);if(declared>REMOTE_MAX_BYTES)throw new Error(`REMOTE_PREVIEW_TOO_LARGE:${declared}`);const blob=await response.blob();if(blob.size>REMOTE_MAX_BYTES)throw new Error(`REMOTE_PREVIEW_TOO_LARGE:${blob.size}`);if(!/^image\//i.test(blob.type||''))throw new Error('REMOTE_PREVIEW_NOT_IMAGE');return decodeRemoteBlob(blob);}
  function registerRemotePreview(id,{url,lowUrl=null,width=96,height=96,label=null,category='external',progressive=true}={}){id=String(id||'');if(!id||!url)throw new Error('STUDIO_REMOTE_PREVIEW_REQUIRED');unregisterRemotePreview(id);const controller=new AbortController(),row={id,label:label||id,category,width:Math.max(16,Math.min(512,Number(width)||96)),height:Math.max(16,Math.min(512,Number(height)||96)),remotePreview:true,url:String(url),lowUrl:lowUrl?String(lowUrl):null,controller,image:null,objectUrl:null,error:null,bytes:0,quality:'loading',promise:null,upgradePromise:null};const install=(result,quality)=>{if(controller.signal.aborted){try{URL.revokeObjectURL(result.objectUrl);}catch{}return row;}const previous=row.objectUrl;row.image=result.image;row.objectUrl=result.objectUrl;row.bytes=result.bytes;row.mime=result.mime;row.quality=quality;if(previous&&previous!==result.objectUrl)try{URL.revokeObjectURL(previous);}catch{}return row;};const low=row.lowUrl&&row.lowUrl!==row.url?row.lowUrl:null;if(progressive&&low){row.promise=fetchRemoteImage(low,controller).then(result=>install(result,'low')).then(()=>{row.upgradePromise=fetchRemoteImage(row.url,controller).then(result=>install(result,'full')).catch(error=>{if(!controller.signal.aborted)row.upgradeError=error;return row;});return row;}).catch(error=>{if(controller.signal.aborted)throw error;return fetchRemoteImage(row.url,controller).then(result=>install(result,'full'));});}else row.promise=fetchRemoteImage(row.url,controller).then(result=>install(result,'full')).catch(error=>{if(!controller.signal.aborted)row.error=error;throw error;});remoteAssets.set(id,row);trimRemote(id);return row.promise;}
  function clearRemotePreviews(){for(const row of remoteAssets.values())disposeRemote(row);remoteAssets.clear();}
  function remoteStats(){return{entries:remoteAssets.size,ready:[...remoteAssets.values()].filter(x=>!!x.image).length,loading:[...remoteAssets.values()].filter(x=>!x.image&&!x.error).length,errors:[...remoteAssets.values()].filter(x=>!!x.error).length,low:[...remoteAssets.values()].filter(x=>x.quality==='low').length,full:[...remoteAssets.values()].filter(x=>x.quality==='full').length,bytes:[...remoteAssets.values()].reduce((n,x)=>n+Number(x.bytes||0),0),maxEntries:REMOTE_MAX_ENTRIES,maxBytesPerPreview:REMOTE_MAX_BYTES};}
