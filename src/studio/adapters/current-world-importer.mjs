/* KELO-INDEX
 * area: STUDIO / IMPORT
 * owns: projection of current Kelo world/property snapshots into a raw WorldDocument payload
 * does-not-own: runtime state, authority, asset definitions or final document cloning
 * public-api: importCurrentKeloWorld()
 * online: read-only; can import published or an already-authorized mutable draft view
 * mobile: A10 returns borrowed snapshot references; the Studio kernel is the single deep-normalization boundary
 */
...
function placementToEntity(placement,catalog){
  ...
  if(template?.parts?.length)components.visual={source:'property-catalog',parts:template.parts};
...
async function readDraftSnapshot(adapter,payload){
  try{return await adapter.worldEditRequest('world:preview:enter',payload);}
  catch(error){
...
export async function importCurrentKeloWorld({adapter,mode='world',actorId=null,parcelId=null,view='published',draftId=null}={}){
  ...
  if(mode==='parcel'){
    ...
    viewMeta=result?.viewMeta||result?.revision||result?.draft||null;
  }
  ...
  return {
    ...
    metadata:{name:mode==='parcel'?'My Parcel':'Kelo World',description:'',tags:[mode,'imported',view]},
    settings:{tileSize:adapter.tileRegistry?.worldTileSize||32,chunkSize:adapter.worldRenderer?.chunkSize||512},
    terrain:snapshot?.cells||{},
    entities,
    navigation:{collisions:snapshot?.collisions||{}},
    revision:{id:viewMeta?.revisionId||viewMeta?.id||viewMeta?.draftId||null,number:Number(viewMeta?.number||viewMeta?.revisionVersion)||0}
  };
}
