 ...
 * mobile: text synchronization is idempotent so lifecycle observation cannot self-trigger forever
 */
...
export function resolveRoomMaterialCandidates({prefabs=[]}={}){
  ...
  for(const [kind,definition] of Object.entries(DEFINITIONS)){
    const scored=[];
    for(const prefab of prefabs||[]){const score=scorePrefab(prefab,definition);if(score>0)scored.push({id:String(prefab.id),label:String(prefab.label||prefab.id),score});}
...
export function createRoomMaterialTool(kernel,{root=globalThis}={}){
  ...
  function selectedRoomId(){
    ...
    for(const id of ids){const row=kernel.spatial.get?.(String(id))?.data||kernel.document.entities.find(entity=>String(entity.id)===String(id));const roomId=row?.components?.buildingPiece?.roomId;if(roomId)return String(roomId);}
    return null;
  }
  function roomRows(roomId=selectedRoomId()){if(!roomId)return[];return (kernel.document.entities||[]).filter(row=>String(row?.components?.buildingPiece?.roomId||'')===roomId);}
  function currentPrefab(kind,rows=roomRows()){const match=rows.find(row=>row?.components?.buildingPiece?.type===kind);return match?String(match.prefabId||match.components?.buildingPiece?.prefabId||''):'';}
  function candidateFor(kind,prefabId){return (candidates[kind]||[]).find(row=>row.id===String(prefabId))||null;}

  async function applyRoomMaterial(kind,prefabId){
    kind=String(kind||'');if(!DEFINITIONS[kind])return null;
    const roomId=selectedRoomId(),target=candidateFor(kind,prefabId);if(!roomId||!target)return null;
    ...
    const result=await kernel.execute(command);syncUi();return result;
  }

  async function cycleRoomMaterial(kind){kind=String(kind||'');const rows=roomRows(),list=candidates[kind]||[];if(!rows.length||list.length<2)return null;const current=currentPrefab(kind,rows),index=Math.max(-1,list.findIndex(row=>row.id===current));const next=list[(index+1)%list.length];return applyRoomMaterial(kind,next.id);}
  ...
  function syncUi(){
    ...
    for(const button of buttons){
      ...
      const aria=available?'false':'true';if(button.getAttribute('aria-disabled')!==aria)button.setAttribute('aria-disabled',aria);
      const current=currentPrefab(kind,rows),currentRow=list.find(row=>row.id===current);
      setText(button,`${DEFINITIONS[kind].glyph} ${DEFINITIONS[kind].label}${currentRow?` · ${currentRow.label}`:''}`);
      const title=available?`Cycle ${kind} material for the selected ROOM`:`Select a ROOM with at least two ${kind} prefabs available`;if(button.title!==title)button.title=title;
    }
  }

  function ensureUi(){
    if(destroyed||!document)return;
    const palette=document.querySelector?.('.ks-qb-palette');if(!palette)return;
    if(buttons.length===2&&buttons.every(button=>button?.isConnected)){syncUi();return;}
    for(const button of buttons)button?.remove?.();buttons=[];
    const cancel=palette.querySelector?.('[data-qb-cancel]');
    for(const kind of ['wall','floor']){
      const button=document.createElement('button');button.type='button';button.className='ks-qb-piece';button.dataset.roomMaterial=kind;
      ...
      palette.insertBefore(button,cancel||null);buttons.push(button);
    }
    syncUi();
  }

  function destroy(){if(destroyed)return;destroyed=true;observer?.disconnect?.();unsubSelection?.();for(const button of buttons)button?.remove?.();buttons=[];}
  ...
  return Object.freeze({id:'roomMaterial',version:'studio-room-material-v1.0.1-idempotent-dom',candidates:{wall:candidates.wall.map(copy),floor:candidates.floor.map(copy)},applyRoomMaterial,cycleRoomMaterial,getSelectedRoomId:selectedRoomId,destroy});
}