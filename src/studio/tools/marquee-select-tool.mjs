...
export function createMarqueeSelectTool(kernel){
  ...
  function commit(){if(!state)return[];const box=rect(),append=state.append;state=null;const ids=kernel.spatial.queryRect(box,{category:'entity'}).map(row=>row.id);if(append){for(const id of ids)kernel.selection.add(id);}else kernel.selection.set(ids);return ids;}
