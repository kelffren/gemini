...
export function planRoomFloor({roomId,floorPiece,floorPrefab,x0,y0,x1,y1}={}){
  ...
  for(let row=0;row<rows;row++)for(let col=0;col<cols;col++){
    result.push({
      ...
      bounds:{...bounds},
      components:{buildingPiece:{type:'floor',system:'quick-build',version:3,roomGenerated:true,roomId,roomInterior:true,roomFloorCol:col,roomFloorRow:row,prefabId:String(floorPiece.prefabId),snapPoints:copy(floorPiece.snapPoints||[])}}
    });
  }
  return result;
}
