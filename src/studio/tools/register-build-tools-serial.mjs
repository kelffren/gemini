 ...
 * keys: STUDIO MOBILE BUILD PAINT COPIES QUICK ROOM SURGERY LAZY
...
export async function registerBuildToolsSerial(kernel,core={},{wait=async()=>{}}={}){
  ...
  const out={paintCopies:null,quickBuild:null,roomBuild:null,roomOpening:null,roomMaterial:null};
  ...
  const ensure=async(id,path,factory,{requires=[]}={})=>{
    ...
    for(const dependency of requires){
      if(!dependency){disabled(id,'dependency-off');return null;}
    }
    ...
    try{
      ...
      out[id]=tool;
      S?.moduleDone?.(id,started);
      await wait();
      return tool;
    }catch(error){
      S?.moduleFailed?.(id,error);
      throw error;
    }
  };
  ...
  return Object.freeze({paintCopies,quickBuild,roomBuild,roomOpening,roomMaterial});
}
