...
export function registerBasicTools(kernel) {
  const select = createSelectTool(kernel), placement = createPlacementTool(kernel), transform = createTransformTool(kernel), terrain = createTerrainTool(kernel), collision = createCollisionTool(kernel), prefabStamp = createPrefabStampTool(kernel);
  for (const tool of [select, placement, transform, terrain, collision, prefabStamp]) if (!kernel.tools.get(tool.id)) kernel.tools.register(tool);
  return Object.freeze({ select, placement, transform, terrain, collision, prefabStamp });
}
