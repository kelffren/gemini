 ...
 * owns: rectangular room planning, semantic room identity, intentional drag gating, live quantized measurement, desktop square/center modifiers and batch commit through placement
...
export function createRoomBuildTool(kernel,{placement=null,quickBuild=null,root=globalThis}={}){
  ...
  function planRect(ax,ay,bx,by,{square=false,centered=false}={}){
    ...
    const signature=`${roomId}|${x0}|${y0}|${x1}|${y1}|${countX}|${countY}|${square?1:0}|${centered?1:0}|${length}`;
    const nextMeasurement={x:x0,y:y0,width:x1-x0,height:y1-y0,requestedWidth,requestedHeight,deltaWidth:(x1-x0)-requestedWidth,deltaHeight:(y1-y0)-requestedHeight,modulesX:countX,modulesY:countY,wallLength:length,totalWalls:2*(countX+countY),squareLocked:!!square,centered:!!centered,centerX:centered?startX:null,centerY:centered?startY:null};
  ...
  function syncButton(){if(!button)return;button.classList.toggle('on',active);button.setAttribute('aria-pressed',active?'true':'false');button.textContent=active?(measurement?`▣ ${measurement.centered?'⊙ ':''}${measurement.squareLocked?'□ ':''}${Math.round(measurement.width)}×${Math.round(measurement.height)}`:'▣ ROOM ON'):'▣ ROOM';}
  ...
  ensureButton();
  return Object.freeze({id:'roomBuild',version:'studio-room-build-v1.6.0-centered-drag',activate,deactivate,planRect,commitRoom,getPreviews:()=>copy(previews),getMeasurement:()=>copy(measurement),getPlannerStats:()=>({...plannerStats}),get active(){return active;},destroy});
}
