...
export function createRoomBuildTool(kernel,{placement=null,quickBuild=null,root=globalThis}={}){
  ...
  function syncButton(){if(!button)return;const on=!!active,pressed=on?'true':'false',label=on?(measurement?`▣ ${measurement.centered?'⊙ ':''}${measurement.squareLocked?'□ ':''}${Math.round(measurement.width)}×${Math.round(measurement.height)} · ${measurement.totalFloors||0}F`:'▣ ROOM ON'):'▣ ROOM';if(button.classList.contains('on')!==on)button.classList.toggle('on',on);if(button.getAttribute('aria-pressed')!==pressed)button.setAttribute('aria-pressed',pressed);if(button.textContent!==label)button.textContent=label;}
  ...
  if(document?.documentElement&&root?.MutationObserver){observer=new root.MutationObserver(()=>{if(!button?.isConnected)ensureButton();});observer.observe(document.documentElement,{childList:true,subtree:true});}
  ensureButton();
  return Object.freeze({id:'roomBuild',version:'studio-room-build-v2.0.2-observer-safe',activate,deactivate,planRect,commitRoom,getPreviews:()=>copy(previews),getMeasurement:()=>copy(measurement),getPlannerStats:()=>({...plannerStats}),get active(){return active;},destroy});
}
