 ...
 * owns: rectangular room planning, semantic room identity and batch commit through placement
...
export function createRoomBuildTool(kernel,{placement=null,quickBuild=null,root=globalThis}={}){
  ...
  let active=false,drag=null,previews=[],destroyed=false,button=null,observer=null,busy=false,plannedRoomId=null;
  ...
  function activate(){if(active)return true;if(!wallPiece())return false;quickBuild.activate?.('wall');active=true;drag=null;previews=[];plannedRoomId=null;kernel.input.push(CONTEXT);syncButton();return true;}
  function deactivate(){if(!active)return false;active=false;drag=null;previews=[];plannedRoomId=null;kernel.input.pop(CONTEXT);syncButton();return true;}
  async function commitRoom(){if(!active||busy||previews.length<4)return null;busy=true;try{const rows=previews.map(copy),roomId=plannedRoomId;const committed=await placement.commitBatch(rows,{label:`Build room (${rows.length} walls)`});drag=null;previews=[];plannedRoomId=null;syncButton();return Object.assign(committed,{roomId});}finally{busy=false;}}
  const handlers={
    ...
    pointerup:e=>{if(!active||!drag)return!!active;planRect(drag.x,drag.y,e.worldX,e.worldY);const enough=previews.length>=4;drag=null;if(enough)void commitRoom().catch(err=>console.warn('[Kelo Studio] Room Build commit failed',err));else{previews=[];plannedRoomId=null;syncButton();}return true;},
    pointercancel:()=>{drag=null;previews=[];plannedRoomId=null;syncButton();return!!active;}
  };
  ...
  function destroy(){if(destroyed)return;destroyed=true;active=false;drag=null;previews=[];plannedRoomId=null;kernel.input.pop(CONTEXT);unregister?.();observer?.disconnect?.();button?.remove();button=null;}
  ...
  ensureButton();
  return Object.freeze({id:'roomBuild',version:'studio-room-build-v1.1.0-semantic-id',activate,deactivate,planRect,commitRoom,getPreviews:()=>copy(previews),get active(){return active;},destroy});
}
