 ...
 * owns: contextual editing of selected Quick Build pieces and handoff back into Quick Build
...
export function createQuickEditTool(kernel,{quickBuild=null,root=globalThis}={}){
  ...
  async function replaceSelected(type){
    ...
    const components={...(copy(row.components)||{}),buildingPiece:semanticFor(piece,bounds,row.components?.buildingPiece||{})};
    busy=true;
  ...
  function ensureUi(){
    ...
    if(!style){
      style=document.createElement('style');style.dataset.keloQuickEdit='1';style.textContent=`
        ...
        #kelo-studio-live:has(.ks-qb-palette:not([hidden])) .ks-qe-bar{display:none}
        ...
        #kelo-studio-live .ks-qe-piece{min-width:42px;padding:0 7px}.ks-qe-piece.on{border-color:#8fb4ff;background:#1b3152}.ks-qe-action.primary{border-color:#568a70;background:#123026;color:#c9f8da}.ks-qe-action:disabled,.ks-qe-piece:disabled{opacity:.4}
    ...
    if(!bar?.isConnected){
      ...
      bar.innerHTML=`<span class="ks-qe-title">EDIT</span><button type="button" class="ks-qe-action" data-qe-rotate="1">↻ ROTATE</button><button type="button" class="ks-qe-action" data-qe-duplicate="1">⧉ DUP</button><button type="button" class="ks-qe-action primary" data-qe-build="1">➜ BUILD</button>${quickBuild.pieces.map(piece=>`<button type="button" class="ks-qe-piece" data-qe-type="${piece.type}" title="Replace with ${piece.label}">${piece.slot} ${piece.glyph}</button>`).join('')}`;
  ...
  return Object.freeze({
    id:'quickEdit',version:'studio-quick-edit-v1.0.0-contextual',
    canEdit:()=>!!selectedEditable(),getSelected:()=>copy(selectedEditable()),
    rotateSelected,duplicateSelected,replaceSelected,continueBuild,destroy
  });
}
