...
export function createTransformTool(kernel) {
  ...
  function begin(entityId,{useSelection=true}={}) {
    ...
    const anchor=rows.find(row=>row.entityId===entity.id)||rows[0];state={entityId:entity.id,anchorFrom:{...anchor.from},rows};return snapshot();
  }

  function previewMove(x,y,{snap=1}={}){
    if(!state)return null;const s=Math.max(1,Number(snap)||1),tx=Math.round((Number(x)||0)/s)*s,ty=Math.round((Number(y)||0)/s)*s,dx=tx-(Number(state.anchorFrom.x)||0),dy=ty-(Number(state.anchorFrom.y)||0);
    for(const row of state.rows){row.preview.x=(Number(row.from.x)||0)+dx;row.preview.y=(Number(row.from.y)||0)+dy;}return snapshot();
  }
  ...
  async function commit(){
    ...
    for(const row of current.rows){const moved=Number(row.from.x)!==Number(row.preview.x)||Number(row.from.y)!==Number(row.preview.y);if(moved)commands.push(createMoveEntityCommand(row.entityId,{x:row.preview.x,y:row.preview.y}));const rotated=Number(row.from.rotation||0)!==Number(row.preview.rotation||0);if(rotated){const e=find(row.entityId);commands.push(createPatchEntityCommand(row.entityId,{transform:{...(e?.transform||{}),rotation:row.preview.rotation}}));}}
