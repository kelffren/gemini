...
export function createCommandBus({ history, onAfterExecute, onRollback } = {}) {
  ...
  async function execute(command, context) {
    ...
    try {
      ...
      history.push({ type: command.type || 'anonymous', label: command.label || command.type || 'Command', serialized, affectedRects: event.affectedRects, mergeKey: command.historyMergeKey || null, mergeWindowMs: command.historyMergeWindowMs || 0, mergeSerialized: typeof command.historyMergeSerialized === 'function' ? command.historyMergeSerialized : null, undo: () => command.undo(context), redo: () => typeof command.redo === 'function' ? command.redo(context) : command.execute(context) });
