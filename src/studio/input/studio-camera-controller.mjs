...
export function createStudioCameraController({root=globalThis,onNavigateStart=()=>{},isUi=()=>false}={}){
  ...
  const pointers=new Map();let enabled=true,zoom=1,space=false,mousePan=null,pinch=null;
  ...
  function pointerdown(e){if(!enabled||isUi(e))return;if(e.pointerType==='touch'){pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});if(pointers.size===2){onNavigateStart();const [a,b]=[...pointers.values()],cx=(a.x+b.x)/2,cy=(a.y+b.y)/2;pinch={cx,cy,d:Math.max(1,Math.hypot(a.x-b.x,a.y-b.y)),zoom};consume(e);}return;}if(e.button===1||(e.button===0&&space)){onNavigateStart();mousePan={id:e.pointerId,x:e.clientX,y:e.clientY};consume(e);}}
  function pointermove(e){if(!enabled)return;if(e.pointerType==='touch'&&pointers.has(e.pointerId)){pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});if(pointers.size>=2&&pinch){const [a,b]=[...pointers.values()].slice(0,2),cx=(a.x+b.x)/2,cy=(a.y+b.y)/2,d=Math.max(1,Math.hypot(a.x-b.x,a.y-b.y));panScreen(cx-pinch.cx,cy-pinch.cy);setZoom(pinch.zoom*(d/pinch.d),{anchorX:cx,anchorY:cy});pinch={...pinch,cx,cy};consume(e);}return;}if(mousePan&&e.pointerId===mousePan.id){const dx=e.clientX-mousePan.x,dy=e.clientY-mousePan.y;mousePan.x=e.clientX;mousePan.y=e.clientY;panScreen(dx,dy);consume(e);}}
  function pointerup(e){if(e.pointerType==='touch'){pointers.delete(e.pointerId);if(pointers.size<2)pinch=null;return;}if(mousePan&&e.pointerId===mousePan.id){mousePan=null;consume(e);}}
  function resume(){if(enabled)return;enabled=true;freezeFollow();applyCanvasZoom();}
  function suspend(){if(!enabled)return;enabled=false;restoreFollow();mousePan=null;pinch=null;pointers.clear();if(canvas){canvas.style.transform=saved.transform;canvas.style.transformOrigin=saved.transformOrigin;}}
