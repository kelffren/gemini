...
export function createStudioNudgeController({root=globalThis,kernel}={}){
  ...
  function syncPad(){if(!pad)return;const visible=(root.innerWidth||9999)<=760&&canTouchNudge();pad.hidden=!visible;const modeButton=pad.querySelector('[data-nudge-mode]');if(modeButton){modeButton.dataset.mode=mobileMode;modeButton.textContent=mobileMode==='fine'?'1 PX':mobileMode==='coarse'?'4×':'SNAP';modeButton.setAttribute('aria-label',`Nudge ${mobileMode}`);}pad.querySelectorAll('[data-nudge-dir]').forEach(button=>button.disabled=busy||!visible);}
