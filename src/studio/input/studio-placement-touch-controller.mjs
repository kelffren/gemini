...
export function createStudioPlacementTouchController({root=globalThis,placement}={}){
  ...
  function ensure(){
    ...
    if(!pad?.isConnected){
      ...
      pad.addEventListener('click',onClick);host.appendChild(pad);
    }
    return pad;
  }
  ...
  function cancel(){if(!preview)return false;placement.cancel?.();pulse();return true;}

  function onClick(event){
    const dir=event.target?.closest?.('[data-place-dir]')?.dataset.placeDir;
    if(dir){const map={up:[0,-1],down:[0,1],left:[-1,0],right:[1,0]};move(...map[dir]);return;}
  ...
  const unsubscribe=placement.onPreview(next=>{preview=next;sync();});
  const onResize=()=>sync();root.addEventListener?.('resize',onResize,{passive:true});
