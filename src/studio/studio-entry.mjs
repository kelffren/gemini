...
export async function bootKeloStudio({ mode = 'world', actorId = null, document = null, root = globalThis } = {}) {
  ...
  session = Object.freeze({ version: 'kelo-studio-foundation-v1.5.0', mode, actorId, kernel, tools, overlayRenderer, assetPreview, compiler, worker, store, profiler, adapter,
    ...
    close(){unsubscribeJournal();worker.close();profiler.close();assetPreview.close();store.close().catch(()=>{});session=null;}
  });
  return session;
}
