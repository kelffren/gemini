...
export function createAvatarFrameProjectStore({root=globalThis}={}){
  ...
  async function database(){
    ...
    if(!dbPromise){
      dbPromise=openDatabase(root).then(db=>{if(!db)return null;db.onversionchange=()=>{try{db.close();}catch{}dbPromise=null;};try{db.onclose=()=>{dbPromise=null;};}catch{}return db;}).catch(error=>{dbPromise=null;throw error;});
    }
    return dbPromise;
  }
  ...
  async function saveProject(project){
    ...
    return serializeWrite(async()=>{const db=await database();if(!db){memory(root).projects.set(project.id,value);return clone(value);}const tx=db.transaction(PROJECTS,'readwrite'),done=transactionDone(tx),req=tx.objectStore(PROJECTS).put(value);await Promise.all([request(req),done]);return clone(value);});
  }
  ...
  async function putSource(key,blob,metadata={}){
    ...
    return serializeWrite(async()=>{const db=await database();if(!db){memory(root).sources.set(value.key,value);return value;}let stored=value;if(typeof blob.arrayBuffer==='function'){const bytes=await blob.arrayBuffer();stored={key:value.key,bytes,blobType:String(blob.type||metadata.type||''),metadata:value.metadata,updatedAt:value.updatedAt};}const tx=db.transaction(SOURCES,'readwrite'),done=transactionDone(tx),req=tx.objectStore(SOURCES).put(stored);await Promise.all([request(req),done]);return value;});
  }
  ...
  async function removeSource(key){
    return serializeWrite(async()=>{const db=await database();if(!db){memory(root).sources.delete(String(key));return;}const tx=db.transaction(SOURCES,'readwrite'),done=transactionDone(tx),req=tx.objectStore(SOURCES).delete(String(key));await Promise.all([request(req),done]);});
  }
