...
export function createAvatarFrameProjectStore({root=globalThis}={}){
  ...
  async function database(){
    ...
    if(!dbPromise){
      dbPromise=openDatabase(root).then(db=>{if(!db)return null;db.onversionchange=()=>{try{db.close();}catch{}dbPromise=null;};db.onclose=()=>{dbPromise=null;};return db;}).catch(error=>{dbPromise=null;throw error;});
    }
    return dbPromise;
  }
  ...
  async function read(storeName,key=null,{all=false}={}){
    ...
    const tx=db.transaction(storeName,'readonly'),done=transactionDone(tx),store=tx.objectStore(storeName),value=await request(all?store.getAll():store.get(key));await done;return value;
  }
  async function saveProject(project){
    if(!project?.id)throw new Error('FRAME_PROJECT_ID_REQUIRED');const value=clone(project);
    return serializeWrite(async()=>{const db=await database();if(!db){memory(root).projects.set(project.id,value);return clone(value);}const tx=db.transaction(PROJECTS,'readwrite'),done=transactionDone(tx);tx.objectStore(PROJECTS).put(value);await done;return clone(value);});
  }
  ...
  async function deleteProject(id,{deleteSources=false}={}){
    const existing=deleteSources?await loadProject(id):null;
    return serializeWrite(async()=>{const db=await database();if(!db){const state=memory(root);state.projects.delete(id);if(deleteSources)for(const slot of existing?.slots||[])if(slot.sourceKey)state.sources.delete(slot.sourceKey);return;}const tx=db.transaction(deleteSources?[PROJECTS,SOURCES]:[PROJECTS],'readwrite'),done=transactionDone(tx);tx.objectStore(PROJECTS).delete(id);if(deleteSources)for(const slot of existing?.slots||[])if(slot.sourceKey)tx.objectStore(SOURCES).delete(slot.sourceKey);await done;});
  }
  async function putSource(key,blob,metadata={}){
    if(!key||!blob)throw new Error('FRAME_PROJECT_SOURCE_REQUIRED');const value={key:String(key),blob,metadata:{...metadata},updatedAt:Date.now()};
    return serializeWrite(async()=>{const db=await database();if(!db){memory(root).sources.set(value.key,value);return value;}const tx=db.transaction(SOURCES,'readwrite'),done=transactionDone(tx);tx.objectStore(SOURCES).put(value);await done;return value;});
  }
  async function getSource(key){const value=await read(SOURCES,String(key));return value||null;}
  async function removeSource(key){
    return serializeWrite(async()=>{const db=await database();if(!db){memory(root).sources.delete(String(key));return;}const tx=db.transaction(SOURCES,'readwrite'),done=transactionDone(tx);tx.objectStore(SOURCES).delete(String(key));await done;});
  }
  ...
  return F({version:'avatar-frame-project-store-v1.1.0-webkit-stable',saveProject,loadProject,listProjects,deleteProject,putSource,getSource,removeSource,close});
}
