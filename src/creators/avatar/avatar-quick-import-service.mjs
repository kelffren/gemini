 ...
 */
import { compileUniversalAvatarRuntime } from './kelo-universal-asset-compiler.mjs';
...
export function createAvatarQuickImportService({contentSession,contentRepository,contentService,root=globalThis}={}){
  ...
  const compileBridge=createCompileBridge(root);
  ...
  async function importAndUse(file,analysis,{displayName='Mi Avatar',onProgress=null,signal=null}={}){
    ...
    const sourceHash=await sha256(file,root),runtimeHash=await sha256(compiled.blob,root),ext=compiled.type==='image/png'?'png':'webp',slug=clean(`avatar-${sourceHash.slice(0,16)}`),runtimePath=`${uid}/characters/${runtimeHash.slice(0,24)}.${ext}`,publicUrl=contentRepository.publicUrl('avatars',runtimePath);
