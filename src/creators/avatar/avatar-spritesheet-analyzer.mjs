...
export async function compileAvatarRuntime(file,config,{root=globalThis}={}){
  ...
  if(config?.removeBackground){const image=ctx.getImageData(0,0,width,height),d=image.data,bg=(config.backgroundRgb||[255,255,255]).map(Number);for(let i=0;i<d.length;i+=4){const dist=Math.hypot(d[i]-bg[0],d[i+1]-bg[1],d[i+2]-bg[2]);if(dist<28)d[i+3]=0;else if(dist<64)d[i+3]=Math.round(d[i+3]*clamp((dist-28)/36,0,1));}ctx.putImageData(image,0,0);}
