...
export function installAbilityTouchTuner(session,{root=globalThis}={}){
  ...
  const onChange=event=>{if(node.contains(event?.target))return;root.setTimeout?.(()=>refresh(),80);};session.shell.root?.addEventListener?.('change',onChange,true);session.shell.root?.addEventListener?.('click',onChange,true);
