...
function frameList(value,columns){
  ...
  return raw.map(value=>clampFrame(value,columns));
}
...
export function resolveCharacterTestTrack({row,state='idle',columns=1}={}){
  const def=STATE_BY_ID.get(String(state))||STATE_BY_ID.get('idle'),cols=Math.max(1,Math.floor(finite(columns,1))),mapped=mappedValue(row,def.id);let frames=[],loop=def.loop,frameMs=def.frameMs,source='idle-fallback',authored=false;
  if(mapped){const raw=mapped.value,obj=raw&&typeof raw==='object'&&!Array.isArray(raw)?raw:null;frames=frameList(raw,cols);loop=obj?.loop==null?def.loop:obj.loop===true;frameMs=Math.max(70,Math.min(2000,finite(obj?.frameMs??obj?.speedMs??obj?.ms,def.frameMs)));source=mapped.source;authored=frames.length>0;}
...
export function createCreatorCharacterTestChamber({root=globalThis,preview,onSnapshot=null}={}){
  ...
  let row=null,source='',asset={},face='down',state='idle',playing=false,startedAt=0,lastFrame=null,lastPreview=null,lastTrack=resolveCharacterTestTrack(),raf=0,disposed=false;
  ...
  function emit(){const snapshot=Object.freeze({version:'creator-character-test-chamber-v1',state,playing,face,frame:lastFrame,track:lastTrack,preview:lastPreview});try{onSnapshot?.(snapshot);}catch{}return snapshot;}
  async function paint(frame,trackValue){if(disposed)return null;lastFrame=frame;try{const rendered=await preview.render({row,source,asset,face,motion:state,frame});if(disposed)return null;lastPreview=rendered;lastTrack=trackValue;return emit();}catch(error){lastPreview=null;lastTrack=trackValue;const snapshot=emit();try{onSnapshot?.(Object.freeze({...snapshot,error:String(error?.message||error)}));}catch{}return snapshot;}}
  ...
  function restart(){stopRaf();startedAt=now(root);lastFrame=null;lastTrack=track();if(!row)return emit();playing=true;if(rafFn)raf=rafFn(step);else void paint(lastTrack.frames[0]??0,lastTrack);return emit();}
  function configure(next={}){row=next.row??row;source=next.source??source;asset=next.asset??asset;face=String(next.face||face||'down');state=STATE_BY_ID.has(String(next.state))?String(next.state):state;const shouldPlay=next.playing!==false;const snapshot=restart();if(!shouldPlay){playing=false;stopRaf();void paint(lastTrack.frames[0]??0,lastTrack);}return snapshot;}
  ...
  function snapshot(){return Object.freeze({version:'creator-character-test-chamber-v1',state,playing,face,frame:lastFrame,track:lastTrack,preview:lastPreview});}
  function dispose(){disposed=true;playing=false;stopRaf();row=null;source='';asset={};lastPreview=null;}
  return Object.freeze({version:'creator-character-test-chamber-v1',states:STATE_DEFS,configure,play,pause,reset,setFace,snapshot,dispose});
}
