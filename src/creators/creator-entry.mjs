...
import { registerWorldWorkspace } from './workspaces/world-workspace.mjs?v=world-bridge-20260915-21';
...
let platform=null;
function ensureCreatorInputLocks(root){if(root?.KeloInputLocks?.acquire&&root?.KeloInputLocks?.release)return()=>{};const held=new Set();let seq=0;const fallback={__keloCreatorFallback:true,acquire(owner='creator-workspace',meta=null){const token=Object.freeze({id:`creator-lock-${++seq}`,owner:String(owner),meta});held.add(token);return token;},release(token){return held.delete(token);},isLocked(){return held.size>0;},has(token){return held.has(token);},clear(){held.clear();},get size(){return held.size;}};try{root.KeloInputLocks=fallback;}catch{}return()=>{held.clear();try{if(root.KeloInputLocks===fallback)delete root.KeloInputLocks;}catch{}};}
export async function bootKeloCreators({root=globalThis,stateAdapter=null}={}){
  ...
  async function ensureSpriteAbilityExtensions(){if(spriteAbilityExtensionsPromise)return spriteAbilityExtensionsPromise;spriteAbilityExtensionsPromise=(async()=>{try{const visualUi=await import('./sprite-ability/sprite-ability-visual-ui.mjs');if(typeof visualUi.installSpriteAbilityVisualUI==='function')visualUiDispose=visualUi.installSpriteAbilityVisualUI({root});}catch(error){console.warn('[Creators] Sprite Ability visual UI unavailable',error);}try{const manualCutter=await import('./sprite-ability/sprite-ability-manual-cutter.mjs');if(typeof manualCutter.installSpriteAbilityManualCutter==='function')manualCutterDispose=manualCutter.installSpriteAbilityManualCutter({root});}catch(error){console.warn('[Creators] Sprite Ability manual cutter unavailable',error);}try{const repairStudio=await import('./sprite-ability/sprite-ability-repair-studio.mjs');if(typeof repairStudio.installSpriteAbilityRepairStudio==='function')repairStudioDispose=repairStudio.installSpriteAbilityRepairStudio({root});}catch(error){console.warn('[Creators] Sprite Ability repair studio unavailable',error);}try{const repairTouch=await import('./sprite-ability/sprite-ability-repair-touch.mjs');if(typeof repairTouch.installSpriteAbilityRepairTouch==='function')repairTouchDispose=repairTouch.installSpriteAbilityRepairTouch({root});}catch(error){console.warn('[Creators] Sprite Ability repair touch unavailable',error);}try{const irregularImport=await import('./sprite-ability/sprite-ability-irregular-import.mjs');if(typeof irregularImport.installSpriteAbilityIrregularImport==='function')irregularImportDispose=irregularImport.installSpriteAbilityIrregularImport({root});}catch(error){console.warn('[Creators] Sprite Ability irregular import unavailable',error);}try{const looseImport=await import('./sprite-ability/sprite-ability-loose-import.mjs');if(typeof looseImport.installSpriteAbilityLooseImport==='function')looseImportDispose=looseImport.installSpriteAbilityLooseImport({root});}catch(error){console.warn('[Creators] Sprite Ability loose import unavailable',error);}try{const eventLab=await import('./sprite-ability/sprite-ability-event-lab.mjs');if(typeof eventLab.installSpriteAbilityEventLab==='function')eventLabDispose=eventLab.installSpriteAbilityEventLab({root});}catch(error){console.warn('[Creators] Sprite Ability event lab unavailable',error);}try{const easyUi=await import('./sprite-ability/sprite-ability-easy-ui.mjs');if(typeof easyUi.installSpriteAbilityEasyUI==='function')easyUiDispose=easyUi.installSpriteAbilityEasyUI({root});}catch(error){console.warn('[Creators] Sprite Ability easy UI unavailable',error);}return true;})();return spriteAbilityExtensionsPromise;}
  ...
  async function openWorkspace(id,context={}){const manifest=workspaces.resolve(id);if(!manifest)throw new Error(`CREATOR_WORKSPACE_NOT_FOUND:${id}`);if(manifest.capability)permission.require(manifest.capability,permission.actorId(),context.projectId||null);if(id==='sprite-ability')await ensureSpriteAbilityExtensions();return workspaces.open(id,{root,projects,permission,dependencies,contentSession,contentRepository,contentService,runtimeContent,avatarRuntime,avatarQuick,openWorkspace,...context});}
  platform=Object.freeze({version:'kelo-creators-core-v1.24.0-world-a10',permission,projects,workspaces,dependencies,contentSession,contentRepository,contentService,runtimeContent,avatarRuntime,avatarQuick,openWorkspace,close(){try{looseImportDispose?.();}catch{}try{irregularImportDispose?.();}catch{}try{repairTouchDispose?.();}catch{}try{repairStudioDispose?.();}catch{}try{manualCutterDispose?.();}catch{}try{easyUiDispose?.();}catch{}try{eventLabDispose?.();}catch{}try{visualUiDispose?.();}catch{}try{localState.close?.();}catch{}try{inputLocksDispose?.();}catch{}platform=null;}});
  return platform;
}
