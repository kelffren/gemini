 ...
 * keys: EXACT GRID PRENORMALIZED ROWS COLUMNS FRAME SIZE BRIDGE
 ...
 * lazy: Sprite Ability live controller is loaded only when normalized-grid application is actually requested
 */
...
async function currentBuilder(){return (await controller()).getSpriteAbilityBuilder?.()||null;}
...
export async function applyNormalizedGrid({root=globalThis,workspace,fileName,columns,rows,frameWidth,frameHeight,startFrame=0,endFrame=null,timeout=8000}={}){
  ...
  while(Date.now()-started<timeout){builder=await currentBuilder();const current=String(builder?.draft?.sheet?.fileName||'');if(builder&&(!needle||current.includes(needle)))break;await sleep(root,35);}
  ...
  builder=await currentBuilder();builder?.setFrame?.(start,{pause:true,center:true});await builder?.save?.();
