...
export async function applyNormalizedGrid({root=globalThis,workspace,fileName,columns,rows,frameWidth,frameHeight,startFrame=0,endFrame=null,timeout=8000}={}){
  ...
  while(Date.now()-started<timeout){builder=getSpriteAbilityBuilder();const current=String(builder?.draft?.sheet?.fileName||'');if(builder&&(!needle||current.includes(needle)))break;await sleep(root,35);}
  ...
  builder=getSpriteAbilityBuilder();builder?.setFrame?.(start,{pause:true,center:true});await builder?.save?.();
