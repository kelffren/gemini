 ...
 * keys: MINIMIZE TEST IN GAME RUNTIME PREVIEW GENERATED ANIMATION AVATAR
 * purpose: decorate the current Sprite Ability Combat Lab without replacing it, add explicit minimize labeling and live generated-animation playback on the local actor
 * does-not-own: Sprite Ability editing, combat authority, damage resolution, publishing, server state
 * reuse: current Sprite Ability Builder + AnimationPreviewAdapter + KeloAnimation + KeloAvatar
 */
...
#kelo-studio-workspace.sab-runtime-test .ksw-main,#kelo-studio-workspace.sab-runtime-test .ksw-mobile-tabs,#kelo-studio-workspace.sab-runtime-test .ksw-timeline,#kelo-studio-workspace.sab-runtime-test .ksw-status{display:none!important}
...
@media(max-width:760px){.sab-runtime-controls{width:calc(100vw - 28px);justify-content:center}.sab-runtime-controls strong{display:none}}
`;doc.head.append(style);
  ...
  function relabelMinimize(){for(const button of workspaceRoot.querySelectorAll('[data-act="preview"],[data-act="preview-mobile"]')){if(button.getAttribute('aria-pressed')!=='true'&&button.textContent.trim()!=='MINIMIZE')button.textContent='MINIMIZE';button.title=button.getAttribute('aria-pressed')==='true'?'Restore editor':'Minimize editor and focus preview';}}
  ...
  function cleanup(destroyCore=false){if(cleaning)return true;cleaning=true;stopRuntimePreview();observer?.disconnect();controls.remove();style.remove();if(active?.projectId===core.projectId)active=null;const result=destroyCore?core.destroy?.():true;return result;}
  ...
  observer=new MutationObserver(()=>decorate());observer.observe(doc.body,{childList:true,subtree:true});decorate();

  active=Object.freeze({version:'sprite-ability-runtime-extension-v1.0.0',coreVersion:core.version,projectId:core.projectId,workspaceRoot,get draft(){return core.draft;},get runtimeTest(){return runtimeTest?{animationId:runtimeTest.animationId,animationProjectId:runtimeTest.animationProjectId,abilityProjectId:runtimeTest.abilityProjectId,startedAt:runtimeTest.startedAt}:null;},generate:()=>core.generate(),testInGame,replayRuntime,destroy:()=>cleanup(true),close:()=>cleanup(true)});
