...
export function createCreatorContentDelivery({root=globalThis,runtimeRegistry=null}={}){
  ...
  async function ensureOwner(contentType){
    ...
    try{root.KELO_CREATOR_USE_AUTHORITY?.attachRuntimeGuards?.();}catch(error){console.warn('[Creator delivery] use guard attach failed',error);}
  }
  ...
  function diagnostics(){return F({version:'creator-content-delivery-v1.0.1-use-guards',provider:provider?.name||'KeloOnlineAuth',accountId:accountId()||null,cachedManifests:cache.size,inflight:inflight.size,registryVersion:registry?.version||root.KELO_CREATOR_CONTENT_REGISTRY?.version||null});}
  const api=F({version:'creator-content-delivery-v1.0.1-use-guards',bindProvider,getManifest,activateRevision,invalidateIdentity,diagnostics});return api;
}
