...
export function createRuntimeContentRegistry({root=globalThis}={}){
  ...
  function access(record){
    const gate=root.KeloCreatorEntitlements;if(!gate?.checkRecord)return{ok:false,reason:'CREATOR_ENTITLEMENT_GUARD_REQUIRED'};
  ...
  function adapt(record){
    const permission=access(record);if(!permission.ok)return{status:'restricted',owner:'KeloCreatorEntitlements',reason:permission.reason,revisionId:record.revisionId||null};
    const type=String(record.contentType||'');if(type==='world'||type==='tile')return adaptWorld(record);if(type==='appearance'||type==='equipment')return adaptAppearance(record);if(type==='mount')return adaptMount(record);if(type==='character')return adaptCharacter(record);return{status:'active',owner:'KELO_CREATOR_CONTENT_REGISTRY',runtimeId:runtimeId(record),note:'Semantic definition is entitlement-approved; specialized runtime adapter can consume it by contentType.'};
  }
  function publishRow(base,activation){const row=F({...base,activation:F(copy(activation))});rows.set(base.contentId,row);listeners.forEach(fn=>{try{fn(row);}catch{}});try{root.dispatchEvent?.(new CustomEvent('kelo:creator-content-ready',{detail:{contentId:base.contentId,contentType:row.contentType,activation:row.activation}}));}catch{}return row;}
  ...
  function reactivate(id){const current=rows.get(String(id));if(!current)return null;if(current.activation?.status==='active')return current;const base=F({...current,activation:undefined});delete base.activation;return publishRow(base,adapt(base));}
