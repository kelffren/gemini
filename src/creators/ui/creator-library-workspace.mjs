 ...
 * keys: CREATOR LIBRARY CHARACTER SKIN WEAPON ARMOR PROP MARKETPLACE DISCOVER TOOLS IMAGE LAB ROUTING MOBILE LAZY
 * purpose: present one mobile-first creation library and route authoring/market intent to existing specialized owners
 * public-api: openCreatorLibraryWorkspace, getCreatorLibraryWorkspace, closeCreatorLibraryWorkspace
 * consumes: Creator workspace registry/openWorkspace, creator-content-types, project repository, Creator Marketplace service, KeloInputLocks
 * state-owned: ephemeral library UI state only; projects/assets/listings/wallets remain owned by existing systems
 * extension-points: creator-content-types.mjs + workspace manifests + lazy marketplace surface
 * reuse: direct Luxe Creator entry for create/library/market/tools
 * online: market tab delegates to server-backed Creator Marketplace; Library itself is never economy authority
 * do-not: no second runtime catalog, no gameplay writes, no local market authority, no eager marketplace/assets boot
 */
...
const TOOL_ROWS=Object.freeze([
  ...
  Object.freeze({id:'content-studio',icon:'⇪',label:'Content Studio',detail:'Lotes/metadata · release/review online'})
]);
...
export async function openCreatorLibraryWorkspace({root=globalThis,projects=null,permission=null,workspaces=null,openWorkspace=null,marketplace=null}={}){
  ...
  const close=el(doc,'button',{class:'kcl-close',text:'CLOSE'}),head=el(doc,'header',{class:'kcl-head'},[
    el(doc,'div',{class:'kcl-mark',text:'CL'}),el(doc,'div',{class:'kcl-title'},[el(doc,'strong',{text:'KELO CREATOR LIBRARY'}),el(doc,'small',{text:'Create · prepare · publish · discover · own'})]),close
  ]),tabs=el(doc,'nav',{class:'kcl-tabs'}),main=el(doc,'main',{class:'kcl-main'});shell.append(head,tabs,main);doc.body.append(shell);
  const tabRows=[['create','CREATE'],['library','MY LIBRARY'],['market','MARKET'],['tools','TOOLS'],['pipeline','PIPELINE']],buttons=new Map();let current='create',busy=false;
  ...
  async function renderMarket(){
    const wrap=el(doc,'div',{class:'kcl-shell'});main.replaceChildren(wrap);if(!marketplace){wrap.append(el(doc,'div',{class:'kcl-empty',text:'Creator Marketplace service unavailable in this Creator session.'}));return;}
    wrap.append(el(doc,'div',{class:'kcl-empty',text:'Loading marketplace on demand…'}));
    try{const mod=await import('./creator-marketplace-surface.mjs');if(current!=='market')return;await mod.mountCreatorMarketplaceSurface({root,container:wrap,marketplace,openWorkspace:(id,context)=>launch(id,context)});}catch(error){console.error('[Creator Library market]',error);wrap.replaceChildren(el(doc,'div',{class:'kcl-empty',text:`Marketplace unavailable · ${String(error?.message||error)}`}));}
  }
  ...
  function renderPipeline(){
    const wrap=el(doc,'div',{class:'kcl-shell'});wrap.append(hero('Creator pipeline','El contenido se crea local/privado, pasa por review/publicación y solo entonces entra al marketplace autoritativo.'));
    const steps=[['1 · IDEA','Elige el tipo de contenido y conserva la intención del creador.'],['2 · PREP','Image Lab / Pixel Forge preparan fuentes y arte sin mutar el runtime.'],['3 · BUILD','El workspace especializado crea definition/asset/animation usando su owner.'],['4 · TEST','Preview o test draft valida antes de contenido compartido.'],['5 · REVIEW / PUBLISH','Release Center somete; el servidor aprueba y publica.'],['6 · MARKET','Solo revisions PUBLISHED pueden listarse; compra KC crea entitlement autoritativo.']],row=el(doc,'div',{class:'kcl-pipeline'});for(const [title,copy] of steps)row.append(el(doc,'div',{class:'kcl-step'},[el(doc,'b',{text:title}),el(doc,'span',{text:copy})]));wrap.append(row,el(doc,'div',{class:'kcl-note',text:`Registrados ${CREATOR_CONTENT_TYPES.length} tipos de entrada. Regla: CONTENT EXISTS ≠ PUBLISHED ≠ OWNED. Discover es metadata-first; assets completos se cargan solo bajo demanda.`}));main.replaceChildren(wrap);
  }
  async function render(id){current=id;for(const [key,b] of buttons)b.setAttribute('aria-selected',String(key===id));if(id==='create')renderCreate();else if(id==='library')await renderLibrary();else if(id==='market')await renderMarket();else if(id==='tools')renderTools();else renderPipeline();}
  ...
  active=Object.freeze({version:'kelo-creator-library-v1.2-marketplace',shell,openType:typeId=>{const row=CREATOR_CONTENT_TYPES.find(x=>x.id===typeId);return row?launch(row.workspaceId,{creatorIntent:{typeId:row.id,defaults:row.defaults}}):null;},show:render,close:destroy,get tab(){return current;}});
