...
async function fetchStatus(root,url,panel){
  try{const {ok,status,data}=await requestCreatorJson(root,url,{headers:{Accept:'application/json'}});if(!ok)throw new Error(data?.error||`HTTP_${status}`);const ready=!!data?.configured;setBadge(panel,ready?'SUPABASE · ZERO GPU READY':'SUPABASE · SPACE SETUP REQUIRED',ready);return data||{configured:false};}catch(error){setBadge(panel,'SUPABASE EDGE · OFFLINE',false);return{configured:false,error:String(error?.message||error)};}
}
...
export async function openSpriteFactoryOnline({root=globalThis}={}){
  ...
  const endpoint=endpointFromRuntime(root);endpointInput.value=endpoint;endpointInput.readOnly=true;if(note)note.textContent='Supabase Edge mode: no depende de Render. Image Treatment limpia alpha/halos; Sprite Compiler corrige geometría; Skeleton Lab usa ZeroGPU cuando los Spaces están configurados.';
  ...
  async function installGenerated(data){
    ...
    try{root.__KELO_SPRITE_COMPILER_LAST__={version:'v1.7-supabase-edge',report:compiler,source:[repaired.sourceWidth,repaired.sourceHeight],output:[repaired.canvas.width,repaired.canvas.height],repairTargets,imageTreatment:treatment.report,sourceImageTreatment:lastSourceTreatment};}catch{}return qa;
  }
  ...
  async function installDirectionRow(data,direction){
    ...
    try{root.__KELO_SPRITE_COMPILER_LAST__={version:'v1.7-supabase-edge',mode:'direction-row',direction,report:compiler,output:[factory.sheet.width,factory.sheet.height],repairTargets,imageTreatment:treatment.report,sourceImageTreatment:lastSourceTreatment};}catch{}return qa;
  }
  ...
  async function finishQueuedJob(token,data){
    ...
    for(let attempt=0;current?.pending&&attempt<4;attempt++){
      aiBtn.textContent=attempt===0?'ZERO GPU QUEUED…':`ZERO GPU WAIT ${attempt+1}/4…`;
  ...
  try{root.__KELO_SPRITE_FACTORY_ONLINE__={version:'v1.7-supabase-edge',endpoint,status,imageTreatment:'image-treatment-v1.0.0',skeletonLab,transport:'supabase-edge'};}catch{}
  return factory;
}