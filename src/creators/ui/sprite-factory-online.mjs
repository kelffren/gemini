 ...
 * purpose: connect Kelo server AI generation to local background cleanup, frame normalization, feet anchoring and QA
...
export async function openSpriteFactoryOnline({root=globalThis}={}){
  ...
  const endpoint=endpointFromRuntime(root);endpointInput.value=endpoint;endpointInput.readOnly=true;if(note)note.textContent='Online mode: Kelo Server generates the raw 4×8 atlas; Kelo Sprite Compiler then removes flat backgrounds, normalizes every frame to one common scale, anchors the feet to one baseline and runs local QA before you use it.';
  ...
  async function installGenerated(data){
    ...
    const repaired=repairSpritesheetImage(root,img,{columns:4,rows:8,targetWidth:64,targetHeight:64,padding:4,removeBackground:true,colorThreshold:34,softEdge:14,imageSmoothing:true});
    ...
    const baseQA=factory.runQA(),qa=Object.freeze({...baseQA,compiler:repaired.report});const tag=sheetPanel?.querySelector('.ksf-pill');if(tag)tag.textContent=`${data.sourceMode==='reference-edit'?'AI · REFERENCE':'AI · GENERATED'} · ${repaired.report.pass?'COMPILED':'CHECK'}`;
    try{root.__KELO_SPRITE_COMPILER_LAST__={version:'v1',report:repaired.report,source:[repaired.sourceWidth,repaired.sourceHeight],output:[repaired.canvas.width,repaired.canvas.height]};}catch{}
    return qa;
  }
  ...
  aiBtn.onclick=async()=>{
    ...
    try{
      ...
      const ok=qa?.passTechnical&&qa?.passArt&&qa?.passMotion&&qa?.compiler?.pass;setBadge(settingsPanel,ok?'SPRITE COMPILER · QA PASS':'SPRITE COMPILER · REVIEW',ok);if(typeof root.showToast==='function')root.showToast(ok?'Sprite Compiler: generado, reparado y QA aprobado':'Sprite Compiler: reparado; revisa los checks antes de usarlo.');
  ...
  const status=await fetchStatus(root,endpoint,settingsPanel);try{root.__KELO_SPRITE_FACTORY_ONLINE__={version:'v1.1-compiler',endpoint,status};}catch{}
  return factory;
}
