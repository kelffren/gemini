...
const CATALOG=Object.freeze([
  ...
  {category:'GAMEPLAY',items:[['mount','Mount','active'],['ability','Ability','active'],['sprite-ability','Sprite Ability','active'],['npc','NPC','soon'],['quest','Quest / Dialogue','soon'],['item','Item','soon'],['crafting','Crafting','soon']]},
  {category:'VISUAL',items:[['avatar','Avatar','active'],['appearance','Appearance','active'],['animation','Animation','active'],['vfx','VFX','active'],['cinematic','Cinematic','soon']]},
  {category:'CONTENT',items:[['content-studio','Content Studio','active'],['prefab','Prefab','soon'],['environment','Environment','soon'],['audio','Audio','soon']]}
]);
const REPOSITORY_PROJECT_TYPES=Object.freeze({animation:'ANIMATION',vfx:'VFX',ability:'ABILITY','sprite-ability':'SPRITE_ABILITY'});
const PROJECT_LABELS=Object.freeze({ANIMATION:'Animation',VFX:'VFX',ABILITY:'Ability',SPRITE_ABILITY:'Sprite Ability'});
const PROJECT_WORKSPACES=Object.freeze({SPRITE_ABILITY:'sprite-ability'});
function css(){return `
...
export async function openCreatorHub({root=globalThis}={}){
  ...
  async function openWorkspace(id,{projectId=null}={}){
    ...
    try{
      // Sprite Ability owns its own zero-training project bootstrap. Route directly so one tap
      // never waits on Hub project listing/naming before the editor can render.
      if(id==='sprite-ability'&&!projectId){await platform.openWorkspace(id,{});destroy();return;}
      let resolvedProjectId=projectId;
      const projectType=REPOSITORY_PROJECT_TYPES[id]||null;
      if(projectType&&!resolvedProjectId){
        const ownerId=platform.permission.actorId(),existing=await platform.projects.list({ownerId,type:projectType}),label=PROJECT_LABELS[projectType]||projectType,project=await platform.projects.create({type:projectType,name:`${label} ${existing.length+1}`,ownerId});
  ...
  async function renderCreate(){main.replaceChildren(make('h1',{text:'Create'}),make('p',{class:'kc-lead',text:'Choose a workspace. Every active Creator reuses shared Kelo Studio infrastructure and its runtime owner.'}));for(const group of CATALOG){const sec=make('section',{class:'kc-section'},[make('h2',{text:group.category})]),grid=make('div',{class:'kc-grid'});for(const [wid,label,state] of group.items){const manifest=platform.workspaces.resolve(wid),implemented=state==='active'&&manifest?.availability==='active',permitted=implemented&&(!manifest.capability||platform.permission.can(manifest.capability,platform.permission.actorId())),status=implemented?(permitted?'ACTIVE':'NO ACCESS'):'COMING SOON';let detail='Shared Studio core · private/local draft';if(wid==='avatar')detail='Subir → preview → usar';if(wid==='content-studio')detail='Spreadsheet · phone · Supabase · instant runtime';if(wid==='sprite-ability')detail='SUBIR → VER → PROBAR → AJUSTAR → GUARDAR';const card=make('button',{class:`kc-card ${permitted?'active':''}`,disabled:!permitted,'aria-label':permitted?`Abrir ${label}`:`${label} ${status.toLowerCase()}`},[make('span',{class:'kc-pill',text:status}),make('strong',{text:label}),make('small',{text:implemented?(permitted?detail:'Your key does not grant this workspace'):'Workspace not implemented yet'})]);if(permitted)card.onclick=()=>void openWorkspace(wid);grid.append(card);}sec.append(grid);main.append(sec);}}
  async function renderProjects(){main.replaceChildren(make('h1',{text:'My Projects'}),make('p',{class:'kc-lead',text:'Projects available through the repository boundary.'}));const rows=await platform.projects.list({ownerId:platform.permission.actorId()});if(!rows.length)return main.append(make('div',{class:'kc-empty',text:'No projects yet.'}));for(const p of rows){const row=make('div',{class:'kc-project'},[make('div',{},[make('strong',{text:p.name}),make('div',{class:'kc-lead',text:`${p.type} · ${p.status}`})])]),workspace=PROJECT_WORKSPACES[p.type]||String(p.type||'').toLowerCase(),manifest=platform.workspaces.resolve(workspace),permitted=!!manifest&&(!manifest.capability||platform.permission.can(manifest.capability,platform.permission.actorId(),p.projectId));if(permitted){const b=make('button',{text:'OPEN'});b.onclick=()=>void openWorkspace(workspace,{projectId:p.projectId});row.append(b);}main.append(row);}}
  async function renderAssets(){main.replaceChildren(make('h1',{text:'My Assets'}),make('p',{class:'kc-lead',text:'Upload spreadsheets and files into the universal online content pipeline.'}));const card=make('button',{class:'kc-card active','aria-label':'Abrir Content Studio'},[make('span',{class:'kc-pill',text:'ONLINE'}),make('strong',{text:'CONTENT STUDIO'}),make('small',{text:'CSV/XLSX + images → Supabase → runtime'})]);card.onclick=()=>void openWorkspace('content-studio');const grid=make('div',{class:'kc-grid'},[card]);main.append(grid);}
  ...
  async function render(id){current=id;for(const [key,b] of buttons)b.setAttribute('aria-selected',String(key===id));if(id==='create')return renderCreate();if(id==='projects')return renderProjects();if(id==='assets')return renderAssets();if(id==='shared')return renderEmpty('Shared With Me','Shared projects will plug into the project repository later.');if(id==='invites')return renderEmpty('Test Invites','Private test invitations will arrive through a future invite service.');if(id==='reviews')return renderEmpty('Reviews','Approval remains capability-gated and authority-owned.');if(id==='published')return renderEmpty('Published','Only immutable approved revisions will appear here.');}
  ...
  active=Object.freeze({version:'kelo-creator-hub-v1.9.0-sprite-ability-direct',hub,platform,get section(){return current;},show:render,close:destroy});await render('create');return active;
}
