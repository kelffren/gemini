...
export async function openAnimationCreator({root=globalThis,projectId=null,projects}={}){
  ...
  try{
    ...
    async function execute(command){return kernel.execute(command);}
    async function save(){const report=validateAnimationDocument(kernel.document,{assetRegistry:root.KeloAssetRegistry});if(!report.ok)throw new Error(`ANIMATION_VALIDATION_FAILED:${report.errors.join(',')}`);await projects.saveDraft(project.projectId,kernel.document);toast(root,'Animation draft saved');return report;}
    ...
    function addTrack(){const start=timeline.playhead,end=Math.min(kernel.document.clip.duration,start+Math.max(0,Number(trackWindow)||0));void guarded(()=>execute(createUpsertAnimationTrackEventCommand(trackType,{start,end,label:trackLabel||trackType,ref:trackLabel||null})));}
    function renderLeft(){const host=shell.left;host.replaceChildren();const c=kernel.document.clip;
      ...
      const duration=input(doc,'number',c.duration);duration.min='.05';duration.step='.01';duration.onchange=()=>void guarded(()=>execute(createPatchAnimationClipCommand({duration:Number(duration.value)})));host.append(field(doc,'Duration',duration));
      const priority=input(doc,'number',c.priority);priority.onchange=()=>void guarded(()=>execute(createPatchAnimationClipCommand({priority:Number(priority.value)})));host.append(field(doc,'Priority',priority));
