 ...
 * keys: SPRITE SILHOUETTE MASK JACCARD MEDOID OUTLIER FRAME CONSISTENCY
 * purpose: detect AI frames whose body mass changes implausibly after removing translation/scale differences
 * public-api: normalizedFrameMask, silhouetteDistance, auditSilhouetteConsistency
 * state-owned: none; pure pixels/frames -> QA
 * online: N/A
 * do-not: auto-redraw anatomy or treat normal pose changes as guaranteed defects
 */
...
export function normalizedFrameMask(sourceData,width,height,bounds,{sampleSize=24,alphaThreshold=12}={}){
  ...
  if(!bounds?.width||!bounds?.height)return F({size,mask,occupancy:0});
  let filled=0;
  for(let sy=0;sy<size;sy++)for(let sx=0;sx<size;sx++){
    const px=clamp(Math.floor(bounds.x+(sx+.5)*bounds.width/size),0,width-1),py=clamp(Math.floor(bounds.y+(sy+.5)*bounds.height/size),0,height-1),a=data[(py*width+px)*4+3];
    if(a>alphaThreshold){mask[sy*size+sx]=1;filled++;}
  }
  return F({size,mask,occupancy:filled/(size*size)});
}
...
export function auditSilhouetteConsistency(sourceData,width,height,frames,{sampleSize=24,alphaThreshold=12,outlierThreshold=.48,groupBy='row'}={}){
  ...
  return F({schema:'kelo-silhouette-consistency-v1',sampleSize,outlierThreshold,groups:F(groupReports),frames:F(results),outliers:F(outliers),outlierCount:outliers.length,pass:outliers.length===0});
}
