 ...
 async function compile(file,config={},options={}){
   ...
   const stages=[
    ...
    {name:'release-gates',run:async()=>{
      ...
      const releaseDecision=verifyAssetRelease({input,compiled,budget:options.budget||null,regression:options.regression||null,temporal:options.temporal||null,defects:options.defects||null,aiSuggestions:options.aiSuggestions||null,humanApprovals:options.humanApprovals||[],requiredDomains:options.requiredEvidenceDomains||['security','runtime']});
