...
const HARD_PREFIXES=F(['AI_AUTHORITY_VIOLATION','MISSING_DETERMINISTIC_EVIDENCE','UNSAFE_','RUNTIME_','ASSET_CONTAINER','ASSET_FILE','ASSET_DIMENSION','ASSET_PIXEL','ASSET_DECODE','RUNTIME_BUDGET','VISUAL_REGRESSION','BLOCKING_ASSET_DEFECT']);
...
export function createAssetReviewPacket({assetId='unknown',releaseDecision,evidence=[],aiAdvice=null,sourceSummary=null}={}){
 if(!releaseDecision)throw new Error('ASSET_REVIEW_RELEASE_DECISION_REQUIRED');const rejected=[...(releaseDecision.rejected||[])],review=[...(releaseDecision.reviewRequired||[])],hard=F(rejected.filter(hardReason)),reviewable=F(review.filter(reason=>!hardReason(reason)));
 const status=hard.length?'QUARANTINED':reviewable.length?'HUMAN_REVIEW':'NO_REVIEW_NEEDED';
 return F({schema:'kelo-asset-review-packet-v1',assetId:String(assetId),status,hardFailures:hard,reviewableReasons:reviewable,evidence:F([...(evidence||[])]),aiAdvice:aiAdvice||null,sourceSummary:sourceSummary||null,rule:'Hard safety, integrity and resource failures require a new/corrected asset; they cannot be overridden.'});
}
