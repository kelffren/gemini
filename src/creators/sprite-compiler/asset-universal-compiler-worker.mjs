 ...
 * purpose: hard-isolated worker entry for the existing universal avatar compiler
...
scope.onmessage=async event=>{
 ...
 try{
   ...
   const root=createWorkerCanvasRoot(scope),onProgress=progress=>scope.postMessage({id,type:'progress',progress});
   const compiled=await compileUniversalAvatarRuntime(payload.file,payload.config||null,{root,onProgress,oracle:payload.options?.oracle||null});
