...
function thinBoundaryRepair(root,source,rect,frame){
  ...
  const xThin=Math.max(2,Math.round(rect.h*.032)),yThin=Math.max(2,Math.round(rect.w*.032)),maxX=Math.max(2,Math.floor(rect.w*.38)),maxY=Math.max(2,Math.floor(rect.h*.28));let removedPixels=0,changed=false;
  const trimColumns=(fromRight=false)=>{const edge=fromRight?rect.w-1:0,edgeCount=countColumn(edge);if(edgeCount<1||edgeCount>xThin)return;let encounteredBody=false;for(let depth=0;depth<maxX;depth++){const x=fromRight?rect.w-1-depth:depth,n=countColumn(x);if(n>xThin){encounteredBody=true;break}if(n===0&&depth>2)break;if(n){removedPixels+=clearColumn(x);changed=true}}if(!encounteredBody){changed=false;removedPixels=0;ctx.putImageData(image,0,0)}};
  const snapshot=new Uint8ClampedArray(data);trimColumns(false);if(!changed){image.data.set(snapshot);removedPixels=0}const afterLeft=new Uint8ClampedArray(data);const leftRemoved=removedPixels;let rightChanged=false;const priorChanged=changed;changed=false;removedPixels=0;trimColumns(true);rightChanged=changed;const rightRemoved=removedPixels;if(!rightChanged)image.data.set(afterLeft);changed=priorChanged||rightChanged;removedPixels=leftRemoved+rightRemoved;
  const beforeRows=new Uint8ClampedArray(data);let rowRemoved=0,rowChanged=false;
  const trimRows=(fromBottom=false)=>{const edge=fromBottom?rect.h-1:0,edgeCount=countRow(edge);if(edgeCount<1||edgeCount>yThin)return false;let localRemoved=0,localChanged=false,encounteredBody=false;for(let depth=0;depth<maxY;depth++){const y=fromBottom?rect.h-1-depth:depth,n=countRow(y);if(n>yThin){encounteredBody=true;break}if(n===0&&depth>2)break;if(n){localRemoved+=clearRow(y);localChanged=true}}if(!encounteredBody)return false;rowRemoved+=localRemoved;return localChanged};
  if(trimRows(false))rowChanged=true;const afterTop=new Uint8ClampedArray(data);if(trimRows(true))rowChanged=true;else image.data.set(afterTop);changed=changed||rowChanged;removedPixels+=rowRemoved;
  if(!changed){image.data.set(beforeRows);return null}const maxRemoval=Math.max(16,Math.round(originalPixels*.04));if(removedPixels<=0||removedPixels>maxRemoval)return null;const bounds=localAlphaBounds(data,rect.w,rect.h);if(!bounds)return null;ctx.putImageData(image,0,0);return F({canvas:temp,bounds:F(bounds),removedPixels,originalPixels,removedRatio:removedPixels/originalPixels})
}
...
function composeFrame(root,sourceCanvas,plan,rig,options){const{frameWidth,frameHeight,sourceWidth,sourceHeight}=options,cell=makeCanvas(root,frameWidth,frameHeight),context=cell.getContext('2d',{willReadFrequently:true});context.clearRect(0,0,frameWidth,frameHeight);context.imageSmoothingEnabled=!!options.imageSmoothing;
  ...
  if(plan.patch.layerVisibility?.character!==false){const repaired=!patchHasExplicitSurgery(plan.patch)?thinBoundaryRepair(root,sourceCanvas,plan.source,plan.frame):null;if(repaired){const b=repaired.bounds,rx=localX+(b.x/plan.source.w)*destination.w,ry=localY+(b.y/plan.source.h)*destination.h,rw=(b.w/plan.source.w)*destination.w,rh=(b.h/plan.source.h)*destination.h;context.drawImage(repaired.canvas,b.x,b.y,b.w,b.h,rx,ry,rw,rh);autoBridgeRepair=F({removedPixels:repaired.removedPixels,removedRatio:repaired.removedRatio,bounds:b})}else{context.save();context.translate(cx,cy);context.rotate(plan.patch.rotation||0);context.drawImage(sourceCanvas,plan.source.x,plan.source.y,plan.source.w,plan.source.h,-destination.w/2,-destination.h/2,destination.w,destination.h);context.restore()}}
