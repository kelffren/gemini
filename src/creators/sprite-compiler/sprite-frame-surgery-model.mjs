 ...
 * keys: SPRITE SURGERY TOUCH PATCH HISTORY MASK OVERLAY CLONE FILL CROP GHOST
 ...
 *             normalizeSurgeryPatch, createStroke, createOverlayPatch, serializeSurgeryPatches, surgeryFeatureContract
 * online: N/A
...
export const surgeryFeatureContract = F({
  version: 'kelo-frame-surgery-v1.0.0',
  required: F([
...
function normalizeStroke(stroke = {}) {
  return F({
    ...
    hardness: clamp(finite(stroke.hardness, .9), .05, 1),
...
export function createSurgeryPatch(seed = {}) {
  return normalizeSurgeryPatch({
    scale: 1, x: 0, y: 0, rotation: 0, copyFrom: null,
...
export function normalizeSurgeryPatch(patch = {}) {
  ...
  const selection = patch.selection ? F({
    x: clamp(patch.selection.x, 0, 1), y: clamp(patch.selection.y, 0, 1),
    w: clamp(finite(patch.selection.w, 0), 0, 1), h: clamp(finite(patch.selection.h, 0), 0, 1),
    seedX: clamp(finite(patch.selection.seedX, patch.selection.x), 0, 1),
    seedY: clamp(finite(patch.selection.seedY, patch.selection.y), 0, 1)
  }) : null;
  return freezeDeep({
    ...
    overlays: F((patch.overlays || []).map(normalizeOverlay)),
    selection,
    layerVisibility: F(layerVisibility)
  });
}

export function createStroke(points, {radius = .045, hardness = .9, source = null, id = null} = {}) {
  return normalizeStroke({points, radius, hardness, source, id});
}
...
export function serializeSurgeryPatches(patches = {}) {
  ...
  for (const [index, raw] of Object.entries(patches || {})) {
    ...
    plain.overlays = (plain.overlays || []).map(overlay => {
      const item = {...overlay};
      delete item.image; delete item.canvas; delete item.bitmap;
      return item;
    });
    out[index] = plain;
  }
...
export function reduceSurgeryGesture(patchInput, gesture = {}) {
  ...
  if (type === 'drag') {
    patch.x += finite(gesture.dx);
    patch.y += finite(gesture.dy);
  } else if (type === 'pinch') {
    const start = pointerGeometry(gesture.start || []), current = pointerGeometry(gesture.current || []);
    if (start && current && start.distance > .001) {
      patch.scale *= current.distance / start.distance;
      patch.rotation += current.angle - start.angle;
      patch.x += current.center.x - start.center.x;
      patch.y += current.center.y - start.center.y;
    }
  } else if (type === 'crop') {
    patch.crop = {...patch.crop, [gesture.edge]: finite(gesture.value)};
  } else if (type === 'stroke') {
    ...
    patch.selection = gesture.selection || null;
  } else if (type === 'overlay-add') {
    patch.overlays = [...(patch.overlays || []), createOverlayPatch(gesture.overlay || {})];
  } else if (type === 'overlay-transform') {
    patch.overlays = (patch.overlays || []).map(item => item.id === gesture.id ? createOverlayPatch({...item, ...gesture.transform}) : item);
  } else if (type === 'layer-visibility') {
    patch.layerVisibility = {...patch.layerVisibility, [gesture.layer]: gesture.visible !== false};
  } else if (type === 'reset-transform') {
    patch.x = 0; patch.y = 0; patch.scale = 1; patch.rotation = 0;
  }
...
export function autoFixSurgeryPatch(patchInput, frame = {}, reference = {}) {
  const patch = clone(normalizeSurgeryPatch(patchInput));
  const height = Math.max(1, finite(frame.height ?? frame.h, 1));
  const width = Math.max(1, finite(frame.width ?? frame.w, 1));
  const refHeight = Math.max(1, finite(reference.height ?? reference.h, height));
  const refWidth = Math.max(1, finite(reference.width ?? reference.w, width));
  const frameFoot = finite(frame.footY ?? frame.bottom, height);
  const refFoot = finite(reference.footY ?? reference.bottom, refHeight);
  const frameCenter = finite(frame.centerX, width / 2);
  const refCenter = finite(reference.centerX, refWidth / 2);
  const sizeScale = clamp(Math.sqrt((refHeight * refWidth) / Math.max(1, height * width)), .6, 1.5);
  patch.scale = sizeScale;
  patch.x += refCenter - frameCenter * sizeScale;
  patch.y += refFoot - frameFoot * sizeScale;
  patch.rotation = 0;
  return normalizeSurgeryPatch(patch);
}

export function createSurgeryHistory(initialPatch = createSurgeryPatch(), {limit = 80} = {}) {
  const undoStack = [], redoStack = [];
  let current = normalizeSurgeryPatch(initialPatch);
  ...
  return F({
    ...
    commit(next) {
      ...
      if (undoStack.length > Math.max(5, limit)) undoStack.shift();
      current = normalizeSurgeryPatch(clone(next));
      redoStack.length = 0;
      return current;
    },
...
export const __frameSurgeryInternals = F({clamp, clone, pointerGeometry, normalizeCrop, normalizeStroke, normalizeOverlay});
