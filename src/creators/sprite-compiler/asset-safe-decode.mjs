...
function inspectPng(bytes,{requireComplete=false,maxChunks=4096}={}){
 ...
 let offset=8,animated=false,structureComplete=false,chunks=0,sawIdat=false,endOffset=null;
 while(offset+12<=bytes.length){
   if(++chunks>maxChunks)return F({format:'png',valid:false,reason:'PNG_CHUNK_LIMIT'});
   const length=u32be(bytes,offset),type=ascii(bytes,offset+4,4),next=offset+12+length;
   if(!/^[A-Za-z]{4}$/.test(type))return F({format:'png',valid:false,reason:'PNG_CHUNK_TYPE_INVALID'});
   if(next<=offset||next>bytes.length){if(requireComplete)return F({format:'png',valid:false,reason:'PNG_CHUNK_TRUNCATED'});break;}
   if(type==='IHDR'&&offset!==8)return F({format:'png',valid:false,reason:'PNG_DUPLICATE_IHDR'});
   if(type==='acTL')animated=true;if(type==='IDAT')sawIdat=true;
   if(type==='IEND'){if(length!==0)return F({format:'png',valid:false,reason:'PNG_IEND_INVALID'});structureComplete=true;endOffset=next;break;}
   offset=next;
 }
 if(requireComplete&&!structureComplete)return F({format:'png',valid:false,reason:'PNG_IEND_MISSING'});
 if(requireComplete&&!sawIdat)return F({format:'png',valid:false,reason:'PNG_IDAT_MISSING'});
 if(requireComplete&&endOffset!==bytes.length)return F({format:'png',valid:false,reason:'PNG_TRAILING_BYTES'});
 return F({format:'png',mimeType:'image/png',valid:true,width,height,animated,structureComplete,chunks,sawIdat,bitDepth:bytes[24],colorType:bytes[25]});
}
...
function inspectJpeg(bytes){
 ...
 while(i+3<bytes.length){
   if(++segments>8192)return F({format:'jpeg',valid:false,reason:'JPEG_SEGMENT_LIMIT'});
   ...
   if(marker===0xda)return F({format:'jpeg',mimeType:'image/jpeg',valid:true,width:null,height:null,animated:false,reason:'JPEG_SOF_MISSING_BEFORE_SOS'});
   if(i+1>=bytes.length)break;const length=u16be(bytes,i);if(length<2)return F({format:'jpeg',valid:false,reason:'JPEG_SEGMENT_INVALID'});if(i+length>bytes.length)break;
   if(sof.has(marker)){if(length<8)return F({format:'jpeg',valid:false,reason:'JPEG_SOF_INVALID'});const height=u16be(bytes,i+3),width=u16be(bytes,i+5);if(!width||!height)return F({format:'jpeg',valid:false,reason:'JPEG_DIMENSIONS_INVALID'});return F({format:'jpeg',mimeType:'image/jpeg',valid:true,width,height,animated:false,progressive:marker===0xc2||marker===0xca,segments});}
   i+=length;
 }
...
export function inspectAssetContainer(input,options={}){
 const bytes=U8(input);if(bytes.length<12)return F({format:'unknown',valid:false,reason:'HEADER_TOO_SHORT'});
 if(bytes[0]===0x89&&ascii(bytes,1,3)==='PNG'&&bytes[4]===0x0d&&bytes[5]===0x0a&&bytes[6]===0x1a&&bytes[7]===0x0a)return inspectPng(bytes,options);
...
export async function assertSafeAssetInput(file,{maxBytes=12*1024*1024,maxDimension=4096,maxPixels=16_777_216,maxDecodedBytes=96*1024*1024,maxHeaderScanBytes=1024*1024,allowAnimation=false,maxPngChunks=4096}={}){
 ...
 const scanBytes=Math.min(size,Math.max(65536,Math.min(maxHeaderScanBytes,2*1024*1024))),head=new Uint8Array(await file.slice(0,scanBytes).arrayBuffer());let container=inspectAssetContainer(head,{maxChunks:maxPngChunks});
 if(container.format==='png'&&(!container.structureComplete||scanBytes<size)){const full=new Uint8Array(await file.arrayBuffer());container=inspectAssetContainer(full,{requireComplete:true,maxChunks:maxPngChunks});}
 else if(container.format==='jpeg'&&(!container.width||!container.height)&&scanBytes<size){const full=new Uint8Array(await file.arrayBuffer());container=inspectAssetContainer(full);}
 ...
 return F({...container,size,name:String(file.name||''),pixels,decodedBytes,headerScanBytes:scanBytes,limits:F({maxBytes,maxDimension,maxPixels,maxDecodedBytes,maxHeaderScanBytes,allowAnimation,maxPngChunks})});
}
