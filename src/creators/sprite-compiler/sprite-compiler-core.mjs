 ...
 * owns: background cleanup, cell bounds, common scale, feet anchoring, repair QA
...
export function buildRepairReport({sourceFrames=[],outputFrames=[],plan=null,backgroundCleanup=null}={}){
  ...
  const pass=frameCount>0&&sourceNonEmpty===frameCount&&outputNonEmpty===frameCount&&clippedAfter===0&&feetSpread<=1;
  return Object.freeze({pass,frameCount,sourceNonEmpty,outputNonEmpty,clippedBefore,clippedAfter,feetSpread,commonScale:plan?.scale??1,minOccupancy,maxOccupancy,backgroundMode:backgroundCleanup?.mode||'unknown',backgroundPixelsRemoved:backgroundCleanup?.removed||0});
}
export function repairSpritesheetImage(root,image,{columns=4,rows=8,targetWidth=64,targetHeight=64,padding=4,removeBackground=true,colorThreshold=34,softEdge=14,imageSmoothing=false}={}){
  ...
  const cleanup=removeBackground?cleanBackgroundPixels(raw.data,width,height,{background:'auto',colorThreshold,softEdge}):Object.freeze({data:new Uint8ClampedArray(raw.data),background:null,removed:0,mode:'preserve'});const cleaned=sctx.createImageData(width,height);cleaned.data.set(cleanup.data);sctx.putImageData(cleaned,0,0);
  const sourceFrames=analyzeGridCells(cleanup.data,width,height,{columns,rows}),plan=planFrameNormalization(sourceFrames,{targetWidth,targetHeight,padding}),output=makeCanvas(columns*targetWidth,rows*targetHeight),ctx=output.getContext('2d',{willReadFrequently:true});ctx.clearRect(0,0,output.width,output.height);ctx.imageSmoothingEnabled=!!imageSmoothing;
  ...
  const outPixels=ctx.getImageData(0,0,output.width,output.height),outputFrames=analyzeGridCells(outPixels.data,output.width,output.height,{columns,rows}),report=buildRepairReport({sourceFrames,outputFrames,plan,backgroundCleanup:cleanup});
  return Object.freeze({canvas:output,report,plan,sourceFrames,outputFrames,columns,rows,frameWidth:targetWidth,frameHeight:targetHeight,sourceWidth:width,sourceHeight:height});
}
