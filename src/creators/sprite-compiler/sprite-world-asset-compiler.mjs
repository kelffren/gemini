 ...
 * keys: WORLD ASSET COMPILE PIVOT FOOTPRINT COLLISION PORTAL VARIANT SCALE ALPHA METADATA
 ...
 * consumes: sprite-world-asset-profile.mjs
...
export function compileWorldAssetPixels(sourceData,width,height,options={}){
 const base=buildWorldAssetProfile(sourceData,width,height,options),collision=buildCollisionProfile(base.footprint,base.portal),profile=F({...base,collision});
...
export function compileWorldAssetImage(root,image,options={}){
 ...
 const canvas=root.document.createElement('canvas');canvas.width=width;canvas.height=height;const ctx=canvas.getContext('2d',{willReadFrequently:true});ctx.clearRect(0,0,width,height);ctx.drawImage(image,0,0,width,height);const raw=ctx.getImageData(0,0,width,height),compiled=compileWorldAssetPixels(raw.data,width,height,options),cleaned=ctx.createImageData(width,height);cleaned.data.set(compiled.cleanedPixels);ctx.clearRect(0,0,width,height);ctx.putImageData(cleaned,0,0);
