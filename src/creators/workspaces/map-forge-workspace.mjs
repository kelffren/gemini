 ...
 * reuse: handoff returns through the registered World workspace via openWorkspace()
 */
export function createMapForgeWorkspaceManifest({loader=()=>import('../ui/map-forge-workspace.mjs')}={}){return Object.freeze({id:'map-forge',label:'Map Forge',category:'build',projectTypes:['WORLD'],capability:'world.edit',availability:'active',async open({root=globalThis,openWorkspace=null}={}){const mod=await loader();if(typeof mod.openMapForgeWorkspace!=='function')throw new Error('CREATOR_MAP_FORGE_ENTRY_MISSING');return mod.openMapForgeWorkspace({root,onOpenWorld:typeof openWorkspace==='function'?map=>openWorkspace('world',{mapDefinition:map,source:'map-forge'}):null});}});}
