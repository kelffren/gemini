...
export function createAbilityWorkspaceManifest({loader=()=>import('../ability/ability-live-controller.mjs')}={}){
  return Object.freeze({id:'ability',label:'Ability',category:'gameplay',projectTypes:['ABILITY'],capability:'ability.edit',availability:'active',async open(context={}){const mod=await loader();if(typeof mod.openAbilityCreator!=='function')throw new Error('CREATOR_ABILITY_ENTRY_MISSING');return mod.openAbilityCreator(context);}});
}
