...
export function createDefinitionWorkspaceManifest({id,type,loader=()=>import('../definition/generic-definition-studio.mjs')}={}){
  ...
  return Object.freeze({id:row.id,label:spec.label,category:spec.category,projectTypes:[row.type],availability:'active',async open(context={}){installCreatorTestBridge(context.root||globalThis);const mod=await loader();if(typeof mod.openGenericDefinitionCreator!=='function')throw new Error('CREATOR_GENERIC_DEFINITION_ENTRY_MISSING');return mod.openGenericDefinitionCreator({...context,definitionType:row.type});}});
}
