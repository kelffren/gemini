...
export async function getProviderStatuses(){const cfg=await loadProviderConfig();return(cfg.providers||[]).filter(p=>p.enabled!==false).map(p=>({id:p.id,name:p.name,mode:p.mode,license:p.license,sourceUrl:p.sourceUrl,browseUrl:p.browseUrl||p.sourceUrl,notes:p.notes,live:isLive(p),lazy:isLazy(p),verified:p.verified===true,apiAttributionRequired:p.apiAttributionRequired===true,catalogOnly:p.catalogOnly===true,requiresQuery:p.requiresQuery===true,supportsRemotePreview:p.supportsRemotePreview!==false,supportsCORS:p.supportsCORS!==false,supportsThumbnail:p.supportsThumbnail!==false,supportsOriginalDownload:p.supportsOriginalDownload!==false&&p.catalogOnly!==true,attributionRequirements:p.attributionRequired===true?'required':'not-required'}));}
...
if(typeof window!=='undefined'){window.KELO_EXTERNAL_ASSET_PROVIDERS=EXTERNAL_ASSET_PROVIDERS;window.KELO_EXTERNAL_CONTENT_PROVIDERS=EXTERNAL_CONTENT_PROVIDERS;installQAPreviewHook();installActivePageThumbnailHydrator();installCatalogOnlyActions();installUniversalPreviewInspector();installUniversalSpritePreview();installUniversalAvatarTryOn();installUniversalLookBuilder();installAssetIntelligenceLiveUI({getAssetById:getRecentExternalAsset});}
