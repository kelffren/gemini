 ...
 */
const CONFIG_URL='../../../data/external-asset-providers.json?v=1';
...
export async function getProviderStatuses(){
  ...
  return (cfg.providers||[]).filter(p=>p.enabled!==false).map(p=>({id:p.id,name:p.name,mode:p.mode,license:p.license,sourceUrl:p.sourceUrl,browseUrl:p.browseUrl||p.sourceUrl,notes:p.notes,live:p.mode==='live-index'}));
}

export async function browseProvider(id){
...
export async function searchExternalAssets(query='',options={}){
  ...
  const q=clean(query).toLowerCase(); const bundles=await Promise.all(providers.filter(p=>!wanted||wanted.has(p.id)).map(p=>browseProvider(p.id)));
  let assets=bundles.flatMap(b=>b.assets||[]);
  if(q)assets=assets.filter(a=>`${a.name} ${a.category} ${(a.tags||[]).join(' ')} ${a.description||''}`.toLowerCase().includes(q));
  if(options.category&&options.category!=='all')assets=assets.filter(a=>a.category===options.category);
  return {assets,providers:bundles.map(b=>({id:b.provider.id,name:b.provider.name,mode:b.provider.mode,error:b.error,count:b.assets?.length||0,browseUrl:b.provider.browseUrl||b.provider.sourceUrl,license:b.provider.license}))};
}

export function clearProviderCache(){liveCache=new Map();configPromise=null;}
