 ...
 */
import { compileCanvasAsset, buildKeloAssetManifest } from './asset-sheet-compiler.mjs';
...
function emit(type, detail){ try { window.dispatchEvent(new CustomEvent(type,{detail})); } catch (_) {} }
...
async function storePut(storeName,value){
  ...
  return new Promise((resolve,reject)=>{
    ...
    tx.oncomplete=()=>resolve(value); tx.onerror=()=>reject(tx.error); tx.onabort=()=>reject(tx.error||new Error('VAULT_WRITE_ABORTED'));
  });
}
...
export function normalizeAssetMeta(input={}){
  ...
  return {
    id:String(input.id), provider:String(input.provider||'unknown'), externalId:String(input.externalId||input.id),
    name:String(input.name||input.id), category:String(input.category||'other'), tags:Array.isArray(input.tags)?input.tags.slice(0,40):[],
    previewUrl:input.previewUrl||null, downloadUrl:input.downloadUrl||null, sourceUrl:input.sourceUrl||null,
    license:String(input.license||'UNKNOWN'), author:input.author||null, attributionRequired:!!input.attributionRequired,
    ownership:input.ownership||'discovered', downloaded:!!input.downloaded, integrated:!!input.integrated,
    bytes:Number(input.bytes||0), mime:input.mime||null, downloadedAt:input.downloadedAt||null,
    integratedAt:input.integratedAt||null, updatedAt:input.updatedAt||now(), compiler:input.compiler||null
  };
}

export async function rememberAsset(input){
  const previous = await storeGet(META_STORE,String(input.id));
  const next = normalizeAssetMeta({...previous,...input,updatedAt:now()});
  await storePut(META_STORE,next); return clone(next);
}
...
export function licenseDecision(asset){
  const license=String(asset?.license||'UNKNOWN').toUpperCase();
  if (AUTO_LICENSES.has(license)) return {allowed:true,review:false,reason:'compatible'};
  return {allowed:false,review:true,reason:'license-review-required'};
}
...
export async function downloadAsset(input){
  ...
  const decision=licenseDecision(asset);
  if (!decision.allowed) throw new Error('ASSET_LICENSE_REVIEW_REQUIRED:'+asset.license);
  const response=await fetch(asset.downloadUrl,{mode:'cors',cache:'force-cache'});
...
export async function getObjectURL(id){
  id=String(id); if (objectUrls.has(id)) return objectUrls.get(id);
  const blob=await getBlob(id); if (!blob) return null;
  const url=URL.createObjectURL(blob); objectUrls.set(id,url); return url;
}
export function releaseObjectURL(id){
  id=String(id); const url=objectUrls.get(id); if (!url) return;
  URL.revokeObjectURL(url); objectUrls.delete(id);
}

async function blobToCanvas(blob){
  let bitmap=null, width=0, height=0, cleanup=()=>{};
  if ('createImageBitmap' in globalThis) {
    bitmap=await createImageBitmap(blob); width=bitmap.width; height=bitmap.height; cleanup=()=>bitmap.close?.();
  } else {
    const url=URL.createObjectURL(blob); const img=new Image();
    img.decoding='async'; await new Promise((resolve,reject)=>{img.onload=resolve;img.onerror=()=>reject(new Error('ASSET_IMAGE_DECODE_FAILED'));img.src=url;});
    bitmap=img; width=img.naturalWidth; height=img.naturalHeight; cleanup=()=>URL.revokeObjectURL(url);
  }
  if (!width||!height) { cleanup(); throw new Error('ASSET_ZERO_DIMENSIONS'); }
  const canvas=document.createElement('canvas'); canvas.width=width; canvas.height=height;
  const ctx=canvas.getContext('2d',{willReadFrequently:true}); if(!ctx){cleanup();throw new Error('ASSET_CANVAS_UNAVAILABLE');}
  ctx.clearRect(0,0,width,height); ctx.drawImage(bitmap,0,0); cleanup(); return canvas;
}

export async function integrateAsset(id){
  const asset=await getAsset(id); if(!asset) throw new Error('ASSET_NOT_IN_VAULT');
  if(!asset.downloaded) throw new Error('ASSET_NOT_DOWNLOADED');
  ...
  const analysis=compileCanvasAsset(canvas,{name:asset.name,source:asset.sourceUrl||asset.downloadUrl,provider:asset.provider,license:asset.license});
  if(!analysis?.ok) throw new Error('ASSET_COMPILER_REJECTED');
  const manifest=buildKeloAssetManifest(analysis,{name:asset.name,source:asset.sourceUrl||asset.downloadUrl,provider:asset.provider,license:asset.license,externalAssetId:asset.externalId});
  ...
  emit('kelo:personal-asset-integrated',{asset:next,manifest});
  return {asset:next,manifest};
}

export async function removeLocal(id){
  id=String(id); releaseObjectURL(id); await Promise.all([storeDelete(BLOB_STORE,id),storeDelete(MANIFEST_STORE,id)]);
  const asset=await getAsset(id); if(!asset) return null;
  const next=await rememberAsset({...asset,downloaded:false,integrated:false,bytes:0,mime:null,downloadedAt:null,integratedAt:null});
  emit('kelo:personal-asset-local-removed',{asset:next}); return next;
}

export async function getVaultStats(){
  const rows=await listAssets();
  return {items:rows.length,owned:rows.filter(a=>['free','owned','purchased'].includes(a.ownership)).length,downloaded:rows.filter(a=>a.downloaded).length,integrated:rows.filter(a=>a.integrated).length,bytes:rows.reduce((n,a)=>n+(a.downloaded?Number(a.bytes||0):0),0)};
}
