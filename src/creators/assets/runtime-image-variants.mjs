 ...
 * keys: RUNTIME IMAGE VARIANTS WEBP AVIF LOSSLESS ADAPTIVE QUALITY GATE SAFARI
 * purpose: derive smaller runtime delivery variants while preserving the canonical PNG and proving decoded fidelity
 ...
 * consumes: Sharp, PNG optimizer, image profiler, quality agent
...
function webpPreset(profile) {
  ...
  if (profile.kind === 'fx') return 'picture';
  return 'picture';
}
...
export async function buildRuntimeImageVariants(sourceBuffer, options = {}) {
  ...
  const profile = options.assetProfile || profileAssetImage(
    originalVisual.rgba,
  ...
  candidates.push({
    ...
    score:1,
    reasons:[],
    metrics:{exactPixels:true},
    options:{optimizer:'kelo-lossless'}
  });

  try {
    const webpLossless = await sharp(sourceBuffer, {animated:false})
      .keepMetadata()
    ...
    candidates.push(await evaluateCandidate(
      ...
      originalVisual,
      webpLossless,
      {label:'webp:lossless', format:'webp', track:'lossless', options:{lossless:true, quality:100, effort:6, exact:true}},
      'strict',
      {}
    ));
  } catch (error) {
  ...
  try {
    const avifLossless = await sharp(sourceBuffer, {animated:false})
      .keepMetadata()
      .avif({lossless:true, effort:9, chromaSubsampling:'4:4:4', bitdepth:8})
      .toBuffer();
    candidates.push(await evaluateCandidate(
      sharp,
      originalVisual,
      avifLossless,
      {label:'avif:lossless', format:'avif', track:'lossless', options:{lossless:true, effort:9, chromaSubsampling:'4:4:4', bitdepth:8}},
      'strict',
      {}
    ));
  } catch (error) {
    ...
    for (const quality of (options.webpQualities || [100, 98, 96, 94, 92, 90, 88])) {
      try {
        ...
        candidates.push(await evaluateCandidate(
          ...
          originalVisual,
          buffer,
          {label:`webp:q${quality}`, format:'webp', track:'adaptive', options:{quality, alphaQuality:100, effort:6, exact:true}},
          policy,
          limits
        ));
      } catch (error) {
    ...
    for (const quality of (options.avifQualities || [100, 96, 92, 88, 84, 80])) {
      try {
        ...
        candidates.push(await evaluateCandidate(
          ...
          originalVisual,
          buffer,
          {label:`avif:q${quality}`, format:'avif', track:'adaptive', options:{quality, effort:8, chromaSubsampling:'4:4:4', bitdepth:8, tune:'iq'}},
          policy,
          limits
        ));
      } catch (error) {
  ...
  const runtimeWinner = chooseSmallest([
    ...candidates.filter(candidate => candidate.track === 'lossless'),
    ...candidates.filter(candidate => candidate.track === 'adaptive')
  ]);
