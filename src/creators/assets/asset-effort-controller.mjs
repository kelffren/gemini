 ...
 * purpose: stop expensive PNG search when cheap evidence does not justify more CPU while preserving exact structural opportunities
...
export function optimizePngWithAdaptiveEffort(buffer,options={}){
  ...
  const fastOptions={...(options.losslessOptions||{}),filterStrategies:['adaptive'],disablePalette:uniqueColors>64};
  ...
  if(balancedJustified){
    const balanced=run(buffer,'balanced',{...(options.losslessOptions||{}),filterStrategies:['adaptive',0,4]});
    ...
    if(deepWorthTesting){
      const deep=run(buffer,'deep',{...(options.losslessOptions||{})});
      stages.push(deep);
  ...
  return{buffer:best.result.buffer,report:{...best.result.report,effortController:{version:'kelo-asset-effort-controller-v2',selected:best.label,decision,opportunity:{balancedAllowed,balancedJustified,reasons,uniqueColors,sourceBytes},marginal:{balancedVsFast:balancedMarginal},thresholds:{minExtraBytes,minBytesPerExtraSecond,maxBalancedSourceBytes,maxCheapProbeSourceBytes,maxExactPaletteColors},stages:stages.map(stage=>stagePublic(stage,sourceBytes))}}};
}
