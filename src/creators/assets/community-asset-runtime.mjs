...
import { installCommunityAssetAvatarRenderer } from './community-asset-avatar-renderer.mjs';

export const communityAssetStream = new DynamicAssetStreamManager();
export const communityAssetRenderer = installCommunityAssetAvatarRenderer({ root: globalThis });
...
if (typeof globalThis.addEventListener === 'function' && !globalThis.__KELO_COMMUNITY_ASSET_RUNTIME__) {
  globalThis.__KELO_COMMUNITY_ASSET_RUNTIME__ = Object.freeze({ version: 3, stream: communityAssetStream, renderer: communityAssetRenderer });
  ...
  emit('kelo:community-asset-runtime-ready', { version: 3, fallback: 'base-avatar', renderer: communityAssetRenderer?.version || null });
}
