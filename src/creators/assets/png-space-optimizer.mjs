 ...
 * keys: PNG COMPRESS LOSSLESS PALETTE PALETTE ORDER BIT DEPTH COLOR TYPE ALPHA DROP TRNS GRAYSCALE QUALITY GATE CONFORMANCE
...
import {buildPaletteOrderings,PALETTE_ORDER_FAST} from './png-palette-order.mjs';
...
function paletteRepresentation(decoded){
  ...
  const bitDepth=colors.length<=2?1:colors.length<=4?2:colors.length<=16?4:8;return{...partitions,indexes,colors,colorCount:colors.length,bitDepth};
}
function paletteOrderRepresentation(base,ordering){const colors=ordering.colors,indexes=ordering.indexes,plte=Buffer.alloc(colors.length*3);let lastTransparent=-1;colors.forEach((c,i)=>{plte[i*3]=c[0];plte[i*3+1]=c[1];plte[i*3+2]=c[2];if(c[3]!==255)lastTransparent=i;});const trns=lastTransparent>=0?Buffer.from(colors.slice(0,lastTransparent+1).map(c=>c[3])):null;return{...base,colors,indexes,plte,trns,orderName:ordering.name};}
...
export function optimizePngLossless(buffer,options={}){
  ...
  const palette=options.disablePalette===true?null:paletteRepresentation(decoded);let paletteOrderNames=[];if(palette){const paletteFilters=options.paletteFilterStrategies||filterStrategies,orderings=buildPaletteOrderings({colors:palette.colors,indexes:palette.indexes,width:decoded.ihdr.width,height:decoded.ihdr.height,strategies:options.paletteOrderStrategies||PALETTE_ORDER_FAST});paletteOrderNames=orderings.map(ordering=>ordering.name);for(const ordering of orderings){const variant=paletteOrderRepresentation(palette,ordering),orderTag=ordering.name==='first-seen'?'':`:order=${ordering.name}`;for(const filterStrategy of paletteFilters)for(const profile of profiles)candidates.push(candidateRecord('exact-palette',`palette:${palette.colorCount}:${palette.bitDepth}b${orderTag}:${filterStrategy}:${profile.name}`,palettePng(decoded,variant,filterStrategy,profile.options)));}}
