 ...
 * keys: QUATERNIUS CC0 3D MODEL CURATED LAZY CATALOG FEDERATED PREVIEW MOBILE SAFE
 * purpose: Search a verified Quaternius CC0 pack index without loading model binaries.
 *          Quaternius pack/folder links remain source navigation only until Kelo has a
 *          verified direct-file resolver with immutable digest+size metadata.
 */
...
function toAsset(row){const preview=row.previewPath?PREVIEW_BASE+row.previewPath:null;const sourceUrl=row.sourceUrl||row.driveUrl||'https://quaternius.com/';return{...row,provider:'quaternius',externalId:clean(row.id).replace(/^quaternius:/,''),license:'CC0-1.0',author:'Quaternius',attributionRequired:false,ownership:'discovered',previewKind:'image',previewUrl:preview,downloadUrl:null,externalDownloadUrl:row.driveUrl||null,sourceUrl,downloadable:false,integrationReady:false,catalogOnly:true,verified:true,heavyExternal:true,requiresVerifiedBinaryResolver:true};}
export async function searchQuaterniusAssets(query='',options={}){const offset=Math.max(0,Number(options.offset)||0),limit=Math.max(1,Math.min(Number(options.limit)||80,320)),q=clean(query).toLowerCase();let rows=(await load()).filter(row=>matches(row,q));if(options.contentKind&&options.contentKind!=='all')rows=rows.filter(row=>row.contentKind===options.contentKind);const assets=rows.slice(offset,offset+limit).map(toAsset);return{assets,offset,limit,total:rows.length,hasMore:offset+assets.length<rows.length,engine:'curated-cc0-pack-index-v2',catalogOnly:true};}
