(function(){
'use strict';
const VERSION='equipment-v1.0.1';
...
function equippedItems(skipEnsure){if(!skipEnsure)ensure();const out=[];SLOTS.forEach(function(slot){const id=STATE.equipmentSlots[slot],item=find(id,true);if(item&&item.slot===slot)out.push(item);});return out;}
...
function equipItem(itemId){ensure();const item=find(itemId,true);if(!item)return {ok:false,error:'ITEM_NOT_FOUND'};if(!SLOTS.includes(item.slot))return {ok:false,error:'INVALID_SLOT'};Object.keys(STATE.equipmentSlots).forEach(function(slot){if(STATE.equipmentSlots[slot]===itemId)delete STATE.equipmentSlots[slot];});STATE.equipmentSlots[item.slot]=itemId;recalculate(localPlayer,true);if(typeof saveState==='function')saveState();return {ok:true,item,armorScore:localPlayer.armorScore,auraRank:localPlayer.auraRank};}
function unequipItem(slot){ensure();if(!SLOTS.includes(slot))return {ok:false,error:'INVALID_SLOT'};const id=STATE.equipmentSlots[slot]||null;delete STATE.equipmentSlots[slot];recalculate(localPlayer,true);if(typeof saveState==='function')saveState();return {ok:true,itemId:id};}
