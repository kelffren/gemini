 ...
 * owner: KeloEquipment
 * purpose: reglas de equipamiento, stats, calidad, grado y aura; items portátiles viven en KeloInventory
 * public-api: KeloEquipment
 * consumes: KeloInventory, localPlayer, KeloNetAuthority
 * state-owned: STATE.equipmentSlots + atributos de equipment
 * extension-points: equipItem/unequipItem/applyServerItem
 * reuse: equipo nuevo usa el contrato existente y almacenamiento KeloInventory
 * legacy: equipmentSlots permanece dentro de STATE durante transición
 * do-not: NO escribir STATE.inventory ni crear otra identidad/colección de items
 */
(function(){
'use strict';
const VERSION='equipment-v1.2.0';
const inventoryOwner=window.KeloInventory;
if(!inventoryOwner)throw new Error('KeloInventory unavailable before equipment-system');
...
function externalStoredItems(){return inventoryOwner.getItems('warehouse').concat(inventoryOwner.getItems('market_escrow'));}
...
function ensure(){
 ...
 try{
  ...
  const portableItems=inventoryOwner.getItems('backpack');
  SLOTS.forEach(function(slot){
   let portable=portableItems.find(function(x){return x&&x.kind==='equipment'&&x.id===uid(slot)});
   ...
   if(!item){item=template(slot);inventoryOwner.addItem('backpack',item,{persist:false});portable=item;}
...
function allEquipment(skipEnsure){if(!skipEnsure)ensure();return inventoryOwner.getItems('backpack').filter(function(x){return x&&x.kind==='equipment';});}
function find(id,skipEnsure){return allEquipment(skipEnsure).find(function(x){return x.id===id;})||null;}
function equippedItems(skipEnsure){if(!skipEnsure)ensure();const out=[];SLOTS.forEach(function(slot){const id=STATE.equipmentSlots[slot],item=id?find(id,true):null;if(item&&item.slot===slot)out.push(item);});return out;}
function isEquipped(itemId,skipEnsure){if(!skipEnsure)ensure();return SLOTS.some(function(slot){return STATE.equipmentSlots[slot]===itemId;});}
function getArmorScore(player,skipEnsure){if(player&&player!==localPlayer&&Number.isFinite(player.armorScore))return Math.max(0,Math.floor(player.armorScore));return equippedItems(!!skipEnsure).reduce(function(sum,item){return sum+(GRADE_ARMOR_POINTS[clampTier(item.grade)]||0);},0);}
...
function avg(key,player,skipEnsure){if(player&&player!==localPlayer&&Number.isFinite(player[key]))return Number(player[key]);const items=equippedItems(!!skipEnsure);if(!items.length)return 0;return items.reduce(function(s,x){return s+clampTier(x[key==='averageQuality'?'quality':'grade']);},0)/items.length;}
function getAverageQuality(player,skipEnsure){return avg('averageQuality',player,skipEnsure);}
...
function equipItem(itemId){ensure();const item=find(itemId,true);if(!item)return {ok:false,error:'ITEM_NOT_FOUND'};if(!SLOTS.includes(item.slot))return {ok:false,error:'INVALID_SLOT'};SLOTS.forEach(function(slot){if(STATE.equipmentSlots[slot]===itemId)STATE.equipmentSlots[slot]=null;});STATE.equipmentSlots[item.slot]=itemId;recalculate(localPlayer,true);inventoryOwner.persist();return {ok:true,item,armorScore:localPlayer.armorScore,auraRank:localPlayer.auraRank};}
function unequipItem(slot){ensure();if(!SLOTS.includes(slot))return {ok:false,error:'INVALID_SLOT'};const id=STATE.equipmentSlots[slot]||null;STATE.equipmentSlots[slot]=null;recalculate(localPlayer,true);inventoryOwner.persist();return {ok:true,itemId:id};}
function nextAura(score){const rank=getAuraRank(score);return rank>=9?null:{rank:rank+1,threshold:AURA_THRESHOLDS[rank+1]};}
function applyServerItem(serverItem){ensure();if(!serverItem||!serverItem.id)return null;const item=find(serverItem.id,true);if(!item)return null;['itemLevel','quality','grade'].forEach(function(k){if(Number.isFinite(Number(serverItem[k])))item[k]=k==='itemLevel'?Math.max(1,Math.floor(Number(serverItem[k]))):clampTier(serverItem[k]);});recalculate(localPlayer,true);inventoryOwner.persist();return item;}
ensure();
...
window.KELO_EQUIPMENT_AUDIT={version:VERSION,ready:true,inventoryOwner:'KeloInventory',directInventoryWrites:false,explicitEmptySlots:true,emptySlotEncoding:'null',warehouseAwareStarterIdentity:true,marketEscrowAwareStarterIdentity:true,externalContainerAwareStarterIdentity:true,minSlots:6,slotCount:SLOTS.length,maxQuality:9,maxGrade:9,maxAura:9,maxReachableArmorScore:SLOTS.length*GRADE_ARMOR_POINTS[9]};
})();