 ...
 * owner: KeloEquipment
 * purpose: equipo del jugador legacy-compatible; publica modifiers al owner compartido KeloStats
 * public-api: window.KeloEquipment
 * consumes: STATE inventory/equipmentSlots, KeloStats opcional, KeloNetAuthority
 * state-owned: STATE.equipmentSlots + starter equipment identity
 * extension-points: KeloStats source player-equipment; no añadir nuevos dominios hardcodeados aquí
 * online: syncEquipment mantiene authority boundary existente; final stats se pueden recomputar desde catálogo autoritativo
...
function installStatsSource(){if(statsSourceInstalled||!window.KeloStats||typeof window.KeloStats.registerSource!=='function')return false;window.KeloStats.registerSource('player-equipment',function(){return playerEquipmentModifiers();});statsSourceInstalled=true;return true;}
function statsDirty(){if(window.KeloStats&&typeof window.KeloStats.markDirty==='function')window.KeloStats.markDirty();}
function aggregateFinal(player){if(!player||!window.KeloStats)return null;installStatsSource();const base={attack:Number(player.baseAttack)||0,defense:Number(player.baseDefense)||0,hp:Number(player.baseMaxHp)||Number(player.maxHp)||100};const context={mounted:window.KeloMounts?.isMounted?.()===true,mountId:window.KeloMounts?.getEquippedMountId?.()||null,inCombat:typeof isPvPActive!=='undefined'&&isPvPActive===true};const resolved=window.KeloStats.resolve('player',base,context);player.finalStats=resolved.stats;player.statRevision=resolved.revision;return resolved.stats;}
...
window.KELO_EQUIPMENT_AUDIT={version:VERSION,ready:true,explicitEmptySlots:true,emptySlotEncoding:'null',warehouseAwareStarterIdentity:true,marketEscrowAwareStarterIdentity:true,externalContainerAwareStarterIdentity:true,minSlots:6,slotCount:SLOTS.length,maxQuality:9,maxGrade:9,maxAura:9,maxReachableArmorScore:SLOTS.length*GRADE_ARMOR_POINTS[9],sharedStatAdapter:true,mountEquipmentSeparated:true};
})();