 ...
 * keys: STATUS CC STUN ROOT SLOW SILENCE STAGGER INVULNERABLE BURN POISON BLEED STACK REFRESH SANDBOX TEST LAB
 * purpose: capability de status reutilizable consumida por KeloEffectEngine; no conoce abilities concretas ni visuals
 * public-api: apply/remove/clear/has/snapshot/tick/stepSandbox/movementMultiplier/isActionBlocked/isInvulnerable
 ...
 * reuse: abilities, melee stagger, dodge iFrames, mounts, TestLab y futuros buffs
...
(function(root){
  ...
  function normalize(effect,ctx){
    ...
    return{
      id:String(e.id||nowId(type)),type,source:ctx&&ctx.source||null,sourceId:String(e.sourceId||ctx&&ctx.source&&ctx.source.id||''),target:ctx&&ctx.target||null,sandbox:!!(ctx&&ctx.sandbox===true),
  ...
  function apply(target,effect,context){
    ...
    const rows=list(target,true),same=rows.find(s=>s.type===incoming.type&&s.sourceId===incoming.sourceId&&s.sandbox===incoming.sandbox);
    let status=incoming;
    if(same){
      status=same;
    ...
    appliedCount++;const payload={status:publicStatus(status),target,targetActor:target,targetActorId:String(target.id||''),source:ctx.source||null,sourceActorId:status.sourceId,sandbox:status.sandbox};
  ...
  function periodicDamage(status,target){
    if(!root.KeloDamageResolver||isInvulnerable(target))return null;const amount=Math.max(0,status.magnitude*status.stacks),result=root.KeloDamageResolver.apply(target,amount,{sandbox:status.sandbox===true,status:status.type});
    const payload={source:status.source,target,targetActor:target,targetActorId:String(target.id||''),status:status.type,damageType:status.damageType,amount:result.amount,hp:result.hp,absorbed:result.absorbed,killed:result.killed,sandbox:status.sandbox===true};
  ...
  function tickTarget(target,dt,mode){const rows=list(target,false);for(let i=rows.length-1;i>=0;i--){const s=rows[i];if(mode==='live'&&s.sandbox)continue;if(mode==='sandbox'&&!s.sandbox)continue;s.remaining-=dt;if(PERIODIC.has(s.type)){s.tick-=dt;while(s.tick<=0&&s.remaining>0){s.tick+=s.tickInterval;periodicDamage(s,target);}}
      if(s.remaining<=0){rows.splice(i,1);removedCount++;const payload={status:publicStatus(s),target,targetActor:target,targetActorId:String(target.id||''),reason:'expired',sandbox:s.sandbox===true};emit(root.KeloCombatSchema.events.CC_REMOVED,payload);emit('STATUS_REMOVED',payload);}}
  }
  const knownTargets=new Set();
  function track(target){if(target)knownTargets.add(target);return target;}
  const rawApply=apply;
  function applyTracked(target,effect,context){const result=rawApply(target,effect,context);if(!(context&&context.sandbox===true))track(target);return result;}
  function tick(dt){dt=Math.max(0,Number(dt)||0);ticks++;Array.from(knownTargets).forEach(target=>{if(!target){knownTargets.delete(target);return;}tickTarget(target,dt,'live');if(!list(target,false).some(s=>!s.sandbox&&s.remaining>0))knownTargets.delete(target);});}
  function stepSandbox(target,dt){dt=Math.max(0,Number(dt)||0);if(!target)return 0;tickTarget(target,dt,'sandbox');return list(target,false).filter(s=>s.sandbox&&s.remaining>0).length;}
  ...
  root.KELO_STATUS_EFFECTS_AUDIT=Object.freeze({version:VERSION,ready:true,owner:true,semanticEvents:true,periodicDamageUsesKeloDamageResolver:true,simulationHook:!!(root.KeloSimulation&&root.KeloSimulation.after),sandboxIsolated:true,sandboxStep:true});
  root.KeloStatusEffects=Object.freeze({version:VERSION,apply:applyTracked,remove,clear,has,snapshot,tick,stepSandbox,movementMultiplier,isActionBlocked,isInvulnerable,metrics:()=>Object.freeze({applied:appliedCount,removed:removedCount,ticks,targets:knownTargets.size})});
