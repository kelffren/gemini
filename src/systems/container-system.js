(function(){
'use strict';
const VERSION='container-v1.1.0';
...
function cloneState(){return {inventory:JSON.parse(JSON.stringify(STATE.inventory)),backpack:JSON.parse(JSON.stringify(STATE.backpack)),warehouse:JSON.parse(JSON.stringify(STATE.warehouse)),marketEscrow:JSON.parse(JSON.stringify(STATE.marketEscrow))};}
function restore(s){STATE.inventory=s.inventory;STATE.backpack=s.backpack;STATE.warehouse=s.warehouse;STATE.marketEscrow=s.marketEscrow;}
...
window.KeloContainers=Object.freeze({version:VERSION,schemaVersion:SCHEMA_VERSION,warehouseCapacity:WAREHOUSE_CAPACITY,marketEscrowBaseCapacity:ESCROW_BASE_CAPACITY,ensure,keyForItem,getSlots,getStats,transferItem,canStack,stackSignature});
window.KELO_CONTAINER_AUDIT=Object.freeze({version:VERSION,schemaVersion:SCHEMA_VERSION,identityRule:'one-item-one-container',containerModel:'separate-item-arrays-shared-contract-v2',transferMode:'validate-prepare-execute-persist-rollback-v2',strictDestinationCapacity:true,warehouseImplemented:true,marketEscrowImplemented:true,marketEscrowMerge:false,marketEscrowElastic:true,deferredPersistSupported:true,serverAuthoritative:false});
})();