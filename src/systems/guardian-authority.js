...
root.KeloGuardianAuthority=Object.freeze({
  ...
  communityAttest:payload=>guarded(()=>rpc('guardian_community_attest',{
    p_node_id:String(payload?.nodeId||''),
    p_job_id:String(payload?.jobId||''),
    p_asset_hash:String(payload?.assetHash||'').toLowerCase(),
    p_preset:String(payload?.preset||''),
    p_master_epoch:Math.max(0,Math.floor(Number(payload?.masterEpoch)||0))
  })),
