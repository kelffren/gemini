 ...
 * keys: ARENA HIGHLIGHTS FIRST BLOOD DOUBLE KILL SHUTDOWN TOWER CORE SIGIL COMEBACK ESCAPE
 * purpose: detectar momentos competitivos verificables a partir de eventos/snapshots existentes; no decide gameplay
 ...
 * online: highlights cliente son presentation/analytics; servidor deberá validar highlights oficiales de Ranked
...
const VERSION='kelo-arena-highlights-v1.0.0';
...
let matchId=null,mode=null,moments=[],lastMoments=[],eventsBound=false,firstBlood=false,lastLocalKillAt=-Infinity,localMulti=0,heroStreaks=new Map(),maxDeficit={alpha:0,beta:0},comebackEmitted={alpha:false,beta:false},structureAlive={},critical=null,sigilTrack={team:null,progress:0,contestedTeam:null,contestedProgress:0};
...
function close(){if(matchId)lastMoments=Object.freeze(moments.slice());}
...
function current(){return Object.freeze(moments.slice());}
function snapshot(){return Object.freeze({version:VERSION,bound:eventsBound,matchId,mode,current:current(),last:lastMoments});}
root.KeloArenaHighlights=Object.freeze({version:VERSION,snapshot,current,last:()=>lastMoments,reset});
root.KELO_ARENA_HIGHLIGHTS_AUDIT=Object.freeze({version:VERSION,owner:'KeloArenaHighlights',semantic:true,firstBlood:true,multiKill:true,shutdown:true,towerCore:true,warSigilSteal:true,comeback:true,criticalEscape:true,combatWrapper:false,gameplayAuthority:false,secondLoop:false,serverValidationPending:true});
