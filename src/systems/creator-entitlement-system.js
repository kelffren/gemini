...
(function(root){
'use strict';
const VERSION='creator-entitlements-v1.1.0-provider';
...
function currentAccount(){return providerAccount()||text(authState()?.accountId||accountId);}
...
function bindProvider(next){
  ...
  provider=Object.freeze({name:text(next.name||'bound-provider'),accountId:next.accountId,listAccess:next.listAccess,checkAccess:next.checkAccess});emit('provider-bound');return api;
}
...
async function refresh(){
  ...
  refreshPromise=(async()=>{
    try{
      ...
      if(provider&&providerAccount()){me=providerAccount();rows=await provider.listAccess();}
...
async function verifyRevision(revisionId){
  ...
  if(provider&&providerAccount()){row=await provider.checkAccess(id);me=providerAccount();}
...
for(const name of ['kelo:online-auth-ready','kelo:account-signed-in','kelo:account-created','kelo:profile-complete'])root.addEventListener?.(name,()=>void refresh());
root.addEventListener?.('kelo:account-signed-out',()=>{if(!providerAccount())clear('signed-out');});
