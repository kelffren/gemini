 ...
 * keys: ARENA MASTERY RIVALRY BO3 SERIES SKILL OBJECTIVES TELEMETRY PROGRESSION
 * purpose: progresión competitiva persistente separada de MMR; escucha resultados/telemetría Arena y proyecta Mastery, objetivos y series
 * public-api: snapshot/getMastery/getRivalry/rematchSeries/resetSeries
 * consumes: KeloArena + KeloArenaTelemetry + kelo:arena-match-finished
...
const VERSION='kelo-arena-progression-v1.1.0-telemetry';
...
function evaluateObjectives(result,t,unlocked){if(result.won)unlock('first_win',unlocked);if(result.won&&result.mode==='control')unlock('control_win',unlocked);if(result.won&&result.mode==='moba')unlock('moba_win',unlocked);if(result.won&&Array.isArray(result.accolades)&&result.accolades.includes('OVERTIME'))unlock('overtime_win',unlocked);if(result.won&&Array.isArray(result.accolades)&&result.accolades.includes('REMONTADA'))unlock('comeback_win',unlocked);if(result.won&&result.mode==='moba'&&result.reason==='core-destroyed')unlock('core_breaker',unlocked);if(t){if(t.attacks>=10&&t.accuracy>=.6)unlock('accurate_fighter',unlocked);if(t.objectiveSeconds>=30)unlock('objective_anchor',unlocked);if(t.damageStructures>=150)unlock('siege_pressure',unlocked);if(result.won&&result.mode==='moba'&&t.overextensions===0)unlock('clean_duel',unlocked);if(t.effectiveDodges>=1)unlock('effective_dodge',unlocked);if(t.interrupts>=1)unlock('interruptor',unlocked);}const a=root.KeloArena&&root.KeloArena.snapshot?root.KeloArena.snapshot():null;const streak=a&&a.profile&&a.profile.bests?Number(a.profile.bests.currentStreak)||0:0;if(streak>=3)unlock('three_streak',unlocked);if(state.matches>=10)unlock('ten_matches',unlocked);if(state.matches>=25)unlock('twenty_five_matches',unlocked);}
function xpFor(result,t){const quality=Math.max(0,Math.min(1,Number(result.quality)||0));let raw=18;if(result.won)raw+=14;if(result.reason==='core-destroyed'||result.reason==='control-score')raw+=6;raw+=Math.min(18,(result.accolades&&result.accolades.length||0)*6);if(t){if(t.attacks>=10&&t.accuracy>=.6)raw+=4;if(t.objectiveSeconds>=25||t.damageStructures>=120)raw+=4;if(t.effectiveDodges||t.interrupts)raw+=Math.min(6,(t.effectiveDodges+t.interrupts)*2);}const factor=.35+.65*quality;return Math.max(4,Math.round(raw*factor));}
function process(snapshot){const result=snapshot&&snapshot.lastResult;if(!result||result.aborted)return;const opponent=opponentFrom(snapshot);if(!opponent)return;const t=telemetry();state.matches++;if(result.won)state.wins++;const gain=xpFor(result,t),before=mastery();state.xp+=gain;const after=mastery(),rivalry=ensureRivalry(opponent,result.mode);rivalry.matches++;rivalry.lastPlayedAt=Date.now();if(result.won)rivalry.wins++;else rivalry.losses++;const series=ensureSeries(opponent,result.mode);series.games++;if(result.won)series.playerWins++;else series.opponentWins++;if(series.playerWins>=2||series.opponentWins>=2){series.complete=true;series.winner=series.playerWins>series.opponentWins?'player':'opponent';}const unlocked=[];evaluateObjectives(result,t,unlocked);state.lastGain={xp:gain,from:before.label,to:after.label,tierUp:before.id!==after.id,unlocked:Object.freeze(unlocked.slice()),opponentName:opponent.name,isBot:opponent.isBot,series:Object.freeze({...series}),telemetry:t?Object.freeze({...t}):null};persist();emit('updated',snapshotProgression());}
...
root.KELO_ARENA_PROGRESSION_AUDIT=Object.freeze({version:VERSION,owner:'KeloArenaProgression',mastery:true,skillObjectives:true,telemetryObjectives:true,rivalries:true,bo3:true,statBuffs:false,mmrAuthority:false,secondLoop:false,localFallback:true,serverAuthorityPending:true});emit('ready',snapshotProgression());
