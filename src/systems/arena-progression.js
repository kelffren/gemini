 ...
 * keys: ARENA MASTERY RIVALRY BO3 SERIES SKILL OBJECTIVES TELEMETRY HIGHLIGHTS PROGRESSION HOT BALANCE
 ...
 * consumes: KeloArena + KeloArenaTelemetry + KeloArenaHighlights + KeloArenaProgressionHotBalance + kelo:arena-match-finished
 ...
 * do-not: NO modificar HP/MMR/rating/ganador, NO buffs de stats, NO loot aleatorio, NO segundo loop, NO reescribir XP histórico al aplicar balance
 */
(function(root){
'use strict';
if(root.KeloArenaProgression||!root.KeloArena)return;
const VERSION='kelo-arena-progression-v1.3.0-hot-balance';
...
function liveTiers(){try{const hot=root.KeloArenaProgressionHotBalance;if(hot&&typeof hot.tiers==='function')return hot.tiers(MASTERY_TIERS);}catch(_){}return MASTERY_TIERS;}
function mastery(xp){xp=Number.isFinite(Number(xp))?Math.max(0,Number(xp)):state.xp;const tiers=liveTiers();let tier=tiers[0],next=null;for(let i=0;i<tiers.length;i++){if(xp>=tiers[i].min)tier=tiers[i];else{next=tiers[i];break;}}const span=next?Math.max(1,next.min-tier.min):1,progress=next?Math.max(0,Math.min(1,(xp-tier.min)/span)):1;return Object.freeze({id:tier.id,label:tier.label,xp:Math.round(xp),next:next&&next.label||null,nextAt:next&&next.min||null,progress});}
...
root.KeloArenaProgression=Object.freeze({version:VERSION,get tiers(){return liveTiers();},objectives:OBJECTIVES,snapshot:snapshotProgression,getMastery:mastery,getRivalry,rematchSeries,resetSeries});
root.KELO_ARENA_PROGRESSION_AUDIT=Object.freeze({version:VERSION,owner:'KeloArenaProgression',mastery:true,skillObjectives:true,telemetryObjectives:true,highlightObjectives:true,rivalries:true,bo3:true,hotBalance:true,hotBalanceOwner:'KeloArenaProgressionHotBalance',historicalXpMutation:false,statBuffs:false,mmrAuthority:false,secondLoop:false,localFallback:true,serverAuthorityPending:true});emit('ready',snapshotProgression());
