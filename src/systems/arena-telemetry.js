...
const VERSION='kelo-arena-telemetry-v1.0.0';
const WORLD=Object.freeze({x:2660,y:360,w:720,h:720,cx:3020,cy:720});
let matchId=null,mode=null,lastSummary=null,stats=fresh();
...
function close(){if(!matchId)return;lastSummary=buildSummary();}
...
function tick(ctx){const s=arena();if(!s||s.status!=='active'||!s.match){return;}if(matchId!==s.match.id)begin(s);sample(clamp(Number(ctx&&ctx.dt)||0,0,.05),s);}
const E=root.KeloCombatSchema&&root.KeloCombatSchema.events||{};
if(E.ATTACK_RESOLVED)root.KeloEvents.on(E.ATTACK_RESOLVED,onResolved);
if(E.DAMAGE_APPLIED)root.KeloEvents.on(E.DAMAGE_APPLIED,onDamage);
if(E.DAMAGE_BLOCKED)root.KeloEvents.on(E.DAMAGE_BLOCKED,onBlocked);
if(E.DODGE_STARTED)root.KeloEvents.on(E.DODGE_STARTED,onDodge);
if(E.CC_APPLIED)root.KeloEvents.on(E.CC_APPLIED,onCc);
if(E.ENTITY_KILLED)root.KeloEvents.on(E.ENTITY_KILLED,onKill);
root.addEventListener('kelo:arena-match-started',e=>{const s=e&&e.detail||arena();if(s&&s.match)begin(s);});
...
function snapshot(){return Object.freeze({version:VERSION,current:current(),last:lastSummary});}
root.KeloArenaTelemetry=Object.freeze({version:VERSION,snapshot,current,last:()=>lastSummary,reset});
root.KELO_ARENA_TELEMETRY_AUDIT=Object.freeze({version:VERSION,owner:'KeloArenaTelemetry',semanticEvents:true,combatWrapper:false,secondLoop:false,ratingAuthority:false,masteryAuthority:false,serverValidationPending:true});
