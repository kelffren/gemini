 ...
 * state-owned: targetX/Y, follow tuning, zoom efectivo/base, viewport policy, DPR policy y comandos externos de foco
 ...
 * do-not: NO escribir camera.targetX/Y, CONFIG.zoom/camera tuning, canvas.width/height o reemplazar resize desde features nuevas
 */
(function(root){
  'use strict';
  ...
  const VERSION='kelo-camera-v1.2.0';
  ...
  for(const key of TUNING_KEYS){
    Object.defineProperty(CONFIG,key,{configurable:true,enumerable:true,get:()=>managedTuning[key],set:value=>{const n=Number(value);if(Number.isFinite(n))managedTuning[key]=n;}});
  }
  ...
  function pixelPerfectZoom(target){const dpr=activeDpr();const physicalScale=Math.max(1,Math.round((Number(target)||1)*dpr));return physicalScale/dpr;}
  ...
  function applyZoom(source){
    ...
    if(changed)emit('kelo:camerazoomchange',{source:source||'camera',orientation:mode,baseZoom,effectiveZoom,verticalWorldSpan:root.innerHeight/effectiveZoom,portraitReferenceWorldSpan:Math.max(root.innerWidth,root.innerHeight)/baseZoom});
  ...
  function syncViewport(source){
    screenW=root.innerWidth;screenH=root.innerHeight;
    const dpr=activeDpr(),width=Math.max(1,Math.floor(screenW*dpr)),height=Math.max(1,Math.floor(screenH*dpr));
    if(canvas.width!==width)canvas.width=width;if(canvas.height!==height)canvas.height=height;
    canvas.style.width=screenW+'px';canvas.style.height=screenH+'px';canvas.style.imageRendering=imageRendering;
    ctx.setTransform(dpr,0,0,dpr,0,0);ctx.imageSmoothingEnabled=smoothing;CONFIG.roundPixels=roundPixels;syncViewportCss();
    const zoom=applyZoom(source||'viewport'),key=[screenW,screenH,dpr,zoom].join(':');
    if(key!==lastViewportKey){lastViewportKey=key;emit('kelo:viewportchange',{source:source||'viewport',width:screenW,height:screenH,dpr,orientation:orientation(),baseZoom,effectiveZoom:zoom});try{root.dispatchEvent(new CustomEvent('kelo:world-audit'));}catch(e){}}
    return snapshot();
  }
  ...
  function snapshot(){return Object.freeze({version:VERSION,x:camera.x,y:camera.y,targetX:managedTargetX,targetY:managedTargetY,baseZoom,effectiveZoom,orientation:orientation(),screenW,screenH,dpr:activeDpr(),dprCap,pixelPerfect,roundPixels,smoothing,follow:getFollowTuning()});}
  ...
  root.KeloCamera=Object.freeze({version:VERSION,setTarget,focus,setFollowTuning,getFollowTuning,setBaseZoom,getBaseZoom:()=>baseZoom,getEffectiveZoom:()=>effectiveZoom,cycleZoom,refreshZoom,getOrientation:orientation,configureViewport,syncViewport,scheduleViewportSync,syncViewportCss,activeDpr,pixelPerfectZoom,screenToWorld:screenToWorldPoint,worldToScreen:worldToScreenPoint,snapshot});
  root.KELO_CAMERA_AUDIT=Object.freeze({version:VERSION,owner:'KeloCamera',legacyFollowMath:true,legacyTargetAdapter:true,legacyZoomAdapter:true,legacyTuningAdapter:true,updateCameraOwner:true,viewportOwner:true,zoomOwner:true,targetOwner:true,screenWorldOwner:true});
