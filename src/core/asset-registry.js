 ...
 * keys: ASSET LIBRARY FEATURE GATE LOCALSTORAGE PRESET DIAGNOSTIC MOBILE FEATURE-REGISTRY CIRCUIT-BREAKER
 * purpose: persistent allow-list for optional runtime packs plus a thin effective-load gate for X-Foundation failure isolation. Core boot stays untouched.
 * consumes: KELO_FEATURE_REGISTRY + optional KeloFailureBoundary; fallback legacy for deploy compatibility
 */
(function(root){
'use strict';
if(root.KELO_ASSET_REGISTRY)return;
const VERSION='kelo-asset-registry-v2.1-failure-boundary';
...
function selectionEnabled(id){id=String(id||'');if(!KNOWN.includes(id))return true;return read()[id]!==false;}
function failureBlocked(id){try{return !!(root.KeloFailureBoundary&&typeof root.KeloFailureBoundary.isBlocked==='function'&&root.KeloFailureBoundary.isBlocked(String(id||'')));}catch(_){return false;}}
function isEnabled(id){id=String(id||'');return selectionEnabled(id)&&!failureBlocked(id);}
function getState(){const features=read(),blocked=KNOWN.filter(failureBlocked);return Object.freeze({version:VERSION,registryVersion:root.KELO_FEATURE_REGISTRY?.version||'legacy-fallback',storageKey:STORAGE_KEY,known:KNOWN.slice(),enabled:KNOWN.filter(function(id){return features[id];}),disabled:KNOWN.filter(function(id){return !features[id];}),failureBlocked:Object.freeze(blocked),effectiveEnabled:Object.freeze(KNOWN.filter(function(id){return features[id]&&!blocked.includes(id);})),features:Object.freeze({...features})});}
...
function explain(id){id=String(id||'');const selected=selectionEnabled(id),blocked=selected&&failureBlocked(id),enabled=selected&&!blocked;return Object.freeze({id,enabled,selected,blocked,known:KNOWN.includes(id),reason:!selected?'disabled-by-asset-library':blocked?'failure-circuit-open':'enabled-or-core'});}
...
root.KELO_ASSET_REGISTRY_AUDIT=Object.freeze({version:VERSION,userSelectionPreserved:true,failureBoundaryGate:true,loadAuthority:false,timers:0});
