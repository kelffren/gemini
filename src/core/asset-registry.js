...
function write(features,reason){const normalized=normalize(features);const payload={version:1,updatedAt:new Date().toISOString(),features:normalized};try{localStorage.setItem(STORAGE_KEY,JSON.stringify(payload));}catch(_){}emit('kelo:asset-selection-changed',{features:{...normalized},reason:reason||'apply'});return normalized;}
function isEnabled(id){id=String(id||'');if(!KNOWN.includes(id))return true;return read()[id]!==false;}
function getState(){const features=read();return Object.freeze({version:VERSION,storageKey:STORAGE_KEY,known:KNOWN.slice(),enabled:KNOWN.filter(function(id){return features[id];}),disabled:KNOWN.filter(function(id){return !features[id];}),features:Object.freeze({...features})});}
...
root.KELO_ASSET_REGISTRY=Object.freeze({version:VERSION,storageKey:STORAGE_KEY,catalogUrl:CATALOG_URL,known:KNOWN.slice(),isEnabled,getState,explain,apply,setEnabled,applyPreset,reset,loadCatalog});
