 ...
 * keys: ASSET LIBRARY FEATURE GATE LOCALSTORAGE PRESET DIAGNOSTIC MOBILE FEATURE-REGISTRY INTERNAL
 * purpose: persistent allow-list only for user-toggleable runtime packs. Internal lifecycle packs stay outside user asset selection.
...
const VERSION='kelo-asset-registry-v3-feature-registry-toggleable';
...
function registryToggleable(){
  const registry=root.KELO_FEATURE_REGISTRY;
  if(Array.isArray(registry?.toggleableIds))return registry.toggleableIds.slice();
  if(Array.isArray(registry?.ids)&&typeof registry.get==='function')return registry.ids.filter(function(id){return registry.get(id)?.userToggle!==false;});
  return LEGACY_KNOWN.slice();
}
const KNOWN=Object.freeze(registryToggleable());
...
function write(features,reason){const normalized=normalize(features);const payload={version:3,updatedAt:new Date().toISOString(),features:normalized};try{localStorage.setItem(STORAGE_KEY,JSON.stringify(payload));}catch(_){}emit('kelo:asset-selection-changed',{features:{...normalized},reason:reason||'apply'});return normalized;}
