...
const VERSION='kelo-download-center-v1.0.1-safe';
...
async function render(){
  ...
  if(safe)safe.textContent=paused?'Pausado':(gameplayBusy()?'Gameplay: pausa':'Seguro');if(q)q.textContent='Cola '+queue.length;if(f)f.textContent='Frame '+(lastFrameMs==null?'--':Math.round(lastFrameMs))+' ms';if(!list)return;
