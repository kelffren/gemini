 ...
 * keys: UPDATE GATE VERSION WATCH LAZY TURBO V5 MOBILE SAFARI CONDITIONAL REVALIDATION
 * purpose: detect new builds with tiny conditional requests and wake verified predictive updater V5 only when necessary
 * public-api: KeloUpdateGate.checkNow/wakeHeavy/getState
 * state-owned: one timeout + lightweight build state; V5 owns downloads and verification
 * do-not: NO asset downloads, NO repo walk, NO Service Worker, NO setInterval, NO gameplay state
 */
(function(root){
'use strict';
if(root.KeloUpdateGate)return;
const VERSION='kelo-update-gate-v3-predictive';
...
function getState(){return Object.freeze({version:VERSION,installedBuild:installed(),pendingBuild:pending(),deployedBuild:lastDeployed,checking,heavyLoading:!!heavyLoading,heavyReady:!!root.KeloUpdater||heavyReady,lastError,lastReason,stopped,nextCheckMs:nextDelay()});}
...
function start(){
  ...
  const arm=function(){if(pending()){setTimeout(function(){void wakeHeavy('apply-resume');},0);return;}schedule(FIRST_CHECK_DELAY_MS);};
  if(document.readyState==='complete')arm();else root.addEventListener('load',arm,{once:true});
  document.addEventListener('visibilitychange',function(){if(document.visibilityState==='visible')schedule(250);});
