 ...
 * keys: UPDATE GATE VERSION WATCH LAZY TURBO V4 MOBILE SAFARI ZERO-BOOT
 * purpose: detectar builds nuevos con pocos bytes y cargar el updater V4 instant-delta solo cuando realmente hace falta
 * public-api: KeloUpdateGate.checkNow/wakeHeavy/getState
 * state-owned: un timeout efímero + estado del gate; V4 conserva autoridad de actualización
 * do-not: NO asset downloads, NO repo walk, NO Service Worker, NO polling interval, NO gameplay state
 */
(function(root){
'use strict';
if(root.KeloUpdateGate)return;
const VERSION='kelo-update-gate-v2-instant';
...
function pending(){try{return normalize(sessionStorage.getItem(PENDING_KEY));}catch(_){return null;}}
...
function getState(){return Object.freeze({version:VERSION,installedBuild:installed(),pendingBuild:pending(),deployedBuild:lastDeployed,checking,heavyLoading:!!heavyLoading,heavyReady:!!root.KeloUpdater||heavyReady,lastError,lastReason,stopped});}
...
function load(src){
  return new Promise(function(resolve,reject){
    ...
    const s=document.createElement('script');s.src=src;s.async=false;s.dataset.keloUpdateGate='1';s.onload=resolve;s.onerror=function(){reject(new Error('update_gate_script_load_failed:'+src));};document.head.appendChild(s);
  });
}
function wakeHeavy(reason){
  ...
  heavyLoading=load('src/core/update-system-v4.js?v=4-instant-delta')
    ...
    .then(function(){
      ...
      try{root.dispatchEvent(new CustomEvent('kelo:update:connected',{detail:{version:'turbo-v4',mode:'instant-delta-gate',reason:lastReason}}));}catch(_){}
...
async function checkNow(){
  ...
  try{
    ...
    const current=installed(),resume=pending();
    const requested=normalize(new URL(root.location.href).searchParams.get('kelo_update'));
    if(resume||requested||!current){
      lastReason=resume||requested?'apply-resume':'first-install';emit('heavy-needed',{reason:lastReason});await wakeHeavy(lastReason);return getState();
    }
...
function start(){
  ...
  const arm=function(){
    if(pending()){setTimeout(function(){void wakeHeavy('apply-resume');},0);return;}
