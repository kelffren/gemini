...
const VERSION='kelo-updater-v5-verified-predictive';
...
const state={
  ...
  metrics:{compareMs:null,indexMs:null,downloadMs:null,timeToReadyMs:null,mode:null,verifiedFiles:0,syntaxChecked:0,hotsetHits:0,consistencyRetries:0,compareCacheHit:false,healthAttempts:0}
};
...
function sessionResources(){const out=new Map();try{performance.getEntriesByType('resource').forEach(e=>{const name=e&&e.name;if(!name||!stageable(name)||!/\.(?:js|mjs|css|json|woff2?|ttf|otf)(?:[?#].*)?$/i.test(name))return;const path=repoPath(name);if(path)out.set(path,{url:name,path,source:'session'});});}catch(_){}return out;}
...
function hotsetResources(){const out=new Map();for(const h of readHotset().slice(0,HOTSET_PREWARM_LIMIT)){if(h&&h.path&&h.url&&stageable(h.url))out.set(h.path,{url:h.url,path:h.path,source:'hotset'});}return out;}
...
async function fetchVerifiedIndex(build,compare){const sha=expectedSha(compare,'index.html');for(let attempt=0;attempt<CONSISTENCY_DELAYS.length;attempt++){if(CONSISTENCY_DELAYS[attempt])await sleep(CONSISTENCY_DELAYS[attempt]);const buffer=await fetchIndexBuffer(build);if(!sha)return {buffer,verified:false};const result=await verifyBuffer(buffer.slice(0),{expectedSha:sha,path:'index.html',checkSyntax:false},true);if(result.ok&&result.hashOk!==false){state.metrics.verifiedFiles++;return {buffer,verified:true};}state.metrics.consistencyRetries++;emit('consistency-wait',{build,path:'index.html',attempt:attempt+1,expectedSha:sha,actualSha:result.sha||null});}throw new Error('cdn_consistency_timeout:index.html');}
...
async function warmOne(entry,build,gen){
  ...
  for(let attempt=0;attempt<CONSISTENCY_DELAYS.length;attempt++){
    ...
    if(entry.expectedSha){verify=await verifyBuffer(buffer,{expectedSha:entry.expectedSha,path:entry.path,checkSyntax:/\.js$/i.test(entry.path)&&!/\.mjs$/i.test(entry.path)},true);if(verify.syntaxStatus==='ok')state.metrics.syntaxChecked++;if(verify.ok&&verify.hashOk!==false)state.metrics.verifiedFiles++;}
...
function armHealthCommit(pending,deployed){
  if(healthArmed)return;healthArmed=true;const p=Object.assign({},pending),target=normalize(p.build);p.attempts=Number(p.attempts||0)+1;state.metrics.healthAttempts=p.attempts;writePending(p);let fatal=0;const onError=()=>{fatal++;};global.addEventListener('error',onError);global.addEventListener('unhandledrejection',onError);
  const finish=async()=>{try{await twoFrames();await sleep(HEALTH_SETTLE_MS);if(!global.__keloBootReady)throw new Error('boot_ready_missing');if(fatal>0)throw new Error('post_boot_errors_'+fatal);if(target!==normalize(deployed))throw new Error('deployed_changed_during_health');writeInstalled(target);removeSession(PENDING_KEY);clearStage();state.availableBuild=null;state.status='current';state.lastError=null;emit('health-committed',{build:target,attempts:p.attempts});}catch(error){state.status='health-hold';state.lastError=String(error&&error.message||error);emit('health-hold',{build:target,attempts:p.attempts,error:state.lastError});if(p.attempts>=MAX_HEALTH_ATTEMPTS)blockBuild(target,state.lastError);}finally{global.removeEventListener('error',onError);global.removeEventListener('unhandledrejection',onError);}};
