 ...
 * keys: ADMIN APPROVALREQUEST LAZY MOBILE SAFARI NOTIFICATIONS
 * purpose: lazy entries for Administrador + ApprovalRequest; realtime approval notifications start only for authorized accounts
 * do-not: NO service_role, NO direct publish, NO polling, NO Studio dependency
 */
(function(root){
'use strict';
...
function sync(){
  ...
  let adminBtn=document.getElementById('lx-admin-control');
  if(!locallyAdminEligible())adminBtn?.remove();
  else{if(!adminBtn){adminBtn=document.createElement('button');adminBtn.id='lx-admin-control';adminBtn.type='button';adminBtn.className='lx-menu-item';adminBtn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();void openAdmin();});grid.appendChild(adminBtn);}paintAdmin(adminBtn,!!adminLoading);}

  let approvalBtn=document.getElementById('lx-approval-request');
  if(!locallyApprovalEligible())approvalBtn?.remove();
  else{if(!approvalBtn){approvalBtn=document.createElement('button');approvalBtn.id='lx-approval-request';approvalBtn.type='button';approvalBtn.className='lx-menu-item';approvalBtn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();void openApproval();});grid.appendChild(approvalBtn);}paintApproval(approvalBtn,!!approvalLoading);}
  return true;
}
async function loadPermissions(){
  if(root.KeloAccountPermissions)return root.KeloAccountPermissions;
  const mod=await import('./../auth/account-permissions-runtime.mjs?v=1');
  return mod.installAccountPermissions({root});
}
async function loadAdmin(){
  if(root.KeloAccountAdminPanel)return root.KeloAccountAdminPanel;
  if(adminLoading)return adminLoading;
  adminLoading=(async()=>{const p=await loadPermissions();if(!(p?.hasRole?.('admin')||p?.can?.('admin.panel')))throw new Error('ADMIN_PERMISSION_DENIED');const mod=await import('./../ui/account-admin-panel.mjs?v=3');return mod.installAccountAdminPanel({root});})().finally(()=>{adminLoading=null;sync();});
  return adminLoading;
}
...
async function openAdmin(){
  const btn=document.getElementById('lx-admin-control');paintAdmin(btn,true);
  try{const panel=await loadAdmin();if(!panel?.open)throw new Error('ADMIN_PANEL_UNAVAILABLE');await panel.open();return true;}
  ...
  finally{paintAdmin(document.getElementById('lx-admin-control'),false);}
}
async function openApproval(){
  const btn=document.getElementById('lx-approval-request');paintApproval(btn,true);
  try{const panel=await loadApproval();if(!panel?.open)throw new Error('APPROVAL_PANEL_UNAVAILABLE');await panel.open();approvalUnread=0;sync();return true;}
  catch(error){console.warn('[Kelo Approval lazy gate]',error);toast(String(error?.message||error).includes('APPROVAL_VIEW_DENIED')?'Tu cuenta no tiene permiso para ApprovalRequest':'No se pudo abrir ApprovalRequest');return false;}
  finally{paintApproval(document.getElementById('lx-approval-request'),false);}
}
async function bootApprovalNotifications(){
  if(notificationBoot)return notificationBoot;
  notificationBoot=(async()=>{
    ...
    approvalUnread=runtime.unread||0;sync();return true;
  })().catch(error=>{console.warn('[Kelo Approval notification boot]',error);return false;}).finally(()=>{notificationBoot=null;});
...
root.addEventListener('kelo:permissions-changed',()=>{sync();void bootApprovalNotifications();});
['kelo:online-auth-ready','kelo:account-signed-in','kelo:profile-complete','kelo:account-created'].forEach(name=>root.addEventListener(name,()=>void bootApprovalNotifications()));
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',sync,{once:true});else sync();
if(root.KeloOnlineAuth)queueMicrotask(()=>void bootApprovalNotifications());
