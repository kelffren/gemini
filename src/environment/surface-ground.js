 ...
 * consumes: KELO_TILE_REGISTRY, KELO_ENVIRONMENT_LAYERS, KELO_ATLAS_CONTRACT, KeloCamera, KELO_MOBILE_PERFORMANCE_CONTRACT
...
(function(){
'use strict';
const R=window.KELO_TILE_REGISTRY,L=window.KELO_ENVIRONMENT_LAYERS,A=window.KELO_ATLAS_CONTRACT,CAM=window.KeloCamera,P=window.KELO_MOBILE_PERFORMANCE_CONTRACT;
if(!R||!L?.register||!A?.acquire||!CAM?.worldView||!P?.budgets){console.error('[Kelo surface ground] registry/layers/atlas/camera/performance contract missing');return;}
...
const TILE=R.worldTileSize||32,CHUNK=512,COLS=atlas.columns||5,BASE=style.baseFrames||[1],DETAIL=style.detailFrames||[],MOD=Math.max(2,Number(style.detailModulo)||11),cache=new Map(),MAX=P.budgets.chunkCacheCap;
let img=null,ready=false,failed=false,drawnTiles=0,chunkBuildCount=0,chunkEvictionCount=0;
...
function build(cx,cy){const key=cx+','+cy;if(cache.has(key)){const hit=cache.get(key);cache.delete(key);cache.set(key,hit);return hit;}const c=document.createElement('canvas');c.width=CHUNK;c.height=CHUNK;const g=c.getContext('2d');g.imageSmoothingEnabled=false;const wx=cx*CHUNK,wy=cy*CHUNK,n=Math.ceil(CHUNK/TILE);for(let y=0;y<n;y++)for(let x=0;x<n;x++){const X=Math.floor((wx+x*TILE)/TILE),Y=Math.floor((wy+y*TILE)/TILE);drawFrame(g,frameFor(X,Y),x*TILE,y*TILE)}cache.set(key,c);chunkBuildCount++;if(cache.size>MAX){cache.delete(cache.keys().next().value);chunkEvictionCount++;}return c}
// KELO-INDEX CAMERA CULL: read the actual viewport, including orientation and zoom.
function visibleBounds(){const v=CAM.worldView(),ww=window.CONFIG?.worldWidth||3600,wh=window.CONFIG?.worldHeight||3200;return{minX:Math.max(0,Math.floor(v.left/CHUNK)),maxX:Math.min(Math.ceil(ww/CHUNK)-1,Math.floor(v.right/CHUNK)),minY:Math.max(0,Math.floor(v.top/CHUNK)),maxY:Math.min(Math.ceil(wh/CHUNK)-1,Math.floor(v.bottom/CHUNK))}}
function draw(g){if(!ready||failed||!img)return;const b=visibleBounds();for(let y=b.minY;y<=b.maxY;y++)for(let x=b.minX;x<=b.maxX;x++)g.drawImage(build(x,y),x*CHUNK,y*CHUNK);audit.chunkCacheSize=cache.size;audit.chunkBuildCount=chunkBuildCount;audit.chunkEvictionCount=chunkEvictionCount;audit.lastDrawnTiles=drawnTiles;drawnTiles=0;}
const audit=window.KELO_SURFACE_GROUND_AUDIT={version:'surface-ground-v1.1.0',mode:style.mode,ready:false,failed:false,asset:style.asset,src:atlas.src,tileCount:atlas.tileCount,baseFrameCount:BASE.length,detailFrameCount:DETAIL.length,detailModulo:MOD,worldTileSize:TILE,chunkSize:CHUNK,chunkCacheCap:MAX,chunkCacheSize:0,chunkBuildCount:0,chunkEvictionCount:0,lastDrawnTiles:0,worldWidth:window.CONFIG?.worldWidth||3600,worldHeight:window.CONFIG?.worldHeight||3200,visibleDuringReset:true,capitalTestBoot:'v2',viewportOwner:'KeloCamera',chunkCacheMode:'lru-v1'};
