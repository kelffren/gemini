(function(){
  ...
  const layerGroups=Object.freeze({
    plazaNature:Object.freeze({
      id:'plaza-nature',ownership:'plaza-nature-props-v1',priority:10,renderMode:'layer-stack',
      back:Object.freeze({phase:'props_back'}),front:Object.freeze({phase:'props_front'})
    }),
    ruralBoundary:Object.freeze({
      id:'rural-boundary',ownership:'rural-farm-boundary-props-v1',priority:8,renderMode:'immediate',
      back:Object.freeze({phase:'props_back'})
    })
  });
  ...
  const assets=Object.freeze({
    ...
    ruralProps:Object.freeze({id:'ruralProps',src:ruralPropsAtlas?.src,width:ruralPropsAtlas?.width,height:ruralPropsAtlas?.height,
      frameWidth:ruralPropsAtlas?.tileWidth||TILE,frameHeight:ruralPropsAtlas?.tileHeight||TILE,columns:ruralPropsAtlas?.columns})
  });
  function ruralTile(frame,x,y,id,family){
    return Object.freeze({
      id, family:family||'rural_boundary_prop',asset:'ruralProps',frame,layerGroup:'ruralBoundary',
      position:Object.freeze({x,y}),size:Object.freeze({w:TILE,h:TILE}),anchor:Object.freeze({x:0,y:0}),
      visualBounds:Object.freeze({x,y,w:TILE,h:TILE}),footprint:Object.freeze({x,y:y+Math.round(TILE*0.65),w:TILE,h:Math.max(1,Math.round(TILE*0.35))}),
      collider:Object.freeze({mode:'none'}),layers:Object.freeze({back:'props_back',front:null}),priority:8,district:'rural',
      occlusion:Object.freeze({mode:'none'}),visualOnly:true
    });
  }
  function buildRuralFarmBoundary(farm){
    ...
    for(let x=left+TILE;x<=right-TILE;x+=TILE){
      if(Math.abs(x-gateX)>TILE/2)add(ruralFrames.FENCE_H,x,top,'fence');
      add(ruralFrames.FENCE_H,x,bottom,'fence');
    }
    for(let y=top+TILE;y<=bottom-TILE;y+=TILE){add(ruralFrames.FENCE_V,left,y,'fence');add(ruralFrames.FENCE_V,right,y,'fence');}
    add(ruralFrames.CORNER_LEFT,left,top,'corner');add(ruralFrames.CORNER_RIGHT,right-TILE,top,'corner');
    add(ruralFrames.CORNER_LEFT,left,bottom,'corner');add(ruralFrames.CORNER_RIGHT,right-TILE,bottom,'corner');
    add(ruralFrames.GATE_OPEN,gateX,top,'gate');add(ruralFrames.FIELD_SIGN,left+TILE,top+TILE,'sign');
    add(ruralFrames.WEED_A,left-TILE,top+2*TILE,'vegetation');add(ruralFrames.STONE_A,right+6,top+5*TILE,'stone');
    add(ruralFrames.WEED_B,right+4,bottom-2*TILE,'vegetation');
    return Object.freeze(out);
  }
  const sources=Object.freeze({
    ruralFarmBoundary:Object.freeze({id:'ruralFarmBoundary',layerGroup:'ruralBoundary',build:buildRuralFarmBoundary})
  });
  window.KELO_PROP_CONTRACT=Object.freeze({
    version:'1.2.0',mode:'generic-prop-contract-v2',assets,layerGroups,props:Object.freeze(defs),sources,
