(function(){
  ...
  function depthRootY(actor){
    try{
      const P=window.KELO_AVATAR_PRESENTATION;
      if(P&&typeof P.get==='function'){
        const d=P.get(actor,(actor&&actor._face)||'down');
        if(d&&Number.isFinite(d.depthRootY))return d.depthRootY;
        if(d&&Number.isFinite(d.footRootY))return d.footRootY;
      }
    }catch(_e){}
    return Number(actor&&actor.y)||0;
  }
  ...
  function drawBack(groupKey,g){if(failed)return;backDrawCountByGroup[groupKey]=drawInstances(g,propsForRole(groupKey,'back'),false);}
  function drawFront(groupKey,g){
    ...
    for(const p of props){
      if(p.occlusion?.mode==='actor-base-y-clip-v1'){
        for(const actor of as){if(!overlaps(actor,p.visualBounds)||depthRootY(actor)>=p.occlusion.baseY)continue;const pad=p.occlusion.clipPadding||8,r=Math.max(22,(actor.radius||20)+pad);g.save();g.beginPath();g.rect(actor.x-r,actor.y-r*1.8,r*2,r*2.5);g.clip();if(drawProp(g,p))count++;g.restore();}
        ...
        for(const actor of as){if(!overlaps(actor,bounds)||depthRootY(actor)<=p.occlusion.baseY)continue;renderAvatar(actor,typeof localPlayer!=='undefined'&&actor===localPlayer);actorRedraws++;}
      }
    }
    g.restore();frontDrawCountByGroup[groupKey]=count;actorRedrawCountByGroup[groupKey]=actorRedraws;
  }
  function boundsFor(groupKey,role){return()=>propsForRole(groupKey,role).map(p=>({id:p.id,...p.visualBounds}));}
  ...
  try{for(const [key,group] of stackedGroups){if(group.back)L.register({id:`${group.id}-back`,phase:group.back.phase,priority:group.priority,required:true,ready:()=>audit.ready,draw:g=>drawBack(key,g),ownership:group.ownership,bounds:boundsFor(key,'back')});if(group.front)L.register({id:`${group.id}-front`,phase:group.front.phase,priority:group.priority,required:true,ready:()=>audit.ready,draw:g=>drawFront(key,g),ownership:group.ownership,bounds:boundsFor(key,'front')});}}catch(err){failed=true;audit.failed=true;console.error('[Kelo generic props] layer registration failed',err);return;}
  registerStaticColliders();
  window.KELO_GENERIC_PROPS=Object.freeze({version:'generic-props-v1.6',resourceMode:'atlas-contract-managed-v1',drawInstances,isAssetReady(id){return readyAssets.has(id);},get ready(){return audit.ready&&!audit.failed;}});
