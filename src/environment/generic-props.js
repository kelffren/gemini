(function(){
  ...
  function drawBack(groupKey,g){if(failed)return;backDrawCountByGroup[groupKey]=drawInstances(g,propsFor(groupKey),false);}
  function drawFront(groupKey,g){
    if(failed)return;
    const as=actors();if(!as.length){frontDrawCountByGroup[groupKey]=0;return;}
    let count=0;g.save();g.imageSmoothingEnabled=false;
    for(const p of propsFor(groupKey)){
    ...
    g.restore();frontDrawCountByGroup[groupKey]=count;
  }
