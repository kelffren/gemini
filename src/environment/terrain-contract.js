(function(){
  const DVP=window.KELO_DISTRICT_VISUAL_PROFILES;
  if(!DVP?.profiles||!DVP?.districts){console.error('[Kelo terrain] district visual profiles missing');return;}
  ...
  function districtVisualProfileFor(id){return DVP.get(id);}
  function profileFor(id){const terrainId=DVP.terrainProfileFor(id);return profiles[terrainId]||profiles.default;}
  ...
  window.KELO_TERRAIN_CONTRACT=Object.freeze({
    version:'1.4.0', tileSize:32, topology:'edge-bitmask-4-v1',
    defaults:Object.freeze({baseTerrain:'grass',pathTerrain:'marble'}),
    sideBits:SIDES, materials, transitions, profiles,
    districtVisualProfiles:DVP.profiles, districtOrder:DVP.order, districts:DVP.districts,
    districtVisualProfileVersion:DVP.version, requiredAtlasKeys,
