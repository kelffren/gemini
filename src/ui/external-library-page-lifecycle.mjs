...
function flash(text){const toast=$('toast');if(!toast)return;toast.textContent=String(text||'');toast.classList.add('on');clearTimeout(flash.timer);flash.timer=setTimeout(()=>toast.classList.remove('on'),1800);}
function currentPage(){const text=$('pager')?.querySelector('.page-status b')?.textContent||'';const match=text.match(/(?:Página|Page)\s+(\d+)/i);return match?Math.max(1,Number(match[1])||1):1;}
