...
function buildPackButton(){return menuButton(PACK_ID,'Packs','Instalar grupos · bajo demanda','📦',openPacks,'Abrir Content Packs');}
...
function ensureInMenu(){const grid=document.getElementById('lx-menu-grid');if(!grid)return false;[SPRITE_ID,PACK_ID,COMMUNITY_ID].forEach(id=>{const old=document.getElementById(id);if(old)old.remove();});ensureButton(grid,ID,buildButton,true);if(!gridObserver){gridObserver=new MutationObserver(()=>{const button=document.getElementById(ID);if(!button||button.parentNode!==grid||[SPRITE_ID,PACK_ID,COMMUNITY_ID].some(id=>document.getElementById(id)))queueMicrotask(ensureInMenu);});gridObserver.observe(grid,{childList:true});}return true;}
