 ...
 * keys: CREATORS MENU PREMIUM LAZY ADMIN ANIMATION CAPABILITY AVATAR SPRITE FACTORY ASSET FORGE ONLINE PERMISSIONS DIRECT URL GM LIVE
 * purpose: añade CREATORS, Sprite Factory y Asset Forge al menú Luxe, carga permisos online y permite abrir Creators directamente con ?creators=1
 ...
 * extension-points: Creator Hub / Sprite Factory / Asset Forge / WorkspaceRegistry / Account Admin / Account Live Control
 */
(function(){
  'use strict';
  ...
  function paintForge(button,busy){
    if(!button)return;
    button.innerHTML='<span class="lx-menu-icon" aria-hidden="true">✎</span><span class="lx-menu-copy"><b>'+(busy?'Abriendo…':'Asset Forge')+'</b><small>'+(busy?'Cargando editor':'Dibujar · reparar · mercado')+'</small></span>';
    button.disabled=!!busy;
    if(busy)button.setAttribute('aria-busy','true');else button.removeAttribute('aria-busy');
  }
  ...
  async function openForge(){
    ...
    try{
      window.KELO_LUXE?.closeMenu?.();
      const mod=await import(`./../creators/creator-entry.mjs?v=${CREATOR_BUILD}`);
      const platform=await mod.bootKeloCreators({root:window});
      await platform.openWorkspace('asset-forge');
    }catch(e){
      console.error('[Kelo Asset Forge launcher]',e);toast(friendlyError(e));
    }finally{
      forgeLoading=false;if(btn?.isConnected)paintForge(btn,false);
    }
  }
  function sync(){
  ...
  const api=Object.freeze({version:'studio-launcher-v1.20.2-world-handoff',open,openSpriteFactory:openFactory,openAssetForge:openForge,sync,get allowed(){return allowed();},get directRequested(){return directRequested();}});
