 ...
 * keys: GUARDIAN PVP WORKER ISOLATION FIXED STEP STATUS EFFECTS AUTHORITY SNAPSHOT TAKEOVER
...
const VERSION='guardian-pvp-authority-worker-v2-takeover';
const MAX_PLAYERS=16,MAX_EVENTS=32,MAX_PROJECTILES=64,SNAPSHOT_MS=50,TAKEOVER_COMBO_GRACE_MS=250;
let roomId=null,epoch=0,authority=null,snapshotSeq=0,lastSnapshotAt=0,disposed=false,tickOffset=0,takeoverFromEpoch=0,takeoverSnapshotSeq=0;
...
function restoreSeed(seed,now){
  ...
  let restored=0;for(const p of Object.values(seed.players||{}).slice(0,MAX_PLAYERS))if(restorePlayer(p,now))restored++;
...
function init(msg){
  ...
  roomId=short(msg.roomId,64);epoch=Number(msg.epoch)||0;snapshotSeq=0;lastSnapshotAt=0;tickOffset=0;takeoverFromEpoch=0;takeoverSnapshotSeq=0;playersByNode.clear();disposed=false;
  ...
  post('ready',{roomId,epoch,fixedDt:self.KeloSharedPvPAuthority.FIXED_DT,snapshotHz:self.KeloSharedPvPAuthority.SNAPSHOT_HZ,takeoverRestored:takeover.restored>0,playersRestored:takeover.restored,projectilesReset:takeover.projectilesReset,takeoverFromEpoch,takeoverSnapshotSeq,tickOffset});
}
function ingest(msg){
...
function snapshot(now){
  ...
  post('snapshot',{roomId,epoch,seq:++snapshotSeq,serverTick:tickOffset+Math.max(0,Number(snap.serverTick)||0),serverTime:snap.serverTime,fixedDt:snap.fixedDt,maxRewindMs:snap.maxRewindMs,inputBufferMs:snap.inputBufferMs,movementProfileVersion:snap.movementProfileVersion,players,projectiles:Array.isArray(snap.projectiles)?snap.projectiles.slice(0,MAX_PROJECTILES):[],events,takeoverFromEpoch,takeoverSnapshotSeq});
}
function step(msg){
  if(disposed||!authority||String(msg.roomId)!==roomId||Number(msg.epoch)!==epoch)return;
...
function dispose(){disposed=true;try{authority?.dispose?.();}catch(_){}authority=null;playersByNode.clear();post('disposed',{roomId,epoch});roomId=null;epoch=0;tickOffset=0;takeoverFromEpoch=0;takeoverSnapshotSeq=0;}
...
post('boot',{capabilities:{isolatedRealm:true,noOwnLoop:true,takeoverRestore:true,transientActionsReset:true,projectilesReset:true,persistentAuthority:false,economyAuthority:false,inventoryAuthority:false,rewardsAuthority:false}});
