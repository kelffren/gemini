/* KELO-INDEX
 * area: INSTANCES
 * owner: Instance runtime for scene switching; camera state owned by KeloCamera
 * keys: SCENE RUNTIME HOUSE WORLD CAMERA BOUNDS FOUNDATION
 * purpose: cambia el runtime visual entre mundo e instancia sin cambiar el loop principal y delega cámara al owner
 * public-api: KELO_INSTANCE_RUNTIME
 * consumes: KeloCamera, KELO_ENVIRONMENT_LAYERS, scene/property context
 * state-owned: snapshot local de jugador/cámara previo a entrar y clamp de instancia
 * extension-points: KeloCamera.setTarget/restoreState
 * reuse: transición mundo↔instancia sin crear cámaras paralelas
 * legacy: clamp usa timer hasta migración a simulation owner
 * do-not: NO escribir camera.x/y/targetX/targetY directamente
 */
(function(){
  'use strict';
  ...
  function centerOn(x,y){if(typeof localPlayer!=='undefined'&&localPlayer){localPlayer.x=x;localPlayer.y=y;}cameraOwner.setTarget(x,y,{snap:true,source:'instance-runtime:center'});}
  ...
  async function leave(){stopClamp();document.body.classList.remove('kelo-house-instance');if(worldState){if(worldState.player&&typeof localPlayer!=='undefined'&&localPlayer){localPlayer.x=worldState.player.x;localPlayer.y=worldState.player.y;}if(worldState.camera)cameraOwner.restoreState(worldState.camera,{source:'instance-runtime:leave'});}worldState=null;try{window.KELO_PROPERTY_SYSTEM?.refreshSceneColliders?.();}catch(e){}return true;}
  ...
  window.KELO_INSTANCE_RUNTIME=Object.freeze({version:'instance-runtime-bridge-v1.1-camera-owner',enter,leave,get worldState(){return clone(worldState);}});
})();