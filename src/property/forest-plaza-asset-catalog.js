 ...
 * keys: FOREST PLAZA ASSET SHEET PLACEABLE WORLD EDITOR IRREGULAR ATLAS
...
(function(){
  ...
  if(!M?.atlas||!Array.isArray(M.assets)||!A?.register||!C?.registerTemplate){console.warn('[Kelo forest plaza catalog] compiler manifest or owners missing');return;}
  ...
  try{A.register(ATLAS,{id:ATLAS,src:SRC,width:Number(M.atlas.width)||1448,height:Number(M.atlas.height)||1086,frameMode:'irregular',frames:atlasFrames},{role:'optional'});}catch(err){console.warn('[Kelo forest plaza catalog] atlas already registered or unavailable',err?.message||err);}
  const ids=[];
  for(const frame of M.assets){
    ...
    const template=C.registerTemplate({
      id:`forest-plaza:${String(frame.assetId||key)}`,
  ...
  window.KELO_FOREST_PLAZA_CATALOG_AUDIT=Object.freeze({version:'forest-plaza-catalog-v1',atlasKey:ATLAS,compiler:M.compiler||null,templateCount:ids.length,templateIds:Object.freeze(ids),placeable:true});
