/* KELO-INDEX
 * area: PROPERTY
 * keys: PARCEL ASSET CATALOG TEMPLATE PREFAB PROP UNITS
 * hace: convierte props/prefabs registrados en plantillas reutilizables para parcelas/editor
...
(function(){
  ...
  function frameRect(a,frame){
    ...
    const i=Math.max(0,Math.floor(Number(frame)||0)),cols=Math.max(1,Number(a?.columns)||1),w=Number(a?.frameWidth)||TILE,h=Number(a?.frameHeight)||TILE;
  ...
  // KELO-INDEX PROPERTY/CATALOG deduplica cada frame de prop para venderlo por unidad sin copiar instancias del mundo.
  function registerPropTemplates(){
    ...
    if(rural?.src){
      const cols=Math.max(1,Number(rural.columns)||4),rows=Math.max(1,Math.ceil((Number(rural.height)||128)/(Number(rural.frameHeight)||TILE))),count=cols*rows;
  ...
  // KELO-INDEX PROPERTY/CATALOG conserva el renderPlan de cada prefab para poder recolocarlo sin duplicar lógica gráfica.
