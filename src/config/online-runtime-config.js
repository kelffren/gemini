 ...
 * keys: ONLINE ENDPOINT WSS CONFIG QA OFFLINE SUPABASE AUTH ACCOUNT LOGIN BOOT
...
const AUTH_RUNTIME_URL='src/auth/supabase-auth-runtime.js?v=3';
...
window.KELO_ONLINE_RUNTIME_CONFIG=Object.freeze({version:'kelo-online-runtime-config-v3',defaultWsUrl:DEFAULT_WS_URL,mode,forceOffline,effectiveNet:new URLSearchParams(location.search).get('net')||null,authBootstrap:'supabase-pinned-2.116.0',accountGate:true});
// Este archivo se ejecuta durante el parseo y está colocado inmediatamente antes de engine-net.js.
// document.write mantiene orden estricto: SDK -> Auth -> Account UI -> KeloNetAuthority, sin abrir otro socket.
if(!forceOffline&&!window.KeloOnlineAuth&&document.readyState==='loading'){
  document.write('<script src="'+SUPABASE_JS_URL+'"><\\/script><script src="'+AUTH_RUNTIME_URL+'"><\\/script><script src="'+ACCOUNT_UI_URL+'"><\\/script>');
}
})();
