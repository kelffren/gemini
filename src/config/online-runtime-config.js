...
const AUTH_RUNTIME_URL='src/auth/supabase-auth-runtime.js?v=4&boot=5';
const ACCOUNT_UI_URL='src/ui/account-auth-ui.js?v=1&boot=5';
const GOOGLE_AUTH_UI_URL='src/ui/google-auth-button.js?v=1&boot=5';
...
window.KELO_ONLINE_RUNTIME_CONFIG=Object.freeze({version:'kelo-online-runtime-config-v5',defaultWsUrl:DEFAULT_WS_URL,mode,forceOffline,effectiveNet:new URLSearchParams(location.search).get('net')||null,authBootstrap:'supabase-pinned-2.116.0',accountGate:true,googleAuth:true});
...
if(!forceOffline&&!window.KeloOnlineAuth&&document.readyState==='loading'){
  document.write(scriptTag(SUPABASE_JS_URL)+scriptTag(AUTH_RUNTIME_URL)+scriptTag(ACCOUNT_UI_URL)+scriptTag(GOOGLE_AUTH_UI_URL));
}
})();
