...
self.onmessage = event => {
  ...
  if (!id) return;
  try {
    if (message.type !== 'generate') throw new Error(`MAP_FORGE_WORKER_MESSAGE_UNSUPPORTED:${message.type || 'unknown'}`);
    const recipe = MAP_FORGE_RECIPES[String(message.recipeId || '')];
    if (!recipe) throw new Error(`MAP_FORGE_RECIPE_NOT_FOUND:${message.recipeId || ''}`);
    const result = generateBestOf(recipe, message.options || {});
    self.postMessage({ id, ok: true, result });
  } catch (error) {
    self.postMessage({ id, ok: false, error: String(error?.message || error || 'MAP_FORGE_WORKER_FAILED') });
  }
};
