...
function materializeExitGateways(parts,{worldBounds}={}){
  ...
  for(const exit of parts.exits||[]){
    const declaredDistrict=(parts.districts||[]).find(row=>row.id===exit.district),frame=exitRoadFrame(parts,exit);if(!declaredDistrict||!frame)continue;
    const anchor=exitAnchor(exit,frame),boundaryDistrictId=districtOwnerAt(parts,anchor),boundaryDistrict=(parts.districts||[]).find(row=>row.id===boundaryDistrictId),gatewayDistricts=[declaredDistrict,...boundaryDistrict&&boundaryDistrict.id!==declaredDistrict.id?[boundaryDistrict]:[]];let placement=null;
    pairSearch:for(const gatewayDistrict of gatewayDistricts)for(const pair of PAIRS[gatewayDistrict.kind]||PAIRS.default){
      const lateral=Math.max(pair.lateral,112),leftTarget=targetPoint(anchor,frame,0,-lateral),rightTarget=targetPoint(anchor,frame,0,lateral),used=new Set(),left=chooseSource(rows,gatewayDistrict.id,pair.family,leftTarget,used);if(!left)continue;used.add(left.index);const right=chooseSource(rows,gatewayDistrict.id,pair.family,rightTarget,used);if(!right)continue;
      const originalLeft={...rows[left.index]},originalRight={...rows[right.index]},leftAccepted=acceptedPoint(parts,rows,left.index,leftTarget,worldBounds);if(!leftAccepted)continue;rows[left.index].x=leftAccepted.x;rows[left.index].y=leftAccepted.y;const rightAccepted=acceptedPoint(parts,rows,right.index,rightTarget,worldBounds);if(!rightAccepted){rows[left.index]=originalLeft;rows[right.index]=originalRight;continue;}rows[right.index].x=rightAccepted.x;rows[right.index].y=rightAccepted.y;placement={pair,left,right,leftTarget,rightTarget,originalLeft,originalRight};break pairSearch;
    }
