...
const STREET_EDGE_NUDGE_THRESHOLD=120;
const STREET_EDGE_NUDGE_DISTANCE=12;
...
function tightenUrbanStreetscape(parts){let nudged=0;const decorations=(parts.decorations||[]).map(row=>{if(!STREET_EDGE_FAMILIES.has(row.family))return row;const frame=nearestRoadVector(row,parts.roads);if(!frame||frame.distance<=STREET_EDGE_NUDGE_THRESHOLD)return row;const shift=Math.min(STREET_EDGE_NUDGE_DISTANCE,frame.distance-STREET_EDGE_NUDGE_THRESHOLD+STREET_EDGE_NUDGE_DISTANCE),nx=frame.vx/frame.distance,ny=frame.vy/frame.distance;nudged++;return{...row,x:Math.round((row.x+nx*shift)*10)/10,y:Math.round((row.y+ny*shift)*10)/10};});return{...parts,decorations,generationStats:{...parts.generationStats,decorationStreetEdgeNudgeCount:nudged}};}

export function generateMapCandidate(recipe,{seed=1,assetCatalogVersion='catalog-unbound',style={},constraints={}}={}){const intent=createMapIntent(recipe,{seed,assetCatalogVersion,style,constraints}),rawParts=buildCandidateParts(recipe,intent,createRng(intent.seed,'map-forge')),declustered=declusterDecorationFamilies(rawParts),oriented=orientDirectionalDecorations(declustered),parts=tightenUrbanStreetscape(oriented),base={metadata:{mapId:`map:${recipe.id}:${intent.seed}`,seed:intent.seed,generatorVersion:MAP_FORGE_GENERATOR_VERSION,recipeId:recipe.id,recipeVersion:recipe.version,assetCatalogVersion:intent.assetCatalogVersion,layoutHash:null},worldBounds:{...intent.worldBounds},...parts};const hashPayload={...base,metadata:{...base.metadata,layoutHash:null}};base.metadata.layoutHash=hashString(stableStringify(hashPayload));base.validation=validateMapDefinition(base,recipe);base.quality=scoreMapDefinition(base,recipe,base.validation);return freezeDeep(base);}
