...
const STREET_MARKET_ROAD_GAIN=48;
...
function organizeStreetFurnitureFamilies(parts){
  ...
  for(const district of districts){
    ...
    const marketBest=findRoadAffinitySwap(rows,parts.roads,district,'market_prop','flower',STREET_MARKET_ROAD_GAIN);if(marketBest){swapDecorationIdentity(rows[marketBest.i],rows[marketBest.j]);marketSwaps++;marketRoadGain+=marketBest.gain;}
  }
