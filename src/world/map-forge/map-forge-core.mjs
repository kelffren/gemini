...
const MARKET_PROP_ROAD_GAIN=24;
...
function organizeStreetFurnitureFamilies(parts){
  ...
  for(const district of districts){
    ...
    if(district==='commerce'){
      const marketBest=findRoadAffinitySwap(rows,parts.roads,district,'market_prop','bench',MARKET_PROP_ROAD_GAIN);if(marketBest){swapDecorationIdentity(rows[marketBest.i],rows[marketBest.j]);marketSwaps++;marketRoadGain+=marketBest.gain;}
    }
  }
