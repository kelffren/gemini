 ...
 * keys: SUPABASE SESSION JWT CHARACTER ACCOUNT LOGIN REGISTER GOOGLE OAUTH GUEST MAGIC LINK PASSWORD WEBSOCKET HELLO
...
function preferredCharacterName(user){
  ...
  return cleanDisplayName(meta.display_name)||cleanDisplayName(meta.full_name)||cleanDisplayName(meta.name)||cleanDisplayName(stored(PLAYER_NAME_KEY))||'Kelo';
}
...
async function ensureCharacter(session){
  ...
  const accountId=String(verified.data.user.id).toLowerCase(),initialName=preferredCharacterName(verified.data.user);
...
async function protectGuestWithEmail(email,displayName){
  ...
  store(PLAYER_NAME_KEY,name);
  const result=await client.auth.updateUser({email:normalized,data:{display_name:name}},{emailRedirectTo:appRedirectUrl()});if(result.error)throw result.error;
