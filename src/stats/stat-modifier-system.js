/* KELO-INDEX
 * area: STATS / ATTRIBUTES
 * owner: KeloStatModifiers
 * purpose: resolver compartido y determinista de modificadores de atributos para player/mount sin recalcular por frame
 * public-api: KeloStatModifiers.registerSource/unregisterSource/resolve/markDirty/validateModifier
 * consumes: providers de equipo, monturas, buffs y futuros owners
 * state-owned: registry de providers + revision/cache; NO posee inventario, progreso ni equipment slots
 * extension-points: source providers + scopes + operations
 * online: authority futura entrega sources/modifiers validados; resolver puede permanecer cliente para preview
 * distinction: KeloPlayerStats posee counters de progreso; KeloStatModifiers resuelve atributos derivados de gameplay
 * do-not: no escribir STATE/equipment; no registrar progreso/kills; no meter reglas por itemId; no calcular desde DOM
 */
(function(root,factory){
  const api=factory();
  if(root)root.KeloStatModifiers=api;
  if(typeof module==='object'&&module.exports)module.exports=api;
})(typeof globalThis!=='undefined'?globalThis:this,function(){
  'use strict';
  const VERSION='stat-modifier-v1.1.0';
  ...
  function registerSource(id,provider){
    id=String(id||'');if(!id||typeof provider!=='function')throw new Error('KELO_STAT_MODIFIER_SOURCE_INVALID');
  ...
  function collect(target,targetId,context){
    ...
    for(const [sourceId,provider] of sources){
      let rows=[];try{rows=provider({target,targetId,context:context||{}})||[];}catch(error){console.error('[KeloStatModifiers source]',sourceId,error);continue;}
