 ...
 * security: when bucket/path are present, publicUrl is derived from the canonical Supabase project and never trusted from payload/network input
 */
import { KELO_SUPABASE_URL } from '../online/kelo-supabase-public-config.mjs';
...
function encodeStoragePath(path){return String(path||'').split('/').filter(Boolean).map(encodeURIComponent).join('/');}
function canonicalRuntimeUrl(rt){const bucket=String(rt?.bucket||''),path=String(rt?.path||'');if(bucket==='avatars'&&path)return `${String(KELO_SUPABASE_URL||'').replace(/\/+$/,'')}/storage/v1/object/public/avatars/${encodeStoragePath(path)}`;return String(rt?.publicUrl||'');}
function safeCommunityAssets(raw){
  if(!Array.isArray(raw))return Object.freeze([]);
  return Object.freeze(raw.slice(0,12).filter(item=>item&&typeof item==='object'&&item.id&&item.url).map(item=>Object.freeze({...item,renderProfile:item.renderProfile&&typeof item.renderProfile==='object'?Object.freeze({...item.renderProfile}):null})));
}
function safeManifest(raw){if(!raw||typeof raw!=='object')return null;const payload=raw.payload||raw,rt=payload.avatarRuntime||raw.avatarRuntime||((raw.publicUrl||raw.bucket||raw.path)?raw:null);if(!rt||!raw.contentId)return null;const publicUrl=canonicalRuntimeUrl(rt);if(!publicUrl)return null;const columns=Math.max(1,Number(rt.columns)||1),rows=Math.max(1,Number(rt.rows)||1),directionKeys=Array.isArray(rt.directionKeys)&&rt.directionKeys.length===rows?rt.directionKeys.map(directionKey):rows===8?[...DIRECTION_KEYS]:rows===4?['s','w','e','n']:['s'],rowMap={...FACE_ROWS,...(rt.rowMap||{})};directionKeys.forEach((key,row)=>{rowMap[key]=row;});if(Number.isInteger(rowMap.s))rowMap.down=rowMap.s;if(Number.isInteger(rowMap.n))rowMap.up=rowMap.n;if(Number.isInteger(rowMap.w))rowMap.left=rowMap.w;if(Number.isInteger(rowMap.e))rowMap.right=rowMap.e;const frameCounts=Array.isArray(rt.frameCounts)&&rt.frameCounts.length===rows?rt.frameCounts.map(value=>Math.max(1,Math.min(columns,Math.round(Number(value)||1)))):new Array(rows).fill(columns);return Object.freeze({contentId:String(raw.contentId),displayName:String(raw.displayName||'Avatar'),bucket:String(rt.bucket||''),path:String(rt.path||''),runtimeHash:String(rt.runtimeHash||''),publicUrl,columns,rows,directions:Math.max(1,Number(payload.directions)||directionKeys.length),directionKeys:Object.freeze(directionKeys),frameCounts:Object.freeze(frameCounts),rowMap:Object.freeze(rowMap),frameMs:Math.max(70,Number(rt.frameMs)||140),renderHeight:Math.max(44,Number(rt.renderHeight)||82),communityAssets:safeCommunityAssets(payload.communityAssets)});}
export function installCreatorAvatarRuntime({root=globalThis}={}){
  ...
  function dispatchSelfCommunity(manifest){
    const playerId=root.keloNet?.id;if(playerId==null)return false;
    try{root.dispatchEvent(new CustomEvent('kelo:community-player-assets',{detail:{playerId:String(playerId),assets:Array.isArray(manifest?.communityAssets)?manifest.communityAssets:[],visible:true,distance:0,profileOpen:false,source:'server-avatar-manifest-self'}}));return true;}catch{return false;}
  }
  function select(contentId,{manifest=null,persistLocal=true}={}){const row=manifest?register(manifest):records.get(String(contentId))||null;if(!row)throw new Error('AVATAR_MANIFEST_NOT_REGISTERED');selectedId=row.contentId;selectedManifest=row;if(persistLocal)persist(row);try{if(typeof localPlayer!=='undefined')localPlayer.avatarContentId=row.contentId;}catch{}dispatchSelfCommunity(row);return row;}
  ...
  const api=Object.freeze({version:'kelo-creator-avatar-runtime-v2.2.0-storage-canonical',register,select,current,get:id=>records.get(String(id))||null,list:()=>[...records.values()],clear(){selectedId=null;selectedManifest=null;persist(null);dispatchSelfCommunity(null);},get selectedId(){return selectedId;}});
  ...
  root.KELO_CREATOR_AVATAR_AUDIT=Object.freeze({version:api.version,owner:'KeloAvatar middleware',priority:900,localRestore:true,communitySelfStream:true,canonicalStorageUrls:true,middlewareId});
  return api;
}
