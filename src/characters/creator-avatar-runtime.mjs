 ...
 * owns: creator-avatar manifest cache, local selection and spritesheet draw middleware
...
function safeManifest(raw){if(!raw||typeof raw!=='object')return null;const payload=raw.payload||raw,rt=payload.avatarRuntime||raw.avatarRuntime||null;if(!rt?.publicUrl||!raw.contentId)return null;return Object.freeze({contentId:String(raw.contentId),displayName:String(raw.displayName||'Avatar'),publicUrl:String(rt.publicUrl),columns:Math.max(1,Number(rt.columns)||1),rows:Math.max(1,Number(rt.rows)||1),rowMap:Object.freeze({...FACE_ROWS,...(rt.rowMap||{})}),frameMs:Math.max(70,Number(rt.frameMs)||140),renderHeight:Math.max(44,Number(rt.renderHeight)||82)});}
export function installCreatorAvatarRuntime({root=globalThis}={}){
  ...
  function faceFor(actor){const vx=Number(actor?.vx)||0,vy=Number(actor?.vy)||0;if(Math.hypot(vx,vy)>2){if(Math.abs(vx)>Math.abs(vy))return vx<0?'left':'right';return vy<0?'up':'down';}const f=String(actor?._face||actor?.face||'down').toLowerCase();return ['up','down','left','right'].includes(f)?f:'down';}
  function draw(actor,isSelf,next){let manifest=null;if(isSelf)manifest=selectedManifest;else if(actor?.avatarManifest)manifest=register(actor.avatarManifest);if(!manifest)return next();const state=images.get(manifest.contentId);if(!state?.ready||!state.img?.naturalWidth)return next();
    const canvas=root.document?.getElementById('game-canvas'),ctx=canvas?.getContext?.('2d');if(!ctx)return next();const cols=manifest.columns,rows=manifest.rows,sw=state.img.naturalWidth/cols,sh=state.img.naturalHeight/rows,face=faceFor(actor),row=Math.max(0,Math.min(rows-1,Number(manifest.rowMap[face]??0))),moving=Math.hypot(Number(actor?.vx)||0,Number(actor?.vy)||0)>4,frame=moving?Math.floor((root.performance?.now?.()||Date.now())/manifest.frameMs)%cols:0,h=manifest.renderHeight,w=h*(sw/sh),x=(Number(actor?.x)||0)-w/2,y=(Number(actor?.y)||0)-h+Math.max(6,Number(actor?.radius)||20);
  ...
  const api=Object.freeze({version:'kelo-creator-avatar-runtime-v1.0.0',register,select,current,get:id=>records.get(String(id))||null,list:()=>[...records.values()],clear(){selectedId=null;selectedManifest=null;persist(null);},get selectedId(){return selectedId;}});
