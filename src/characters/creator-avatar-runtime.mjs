 ...
 * does-not-own: base renderer, gameplay, auth, upload, entitlement authority or character persistence
 * access: local/self use of revision-aware Creator avatars requires KeloCreatorEntitlements; remote published avatars remain renderable
 */
...
export function installCreatorAvatarRuntime({root=globalThis}={}){
  ...
  const api=Object.freeze({version:'kelo-creator-avatar-runtime-v2.1.0-entitlement',register,select,current,get:id=>{const row=records.get(String(id))||null;return row&&access(row)?row:null;},getRaw:id=>records.get(String(id))||null,list:()=>[...records.values()].filter(access),clear(){selectedId=null;selectedManifest=null;persist(null);},get selectedId(){return selectedManifest&&access(selectedManifest)?selectedId:null;}});
  ...
  root.KELO_CREATOR_AVATAR_AUDIT=Object.freeze({version:api.version,owner:'KeloAvatar middleware',priority:900,localRestore:true,localEntitlementCheck:true,remotePublishedRendering:true,middlewareId});
  return api;
}
