...
base + head
  -> Asset Bit Ratchet
     ...
     -> changed RGBA: classify as changed information
...
The existing Space Gate already searches increasingly capable lossless representations and verifies exact quality. The ratchet adds memory across repository history: once an unchanged asset reaches a smaller valid representation, a later PR cannot replace it with a larger representation while claiming equivalence.

This creates one-way pressure toward lower stored bytes for unchanged information.

Lossless data cannot be compressed indefinitely. Eventually an asset reaches a practical entropy/codec floor. At that point the ratchet holds the best known representation until a better codec, structure, deduplication strategy, or exact content model is proven.
