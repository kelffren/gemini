...
The exact merge result under test determines the base: CI uses the first parent of GitHub's merge commit, so concurrent `main` changes are not falsely attributed to the PR.
...
Proves a smaller stable payload passes, stable byte/request growth fails, new stable resources fail, unknown local byte length fails closed and one-run boundary traffic remains advisory.
...
CI reports static direct/dependency totals and observed stable/raw totals separately. This answers how much code is directly in boot, how much payload is statically reachable, how much is consistently transferred, which resources sit on the timing boundary and whether a PR made stable boot transfer worse.

## Online-first

N/A for gameplay authority. Future CDN/content-addressed delivery can change transport without changing the logical local resource comparison contract.
