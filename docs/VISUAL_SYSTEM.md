...
El Visual Lab (`?visualLab=1`, `visual-lab-v1.7.0`) tiene dos pestañas. **SKILL** reproduce un VisualProfile con `playCue` / eventos semánticos (`PLAY FULL`, cues honestas por familia incluyendo AREA/ARM/END, `TEST IN GAME` via `engine.predict` / `predictSource`). **PIEZAS** sigue siendo la galería de componentes sueltos y comparte Dirección/Escala/Velocidad/Loop. En móvil el panel se ancla abajo a la derecha y deja hueco al joystick. `PLAY` compacta a una barra con `■` y se reabre al terminar; `PIN` lo deja abierto. `LOOP` repite hasta STOP. Pad de 8 direcciones, filtro de familia, prev/next y `TAP SET` para colocar el dummy en el mundo. `CAM` encuadra el preview y `Seed lock` congela el seed visual. Un dummy de impacto marca el target y parpadea al IMPACT. `GHOST` pinta origen/rango/radio/telegraph con los mismos números de delivery del combate. El dropdown lista todo el catálogo y marca `MISSING`. `■` en la barra para el preview (usa `KeloFX.stopAll`).
