...
El first-playable path termina antes del control de módulos opcionales. Después de `kelo:boot-ready`, la cadena ligera es:

`KELO_FEATURE_REGISTRY → KELO_ASSET_REGISTRY → KELO_FUSEBOX → KELO_MODULE_LOADER`.
...
`KELO_FUSEBOX` no ejecuta features ni crea otro scheduler. Mantiene solamente health/circuit state de módulos opcionales conocidos. Core/unknown permanece fuera de su dominio para que una clasificación defectuosa no pueda apagar movimiento, cámara, canvas o colisión.
...
La Biblioteca de Assets conserva el kill switch manual de paquetes mediante `KELO_ASSET_REGISTRY`; FuseBox lo consulta, pero no duplica ese estado.
...
- health/circuit breaker opcional vía `KELO_FUSEBOX`;
...
La verificación final móvil sigue siendo dispositivo real/LIVE para features que lo requieren. El Main Stability Gate mantiene además el firewall obligatorio de boot + caminata sostenida de 8 s en contexto iPhone-sized, sin freeze/crash/black screen.
