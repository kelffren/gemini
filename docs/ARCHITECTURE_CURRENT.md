...
### 1. Boot / Account
`index.html` → estado persistido normalizado → legacy engine bootstrap → owners Foundation → módulos lazy.
...
### 2. Foundation Core
`src/core/` posee eventos, input locks, input, movement extension, player transition position, camera, avatar composition, render/simulation extension, update system y fronteras de migración legacy.
...
### 3. Legacy Core
`engine-a.js`, `engine-b.js` y `engine-c.js` todavía contienen estado/física/UI/render/gameplay histórico. No reciben capacidades nuevas cuando existe owner moderno.
