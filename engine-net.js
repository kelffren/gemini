 ...
 * keys: WEBSOCKET AUTHORITY POSE AOI CULL LOD VISUAL EVENT CAST PROJECTILE STATUS ONLINE TITLES PROGRESSION COMMERCE TRADE MARKET PERFORMANCE
 * purpose: transporte cliente para pose/sistemas autoritativos; pose change-driven + heartbeat y presentación de peers gobernada por KELO_PERF
 * public-api: KeloNetAuthority + performanceSnapshot
 * consumes: KeloSimulation, KeloRender, KELO_PERF, KeloVisualEventBus, commerce/title/nobility owners
...
(function () {
  ...
  let ws = null, myId = null, sendAcc = 0, heartbeatAcc = 0, lastPose = null;
  ...
  function connect() {
    ...
    ws.onopen = function () {
      ...
      lastPose = null; sendAcc = POSE_MIN_INTERVAL; heartbeatAcc = POSE_HEARTBEAT_INTERVAL;
      ws.send(JSON.stringify({ t: 'hello', name: (localPlayer && localPlayer.name) || 'Kelo', playerKey: window.keloNet.playerKey }));
  ...
  function ingest(map) {
    ...
    for (const id in map) { if (!Object.prototype.hasOwnProperty.call(map,id)) continue; live[id] = true; upsert(map[id]); }
    for (const id in peers) { if (Object.prototype.hasOwnProperty.call(peers,id) && !live[id]) delete peers[id]; }
    paintChip(window.keloNet.on ? 'on' : 'wait', countOnline());
  }

  function performanceSnapshot() {
    let peerCount = 0;
    for (const id in peers) if (Object.prototype.hasOwnProperty.call(peers,id)) peerCount += 1;
    return Object.freeze({
      version:perfAudit.version, online:!!window.keloNet.on, peerCount,
      stateSnapshotsReceived:perfAudit.stateSnapshotsReceived, poseSent:perfAudit.poseSent, poseSkippedUnchanged:perfAudit.poseSkippedUnchanged,
      peerUpdates:perfAudit.peerUpdates, peerUpdatesCulled:perfAudit.peerUpdatesCulled,
      peersRendered:perfAudit.peersRendered, peersRenderCulled:perfAudit.peersRenderCulled, lastPoseReason:perfAudit.lastPoseReason
    });
  }

  window.KeloNetAuthority = Object.freeze({
    version: 'net-authority-v5.1-performance',
  ...
  function currentNetworkZone() {
    try {
      const scene = window.KELO_SCENE_CONTEXT && window.KELO_SCENE_CONTEXT.current();
      if (scene && scene.zoneType === 'instance' && scene.instanceType) return scene.instanceType;
    } catch (e) {}
    return window.keloZone || 'plaza';
  }

  function currentPose() {
    return { x:Number(localPlayer.x)||0, y:Number(localPlayer.y)||0, face:localPlayer._face||'down', gait:localPlayer._gait||'idle', zone:currentNetworkZone() };
  }
  ...
  // KELO-INDEX NETWORK/POSE envía cambios a máximo 10 Hz y heartbeat idle; evita paquetes idénticos permanentes.
