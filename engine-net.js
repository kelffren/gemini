/* KELO-INDEX
 * area: NETWORK
 * keys: WEBSOCKET AUTHORITY POSE VISUAL EVENT CAST PROJECTILE STATUS ONLINE TITLES PROGRESSION
...
(function () {
  ...
  window.keloNet = { on: false, id: null, peers: {}, url: NET, playerKey: readPlayerKey(), nobilitySource: 'local-fallback', titleSource: 'local-fallback', forgeSource: 'local-fallback', visualEventSource: 'local-fallback' };
  ...
  function connect() {
    ...
    ws.onmessage = function (ev) {
      ...
      if (msg.t === 'welcome') { myId = msg.id; window.keloNet.id = myId; window.keloNet.nobilitySource = msg.nobilitySource || window.keloNet.nobilitySource; window.keloNet.titleSource = msg.titleSource || window.keloNet.titleSource; window.keloNet.forgeSource = msg.forgeSource || window.keloNet.forgeSource; ingest(msg.players); }
