 ...
 * purpose: replace suspended Render proxy with Supabase Edge -> Hugging Face Gradio/ZeroGPU
 * security: authenticated generation, server-side HF token only, public status, no paid fallback
 * runtime: Deno 2.1 compatible Supabase Edge Function
 */

const VERSION = 'kelo-sprite-edge-v1';
...
function statusPayload(){const c=config();return{ok:true,service:'kelo-sprite-ai',version:VERSION,transport:'supabase-edge->huggingface-zerogpu',renderIndependent:true,configured:Boolean(c.atlasUrl),realSkeletonReady:Boolean(c.poseUrl),authenticatedZeroGpu:Boolean(c.hfToken),paidFallback:false,pipelines:['atlas-v2','identity-skeleton-v3'],directions:8,framesPerDirection:4,providers:{atlas:{configured:Boolean(c.atlasUrl),space:c.atlasSpace||null,url:c.atlasUrl||null,apiName:c.atlasApi},pose:{configured:Boolean(c.poseUrl),space:c.poseSpace||null,url:c.poseUrl||null,apiName:c.poseApi}}};}
...
function plan(input:any,requestKey:string):{baseUrl:string,data:any[],context:JobContext}{
  ...
  if(realSkeleton){
    if(!c.poseUrl)throw error('SPRITE_AI_POSE_NOT_CONFIGURED',503,'Set KELO_SPRITE_AI_HF_POSE_SPACE or KELO_SPRITE_AI_HF_POSE_URL in Supabase secrets.');
    if(!source)throw error('SPRITE_AI_POSE_REFERENCE_REQUIRED',400);
    const apiName=c.poseApi;return{baseUrl:c.poseUrl,data:[stagePrompt(input),source,seed(input.seed),direction,action,JSON.stringify(input.poseTemplate||{}),short(input.styleHint,300),retry],context:{target:'pose',apiName,pipeline:'identity-skeleton-v3',stage:'direction',realSkeleton:true,sourceMode:'reference+pose',model:c.poseSpace||c.poseUrl,requestKey}};
  }
  if(!c.atlasUrl)throw error('SPRITE_AI_NOT_CONFIGURED',503,'Set KELO_SPRITE_AI_HF_SPACE or KELO_SPRITE_AI_HF_URL in Supabase secrets.');
  if(pipeline==='identity-skeleton-v3'||mode){
    const stage=mode||'master',apiName=c.v3Api;return{baseUrl:c.atlasUrl,data:[stage,subjectPrompt(input),source,seed(input.seed),retry,direction,action,JSON.stringify(input.poseTemplate||{}),String(input.targetFrameDataUrl||''),JSON.stringify(Array.isArray(input.repairRegions)?input.repairRegions:[])],context:{target:'atlas',apiName,pipeline:'identity-skeleton-v3',stage,realSkeleton:false,sourceMode:source?'reference-edit':'text-generation',model:c.atlasSpace||c.atlasUrl,requestKey}};
  }
  const apiName=c.atlasApi;return{baseUrl:c.atlasUrl,data:[subjectPrompt(input),source,seed(input.seed),retry],context:{target:'atlas',apiName,pipeline:'atlas-v2',stage:null,realSkeleton:false,sourceMode:source?'reference-edit':'text-generation',model:c.atlasSpace||c.atlasUrl,requestKey}};
}
function targetBase(target:Target){const c=config();return target==='pose'?c.poseUrl:c.atlasUrl;}
function hfHeaders(){const token=config().hfToken;const h:Record<string,string>={'content-type':'application/json','accept':'application/json'};if(token)h.authorization=`Bearer ${token}`;return h;}
...
function parseSse(text:string){
  ...
  for(const block of blocks){let event='',data='';for(const line of block.split('\n')){if(line.startsWith('event:'))event=line.slice(6).trim();else if(line.startsWith('data:'))data+=(data?'\n':'')+line.slice(5).trim();}
    if(event==='complete'&&data){try{return{done:true,data:JSON.parse(data)}}catch{return{done:true,data:[data]}}}
    if((event==='error'||event==='cancelled')&&data)failure=data;
  }
  if(failure)throw error('SPRITE_AI_HF_JOB_FAILED',502,short(failure,300));return{done:false,data:null};
}
...
function completePayload(context:JobContext,data:any[]){
  ...
  return{ok:true,pending:false,imageDataUrl,metadata,provider:'huggingface-zerogpu-via-supabase',model:context.model,sourceMode:context.sourceMode,pipeline:context.pipeline,stage:context.stage,realSkeleton:context.realSkeleton,poseConditioning:context.realSkeleton?'controlnet-openpose+ip-adapter':context.stage==='direction'?'semantic-keypoint-intent':null,renderIndependent:true,generatedAt:Date.now()};
}
function validContext(raw:any):JobContext{
  const c=config(),target:Target=raw?.target==='pose'?'pose':'atlas',apiName=cleanApiName(raw?.apiName,target==='pose'?c.poseApi:c.atlasApi);const allowed=target==='pose'?new Set([c.poseApi]):new Set([c.atlasApi,c.v3Api]);if(!allowed.has(apiName))throw error('SPRITE_AI_INVALID_JOB_CONTEXT',400);
  return{target,apiName,pipeline:raw?.pipeline==='identity-skeleton-v3'?'identity-skeleton-v3':'atlas-v2',stage:raw?.stage?short(raw.stage,24):null,realSkeleton:Boolean(raw?.realSkeleton),sourceMode:short(raw?.sourceMode,40)||'generated',model:short(raw?.model,200)||null,requestKey:/^[a-f0-9]{64}$/.test(String(raw?.requestKey||''))?String(raw.requestKey):''};
}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(op==='wait'){
      const eventId=String(body.eventId||'');if(!/^[A-Za-z0-9_-]{4,160}$/.test(eventId))throw error('SPRITE_AI_INVALID_EVENT_ID',400);const context=validContext(body.context),baseUrl=targetBase(context.target);if(!baseUrl)throw error('SPRITE_AI_NOT_CONFIGURED',503);
      const result=await waitResult(baseUrl,context.apiName,eventId);if(!result.done)return json({ok:true,pending:true,eventId,context},202,origin);const value=completePayload(context,result.data);if(context.requestKey)cacheSet(context.requestKey,value);return json(value,200,origin);
    }
    ...
    const p=plan(body,requestKey),eventId=await submit(p.baseUrl,p.context.apiName,p.data),result=await waitResult(p.baseUrl,p.context.apiName,eventId);
    if(!result.done)return json({ok:true,pending:true,eventId,context:p.context,provider:'huggingface-zerogpu-via-supabase'},202,origin);
    const value=completePayload(p.context,result.data);cacheSet(requestKey,value);return json({...value,cache:{hit:false,scope:'edge-isolate'}},200,origin);
  }catch(e:any){const status=Number(e?.status)||500,code=String(e?.code||e?.message||'SPRITE_AI_EDGE_ERROR'),detail=short(e?.detail||'',300);console.error('[kelo-sprite-generate]',code,detail);return json({ok:false,error:code,detail:detail||null,renderIndependent:true},status,origin);}
});
