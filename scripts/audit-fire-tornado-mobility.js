 ...
 * owner: audit only
 * keys: FIRE TORNADO MOVEMENT CAST FEEL 60HZ 90HZ 120HZ BASELINE
 * purpose: compara el movementScale LIVE de Fire Tornado contra el baseline .38 sin tocar gameplay
...
const SPEED=185.28,BASELINE=.38;
...
const out={ability:ability.key,durationSec:duration,scaleBefore:BASELINE,scaleAfter:candidate,continuous:{beforePx:continuous(BASELINE),afterPx:continuous(candidate),gainPx:continuous(candidate)-continuous(BASELINE),freePx:continuous(1)},fixed:[60,90,120].map(hz=>({hz,beforePx:fixed(hz,BASELINE),afterPx:fixed(hz,candidate),gainPx:fixed(hz,candidate)-fixed(hz,BASELINE)}))};
for(const row of out.fixed){if(row.gainPx<=0)throw new Error('NO_GAIN_'+row.hz);}
console.log('FIRE_TORNADO_MOBILITY_OK');
