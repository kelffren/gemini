 ...
 * keys: MOBILE LIVE UPLOAD IRREGULAR SPRITESHEET AUTO FIT PREVIEW DUMMY GENERATE ANIMATION ABILITY
...
try{
  ...
  if(initial.version!=='sprite-ability-builder-v1.1.0-auto-fit'||!initial.projectId||!initial.lockOwners.includes('kelo-sprite-ability-builder'))throw new Error(`SPRITE_ABILITY_BOOT_FAILED:${JSON.stringify(initial)}`);
  ...
  await waitBuilder(s=>s?.draft?.sheet?.fps===20&&s.draft.combat.impactFrame===7&&s.draft.combat.damage===37&&s.draft.combat.knockback===29,{label:'edited parameters'});
  ...
  const generated=await page.evaluate(async ids=>{const platform=(await import('./src/creators/creator-entry.mjs')).getKeloCreatorsPlatform(),rows=await platform.projects.list({});const animation=await platform.projects.loadDraft(ids.animationProjectId),ability=await platform.projects.loadDraft(ids.abilityProjectId);return{ids,rows:rows.map(p=>({id:p.projectId,type:p.type,name:p.name})),animation:{type:animation?.documentType,asset:animation?.assetSource?.dataUrl?.slice(0,22),fps:animation?.clip?.fps,impact:animation?.clip?.markers?.impact,width:animation?.assetSource?.width,height:animation?.assetSource?.height,autoFit:animation?.authoring?.autoFit},ability:{type:ability?.documentType,animationProjectId:ability?.links?.animationProjectId,damage:ability?.definition?.effects?.find(x=>x.type==='damage')?.amount,autoFit:ability?.definition?.authoring?.spriteAutoFit}};},settled.draft.generated);
  if(generated.animation.type!=='ANIMATION'||!generated.animation.asset?.startsWith('data:image/png;base64,')||generated.animation.fps!==20||generated.animation.width%8!==0||generated.animation.height%4!==0||generated.animation.autoFit?.sourceWidth!==1983||generated.ability.type!=='ABILITY'||generated.ability.animationProjectId!==generated.ids.animationProjectId||generated.ability.damage!==37||generated.ability.autoFit?.sourceHeight!==793)throw new Error(`SPRITE_ABILITY_GENERATION_FAILED:${JSON.stringify(generated)}`);
  ...
  const report={ok:true,viewport:'390x844',initial,autoFit:{source:`${sheet.autoFit.sourceWidth}x${sheet.autoFit.sourceHeight}`,detected:`${sheet.columns}x${sheet.rows}`,normalized:`${sheet.imageWidth}x${sheet.imageHeight}`,cell:`${sheet.frameWidth}x${sheet.frameHeight}`,confidence:sheet.autoFit.confidence},beforeGenerate:{fps:beforeGenerate.sheet.fps,impact:beforeGenerate.combat.impactFrame,damage:beforeGenerate.combat.damage,knockback:beforeGenerate.combat.knockback,mode:beforeGenerate.preview.mode},generated,locksAfterClose:locks,pageErrors};fs.writeFileSync('artifacts/sprite-ability-builder/report.json',JSON.stringify(report,null,2));console.log('KELO SPRITE ABILITY BUILDER AUTO-FIT MOBILE AUDIT: PASS');console.log(JSON.stringify(report,null,2));
