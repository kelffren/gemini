...
assert.match(perf,/id: 'pvp_low'/,'performance owner must define pvp_low');
...
assert.match(registry,/src\/systems\/pvp-auto-reducer\.js\?v=2-quality-floor/,'reducer must be lazy-loaded with PvP feature pack');
const pvpIndex=registry.indexOf("pvp:{dependencies:");
const reducerIndex=registry.indexOf("src/systems/pvp-auto-reducer.js?v=2-quality-floor");
...
owner.setQualityFloor('medium');
assert.equal(owner.profile.id,'performance','a floor above current quality must never upgrade the phone');
owner.setQualityFloor('pvp_low');
