 ...
 * owns: validation of migration modes against static ownership evidence
 * does-not-own: runtime activation or gameplay behavior
 * purpose: block unsafe NEW activation while legacy conflicts remain
...
const conflicts=new Map((report.conflicts||[]).map(c=>[c.key,c.writers]));
  ...
  if(cfg.mode==='NEW'){
    for(const key of cfg.contracts||[]){
      ...
      if(legacyWriters.length){
        fail(`${domain}: NEW blocked; legacy still writes ${key}: ${legacyWriters.map(w=>w.file).join(', ')}`);
      }
      if(writers.length>1){
        fail(`${domain}: NEW blocked; ${key} still has ${writers.length} detected writers`);
      }
    }
  }
  if(cfg.mode==='SHADOW') ok(`${domain}: SHADOW allowed; legacy retains authority while parity is measured`);
  if(cfg.mode==='LEGACY') ok(`${domain}: LEGACY retained`);
  if(cfg.mode==='NEW') ok(`${domain}: NEW static ownership gate passed`);
}
...
ok('manifest is safe for current static ownership evidence');
