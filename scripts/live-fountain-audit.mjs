...
async function capture(name,y){
  const result=await page.evaluate(({y})=>{
    ...
    let water=0,basin=0,gold=0,ivory=0;
    for(let i=0;i<px.length;i+=4){const r=px[i],gg=px[i+1],b=px[i+2],a=px[i+3];if(a<200)continue;if(r<95&&gg>135&&b>145&&b>r*1.5)water++;if(r<100&&gg<150&&b>100&&b>gg)basin++;if(r>175&&gg>100&&gg<215&&b<105)gold++;if(r>210&&gg>195&&b>155)ivory++;}
    return{dataUrl:c.toDataURL('image/png'),audit:{...f},localY:localPlayer.y,roi:{x0,y0,x1,y1,width:x1-x0,height:y1-y0},water,basin,gold,ivory};
  },{y});
  fs.writeFileSync(`artifacts/${name}.png`,Buffer.from(result.dataUrl.split(',')[1],'base64'));
  delete result.dataUrl;return result;
}

// 1570 is visually behind the lower rim but sits outside the collision rectangle (bottom=1552).
...
if(behind.localY!==1570||behind.audit?.lastLocalDepth!=='behind-front-layer'||behind.audit?.lastDepthCandidates<1)throw new Error(`Behind depth failed: ${JSON.stringify(behind)}`);
if(front.localY!==1620||front.audit?.lastLocalDepth!=='in-front-of-front-layer'||front.audit?.lastFrontActorRedraws<1)throw new Error(`Front depth failed: ${JSON.stringify(front)}`);
