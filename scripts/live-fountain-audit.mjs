...
async function capture(name,y){
  const result=await page.evaluate(({y})=>{
    ...
    return{dataUrl:c.toDataURL('image/png'),audit:{...f},localY:localPlayer.y,expectedLocalDepth:y>f.baseY?'in-front-of-front-layer':'behind-front-layer',roi:{x0,y0,x1,y1,width:x1-x0,height:y1-y0},basin,gold,ivory};
  },{y});
...
if(behind.audit?.backDrawCount<1||behind.audit?.frontDrawCount<1||front.audit?.backDrawCount<1||front.audit?.frontDrawCount<1)throw new Error('Fountain formal back/front passes did not execute');
if(state.fountain?.lastFrontActorRedraws<1)throw new Error('Post-actor fountain pass did not redraw any front-side actor');
