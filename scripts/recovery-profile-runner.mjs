 ...
 * keys: RECOVERY PROFILE PLAYWRIGHT BOOT MOVEMENT WORLD FREEZE EVIDENCE EXIT-CODE
 * purpose: clasifica un checkout como PASS/FAIL/SKIP con perfiles deterministas reutilizables por CI y git bisect
...
const report={
  schema:1,profile,base,startedAt:new Date().toISOString(),gitHead:null,result:'RUNNING',reason:null,
  steps:[],console:[],pageErrors:[],crashed:false,recovery:null
};
...
const save=()=>fs.writeFileSync(path.join(artifactDir,`profile-${profile}-${String(report.gitHead||'unknown').slice(0,12)}.json`),JSON.stringify(report,null,2));
...
async function worldCheck({navigate=true}={}){
  if(navigate){
    const url=targetUrl({creators:'1',recoveryFlow:'world-open',bug:'BUG-0003'});
...
try{
  ...
  try{if(page&&!page.isClosed())await page.screenshot({path:path.join(artifactDir,`failure-${profile}-${String(report.gitHead||'unknown').slice(0,12)}.png`),fullPage:true});}catch{}
}finally{
