...
const shell=await readFile(new URL('../src/studio/ui/studio-live-shell.mjs',import.meta.url),'utf8');assert.match(shell,/ASSETS · VISUAL/);assert.match(shell,/document\.createElement\('canvas'\)/);assert.match(shell,/renderAssetPreview/);assert.match(shell,/data-act=["']focus["']/);assert.match(shell,/EXPLORER/);assert.match(shell,/PROPERTIES/);assert.match(shell,/data-act=\"duplicate\"/);assert.match(shell,/data-act=\"play\"/);assert.match(shell,/data-act=\"brush-size\"/);assert.match(shell,/virtualRange/);
...
const controller=await readFile(new URL('../src/studio/integration/live-studio-controller.mjs',import.meta.url),'utf8');assert.match(controller,/createStudioCameraController/);assert.match(controller,/cameraController\.toWorld/);assert.match(controller,/cameraController\?\.zoom/);assert.match(controller,/studio\.tools\.marquee\.begin/);assert.match(controller,/studio\.tools\.marquee\.commit/);assert.match(controller,/renderAssetPreview/);assert.match(controller,/createCreatorActions/);assert.match(controller,/createCreatorPrefabLibrary/);assert.match(controller,/createCreatorGridOverlay/);assert.match(controller,/analyzeCreatorWorld/);assert.match(controller,/beginStroke/);assert.match(controller,/commitStroke/);assert.match(controller,/togglePlaytest/);assert.match(controller,/KeloInputLocks\.release\(inputLockToken\)/);assert.match(controller,/pasteClipboard/);
