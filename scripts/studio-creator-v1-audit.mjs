...
const controller=await readFile(new URL('../src/studio/integration/live-studio-controller.mjs',import.meta.url),'utf8');assert.match(controller,/createStudioCameraController/);assert.match(controller,/cameraController\.toWorld/);assert.match(controller,/cameraController\?\.zoom/);assert.match(controller,/studio\.tools\.marquee\.begin/);assert.match(controller,/studio\.tools\.marquee\.commit/);assert.match(controller,/renderAssetPreview/);assert.match(controller,/createCreatorActions/);assert.match(controller,/createCreatorPrefabLibrary/);assert.match(controller,/createCreatorGridOverlay/);assert.match(controller,/analyzeCreatorWorld/);assert.match(controller,/beginStroke/);assert.match(controller,/commitStroke/);assert.match(controller,/togglePlaytest/);assert.match(controller,/KeloInputLocks\.release\(inputLockToken\)/);assert.match(controller,/pasteClipboard/);assert.match(controller,/scaleSelection/);
mirror.uninstall();await store.close();
console.log(JSON.stringify({ok:true,creatorV1:true,multiSelect:true,marquee:true,batchHistory:true,duplicate:true,delete:true,rotate:true,rotationAuthoritySafe:true,scale:true,copyPaste:true,continuousBrush:true,brushSizes:true,visualAssetBrowser:true,realPlacementGhost:true,creatorPrefabPreview:true,creatorCamera:true,foundationCameraOwnerFixture:true,focusSelection:true,explorer:true,properties:true,playtestToggle:true,myPrefabs:true,prefabStamp:true,snapGrid:true,mapHealth:true,lazyArchitecturePreserved:true,entities:kernel.document.entities.length,terrainCells:Object.keys(kernel.document.terrain).length,authorityCalls:calls.length},null,2));
