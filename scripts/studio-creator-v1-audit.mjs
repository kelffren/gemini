...
kernel.selection.set([a.id,b.id]);assert.equal(creator.copySelection(),2);const beforePaste=kernel.document.entities.length,historyBeforePaste=kernel.history.undoDepth;const pasted=await creator.pasteClipboard();assert.equal(pasted.length,2);assert.equal(kernel.document.entities.length,beforePaste+2);assert.equal(kernel.history.undoDepth,historyBeforePaste+1,'paste must be one History action');await kernel.undo();assert.equal(kernel.document.entities.length,beforePaste);
...
const controller=await readFile(new URL('../src/studio/integration/live-studio-controller.mjs',import.meta.url),'utf8');assert.match(controller,/createCreatorActions/);assert.match(controller,/createCreatorPrefabLibrary/);assert.match(controller,/createCreatorGridOverlay/);assert.match(controller,/analyzeCreatorWorld/);assert.match(controller,/beginStroke/);assert.match(controller,/commitStroke/);assert.match(controller,/togglePlaytest/);assert.match(controller,/KeloInputLocks\.release\(inputLockToken\)/);assert.match(controller,/onPropertyChange/);assert.match(controller,/pasteClipboard/);
mirror.uninstall();await store.close();
console.log(JSON.stringify({ok:true,creatorV1:true,multiSelect:true,batchHistory:true,duplicate:true,delete:true,rotate:true,rotationAuthoritySafe:true,copyPaste:true,continuousBrush:true,brushSizes:true,explorer:true,properties:true,playtestToggle:true,myPrefabs:true,prefabStamp:true,snapGrid:true,mapHealth:true,lazyArchitecturePreserved:true,entities:kernel.document.entities.length,terrainCells:Object.keys(kernel.document.terrain).length,authorityCalls:calls.length},null,2));
