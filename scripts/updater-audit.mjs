 ...
 * keys: UPDATE PWA SERVICEWORKER AUDIT CI DELTA HASH CACHE INSTANT WATCH
 * purpose: fail closed if Turbo V3 loses content-addressed reuse, adaptive concurrency, gameplay priority or fast detection
 */
...
expect(ui.includes('Nueva versión lista')&&ui.includes('KeloUpdater.applyUpdate()'),'UI updater incompleta');
...
if(!process.exitCode)console.log('UPDATER AUDIT PASS — Turbo V3 delta contract proven');
