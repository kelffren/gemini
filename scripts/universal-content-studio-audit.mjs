...
assert.equal(world.activation.status,'active');assert.equal(world.activation.owner,'KELO_PROPERTY_CATALOG');assert.equal(atlas.length,1);assert.equal(templates.length,1);
...
const migration=fs.readFileSync('supabase/migrations/20260910023500_universal_content_registry.sql','utf8');
