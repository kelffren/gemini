...
try{
  ...
  const report=await page.evaluate(async()=>{
    const b=(await import('./src/creators/sprite-ability/sprite-ability-live-controller.mjs')).getSpriteAbilityBuilder(),s=b.draft.sheet,response=await fetch(s.dataUrl),blob=await response.blob(),bitmap=await createImageBitmap(blob),c=document.createElement('canvas');c.width=bitmap.width;c.height=bitmap.height;const x=c.getContext('2d',{willReadFrequently:true});x.drawImage(bitmap,0,0);bitmap.close?.();const pixels=x.getImageData(0,0,c.width,c.height).data;let transparentCornerFrames=0,opaquePixels=0,transparentPixels=0;for(let f=s.startFrame;f<=s.endFrame;f++){const local=f-s.startFrame,col=local%s.columns,row=Math.floor(local/s.columns),sx=Math.min(c.width-1,col*s.frameWidth+2),sy=Math.min(c.height-1,row*s.frameHeight+2),a=pixels[(sy*c.width+sx)*4+3];if(a<20)transparentCornerFrames++;}for(let p=3;p<pixels.length;p+=4){if(pixels[p]<20)transparentPixels++;else opaquePixels++;}return{sheet:{fileName:s.fileName,columns:s.columns,rows:s.rows,startFrame:s.startFrame,endFrame:s.endFrame,frameWidth:s.frameWidth,frameHeight:s.frameHeight,autoFit:s.autoFit},pixels:{transparentCornerFrames,opaquePixels,transparentPixels,width:c.width,height:c.height},asset:{prefix:String(s.dataUrl).slice(0,32),length:String(s.dataUrl).length,type:blob.type}};
  });
