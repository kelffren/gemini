...
try{const publishable='sb_publishable_fixture';const store=createOnlineIdentityStore({supabaseUrl:'https://example.supabase.co',supabasePublishableKey:publishable,requireAuth:true});const identity=await store.resolve({accessToken:'user.jwt.fixture',characterId:'22222222-2222-4222-8222-222222222222'});assert.equal(identity.authenticated,true);assert.equal(identity.playerKey,'22222222-2222-4222-8222-222222222222');assert.equal(identity.source,'supabase-auth-rls');assert.equal(calls.length,2);for(const call of calls){assert.equal(call.options.headers.apikey,publishable);assert.equal(call.options.headers.Authorization,'Bearer user.jwt.fixture');}assert.equal(store.audit().apiKeyModel,'publishable');await assert.rejects(()=>store.resolve({}),/AUTH_TOKEN_REQUIRED/);}finally{globalThis.fetch=originalFetch;}
