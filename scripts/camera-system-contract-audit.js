...
const context={
  ...
  updateCamera(dt){trace.push('legacy-update:'+dt);this.camera.x+=1;},
  resize(){trace.push('legacy-resize');},
  cycleZoom(){trace.push('legacy-cycle');},
...
trace.length=0;
context.updateCamera(.25);
ok(trace.includes('legacy-update:0.25'),'LEGACY_FOLLOW_CALLED_ONCE');
...
context.KeloCamera.setBaseZoom(1,'test-portrait');
context.KeloCamera.syncViewport('test-portrait');
...
const world={x:525,y:640};
const screen=context.KeloCamera.worldToScreen(world.x,world.y);
const roundTrip=context.KeloCamera.screenToWorld(screen.x,screen.y);
ok(Math.abs(roundTrip.x-world.x)<1e-9&&Math.abs(roundTrip.y-world.y)<1e-9,'SCREEN_WORLD_ROUNDTRIP');

const portraitBase=context.KeloCamera.getBaseZoom();
context.innerWidth=844;context.innerHeight=390;
context.KeloCamera.syncViewport('test-landscape');
...
const before=context.KeloCamera.getBaseZoom();
context.cycleZoom();
