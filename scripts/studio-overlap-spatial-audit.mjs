...
assert.equal(stats.results,3,'point query should return only the three local overlaps');
...
const fallbackSpatial={queryPoint:()=>[{id:'base',data:base}],get:id=>fallbackRects.get(String(id))||null};
...
assert.match(overlapSource,/spatial\.queryPoint\(px,py,\{category:'entity'\}\)/,'overlap selector must use the chunk index point query');
...
console.log(JSON.stringify({ok:true,totalEntities:entities.length,localOverlaps:hits.length,membershipChecks:stats.membershipChecks,fullDocumentScanAvoided:stats.membershipChecks<entities.length,renderOrderPreserved:true,legacyFallback:true,authorityDirectWrite:false},null,2));
