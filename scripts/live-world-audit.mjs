...
const expectedRegistry=process.env.EXPECTED_REGISTRY||'1.10.4';
...
if(!state.architectureRenderer?.ready||!state.architectureRenderer?.rendererWrapped||!state.architectureRenderer?.depthWrapped||state.architectureRenderer?.mode!=='generic-prefab-list-v1'||state.architectureRenderer?.prefabCount<2)throw new Error('Generic architecture prefab renderer invalid');
