 ...
 * keys: PNG SPACE COMPILER CLI REPORT CAPTURE DIFF PROFILE QUALITY ATOMIC WRITE
 * purpose: recursively optimize PNG assets, capture before/after/diff evidence, classify each asset and emit machine-readable quality reports
 * public-api: CLI only
 * state-owned: no runtime state; filesystem writes happen only with --write
 * online: N/A; creator/build-time pipeline
 * consumes: png-space-optimizer.mjs, png-adaptive-optimizer.mjs, asset-image-profiler.mjs
...
function makeDiffPng(beforeBuffer, afterBuffer) {
  const before = decodePngRgba(beforeBuffer);
  const after = decodePngRgba(afterBuffer);
  ...
  for (let pixel = 0; pixel < before.ihdr.width * before.ihdr.height; pixel += 1) {
    ...
    const strength = Math.max(64, Math.min(255, 48 + delta * 8));
    rgba[offset] = 255;
    rgba[offset + 1] = delta > 24 ? 44 : 170;
    rgba[offset + 2] = 35;
    rgba[offset + 3] = strength;
  }
  return {
    buffer:encodeRgbaPng(rgba, before.ihdr.width, before.ihdr.height, {level:9, filterStrategy:'adaptive'}),
    changedPixels,
    maxDelta
  };
}
  ...
  const rows = summary.files.map(item => {
    const captures = capture && item.capture
      ? `<div class="compare">
          ...
          ${item.capture.diff ? `<figure><figcaption>DIFERENCIA ×8</figcaption><img class="diff" src="${htmlEscape(item.capture.diff)}"><small>${item.capture.changedPixels} píxeles · delta máx. ${item.capture.maxDelta}</small></figure>` : ''}
        </div>` : '';
    ...
    return `<article class="${item.status}">
      ...
      <div class="stats">
        ...
        <span>quality ${score}</span><span>${htmlEscape(item.mode || mode)}</span><span>${htmlEscape(exact)}</span>
      </div>
      <p class="profile">${htmlEscape(profile)}${item.profile?.invariants?.preserveBorder ? ' · BORDER LOCK' : ''}${item.profile?.invariants?.preserveAlpha ? ' · ALPHA LOCK' : ''}</p>
      ${captures}
    </article>`;
  }).join('\n');

  const html = `<!doctype html><html lang="es"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Kelo PNG Space Compiler Audit</title><style>
body{font:14px system-ui;background:#101216;color:#f5f7fb;margin:0;padding:24px}main{max-width:1400px;margin:auto}
header{position:sticky;top:0;background:#101216e8;backdrop-filter:blur(12px);padding:12px 0 18px;z-index:2}
article{border:1px solid #303744;border-radius:14px;padding:16px;margin:14px 0;background:#171b22}.stats{display:flex;gap:14px;flex-wrap:wrap}
.profile{color:#aeb9ca}.compare{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin-top:14px}
figure{margin:0;background:#0b0d10;padding:10px;border-radius:10px;overflow:auto}img{max-width:100%;image-rendering:pixelated}.diff{background:repeating-conic-gradient(#1b1d22 0 25%,#262930 0 50%) 0/18px 18px}
figcaption{font-weight:700;margin-bottom:8px}.optimized strong{color:#73e6a4}.skipped strong{color:#ffcb6b}small{display:block;color:#98a4b5;margin-top:6px}
@media(max-width:800px){.compare{grid-template-columns:1fr}body{padding:12px}}
</style><main><header><h1>Kelo PNG Space Compiler</h1>
...
<p>STRICT = RGBA exacto. ADAPTIVE = perfil automático + Quality Agent. DIFERENCIA vacía = ningún píxel cambió.</p></header>${rows}</main></html>`;
...
for (const file of files) {
  ...
  try {
    ...
    optimized = mode === 'adaptive'
      ? await optimizePngAdaptive(original, {sourceName:relative, assetProfile:profile})
      : optimizePngLossless(original);
  } catch (error) {
    const message = String(error?.message || error);
    results.push({file:relative,status:'skipped',mode,beforeBytes:original.length,afterBytes:original.length,savedBytes:0,savedPercent:0,qualityScore:null,profile,error:message,hint:error?.hint || null});
    totalBefore += original.length; totalAfter += original.length;
    console.warn(`SKIP ${relative} — ${message}`);
    continue;
  }
  ...
  if (capture) {
    ...
    const beforeName = `${key}--before.png`, afterName = `${key}--after.png`;
    ...
    try {
      const diff = makeDiffPng(original, after);
      ...
      captureRecord.diffError = String(error?.message || error);
    }
  }
  ...
  const entry = {
    file:relative,status:report.status,mode:report.mode || 'strict',wrote:shouldWrite,
    beforeBytes:original.length,afterBytes:after.length,savedBytes:Math.max(0, original.length - after.length),
    ...
    profile:report.assetProfile || profile,winner:report.winner || null,capture:captureRecord,detail:report
  };
  results.push(entry);
  totalBefore += original.length; totalAfter += after.length;
  console.log(`${entry.status.toUpperCase()} ${entry.file} ${humanBytes(entry.beforeBytes)} → ${humanBytes(entry.afterBytes)} (-${entry.savedPercent.toFixed(2)}%) ${entry.profile?.kind || ''}`);
}

const summary = {
  compiler:'kelo-png-space-compiler-v1.2',generatedAt:new Date().toISOString(),input:safeRelative(inputPath),mode,write,capture,
  fileCount:results.length,beforeBytes:totalBefore,afterBytes:totalAfter,savedBytes:Math.max(0,totalBefore-totalAfter),
  savedPercent:totalBefore ? ((totalBefore-totalAfter)/totalBefore)*100 : 0,files:results
};
