...
const expectedCompositionMode='registry-authored-garden-compositions-v13';
...
function valid(state){
  const water=state.composition?.relocatedWaterAnchors,westPlinth=state.composition?.relocatedWestPlinthAnchor,eastPlinth=state.composition?.relocatedEastPlinthAnchor,step=state.composition?.relocatedSteppingStoneAnchor;
...
if(!loaded){
  ...
  await browser.close();
  throw new Error(`LIVE never reached Gardens v13 contract: ${JSON.stringify(state)}`);
}
...
const shot=await page.evaluate(()=>{
  ...
  localPlayer.x=1510;localPlayer.y=2660;camera.x=1510;camera.y=2660;camera.targetX=1510;camera.targetY=2660;render();
...
if(!valid(finalState))throw new Error(`Final Gardens v13 contract invalid: ${JSON.stringify(finalState)}`);
