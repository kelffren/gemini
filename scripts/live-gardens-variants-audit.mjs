...
const expectedCompositionMode='registry-authored-garden-compositions-v7';
...
const expectedRelocationMode='authored-road-clear-placements-v2';
...
const shot=await page.evaluate(()=>{
  ...
  localPlayer.x=1360;localPlayer.y=2420;camera.x=1360;camera.y=2420;camera.targetX=1360;camera.targetY=2420;render();
...
if(!shot.composition?.ready||shot.composition?.mode!==expectedCompositionMode||shot.composition?.centerVariationMode!=='authored-mid-variant-selection-v2'||shot.composition?.verticalVariationMode!=='mirrored-authored-vertical-mid-v1'||shot.composition?.fixedPlacementMode!==expectedFixedPlacementMode||shot.composition?.navigationSafeRelocationMode!==expectedRelocationMode||shot.composition?.navigationConflictFixCount!==5||shot.composition?.relocatedEastRunAnchor?.[0]!==24||shot.composition?.relocatedEastRunAnchor?.[1]!==13||shot.composition?.relocatedFlowerbedNWAnchor?.[0]!==8||shot.composition?.relocatedFlowerbedNWAnchor?.[1]!==8||shot.composition?.declaredCellCount!==41||shot.composition?.fixedPlacementCount!==10||shot.composition?.altCenterTileCount!==4||shot.composition?.verticalAltUsageCount!==2)throw new Error(`Composition contract invalid: ${JSON.stringify(shot.composition)}`);
