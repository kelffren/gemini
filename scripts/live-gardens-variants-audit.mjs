...
const expectedCompositionMode='registry-authored-garden-compositions-v9';
...
const expectedRelocationMode='authored-road-clear-placements-v4';
...
const shot=await page.evaluate(()=>{
  ...
  localPlayer.x=1350;localPlayer.y=2600;camera.x=1350;camera.y=2600;camera.targetX=1350;camera.targetY=2600;render();
...
if(!shot.composition?.ready||shot.composition?.mode!==expectedCompositionMode||shot.composition?.centerVariationMode!=='authored-mid-variant-selection-v2'||shot.composition?.verticalVariationMode!=='mirrored-authored-vertical-mid-v1'||shot.composition?.fixedPlacementMode!==expectedFixedPlacementMode||shot.composition?.navigationSafeRelocationMode!==expectedRelocationMode||shot.composition?.navigationConflictFixCount!==8||shot.composition?.relocatedEastRunAnchor?.[0]!==24||shot.composition?.relocatedEastRunAnchor?.[1]!==13||shot.composition?.relocatedFlowerbedNWAnchor?.[0]!==8||shot.composition?.relocatedFlowerbedNWAnchor?.[1]!==8||shot.composition?.relocatedFlowerbedNEAnchor?.[0]!==19||shot.composition?.relocatedFlowerbedNEAnchor?.[1]!==13||shot.composition?.relocatedFlowerbedSWAnchor?.[0]!==8||shot.composition?.relocatedFlowerbedSWAnchor?.[1]!==14||shot.composition?.declaredCellCount!==41||shot.composition?.fixedPlacementCount!==10||shot.composition?.altCenterTileCount!==4||shot.composition?.verticalAltUsageCount!==2)throw new Error(`Composition contract invalid: ${JSON.stringify(shot.composition)}`);
