...
const expectedCompositionMode='registry-authored-garden-compositions-v10';
...
const expectedRelocationMode='authored-road-clear-placements-v5';
...
for(let attempt=1;attempt<=24;attempt++){
  try{
    ...
    if(state.joins?.mode===expectedJoinMode&&state.joins?.id==='gardens-joins-v3'&&state.joins?.width===288&&state.joins?.height===32&&state.joins?.columns===9&&state.joins?.tiles?.HEDGE_MID_ALT===6&&state.joins?.tiles?.FLOWER_MID_ALT===7&&state.joins?.tiles?.HEDGE_V_ALT===8&&state.composition?.ready&&state.composition?.mode===expectedCompositionMode&&state.composition?.centerVariationMode==='authored-mid-variant-selection-v2'&&state.composition?.verticalVariationMode==='mirrored-authored-vertical-mid-v1'&&state.composition?.fixedPlacementMode===expectedFixedPlacementMode&&state.composition?.navigationSafeRelocationMode===expectedRelocationMode&&state.composition?.navigationConflictFixCount===10&&state.composition?.relocatedEastRunAnchor?.[0]===24&&state.composition?.relocatedEastRunAnchor?.[1]===13&&state.composition?.relocatedFlowerbedNWAnchor?.[0]===8&&state.composition?.relocatedFlowerbedNWAnchor?.[1]===8&&state.composition?.relocatedFlowerbedNEAnchor?.[0]===19&&state.composition?.relocatedFlowerbedNEAnchor?.[1]===13&&state.composition?.relocatedFlowerbedSWAnchor?.[0]===8&&state.composition?.relocatedFlowerbedSWAnchor?.[1]===14&&water?.[0]?.[0]===9&&water?.[0]?.[1]===11&&water?.[1]?.[0]===19&&water?.[1]?.[1]===11&&state.composition?.declaredCellCount===41&&state.composition?.fixedPlacementCount===10&&state.composition?.altCenterTileCount===4&&state.composition?.verticalAltUsageCount===2&&state.world?.ready&&state.world?.version===expectedWorldVersion&&state.world?.gardensCornerMode===expectedCornerMode&&state.world?.gardensCornerOrientationCount===4&&state.world?.gardensFixedPlacementMode===expectedFixedPlacementMode&&state.world?.gardensFixedPlacementCount===10&&state.world?.gardensJoinAssetLoaded&&state.landmark?.ready&&!state.landmark?.failed)break;
...
const shot=await page.evaluate(()=>{
  ...
  localPlayer.x=1504;localPlayer.y=2520;camera.x=1504;camera.y=2520;camera.targetX=1504;camera.targetY=2520;render();
...
const water=shot.composition?.relocatedWaterAnchors;
if(!shot.composition?.ready||shot.composition?.mode!==expectedCompositionMode||shot.composition?.centerVariationMode!=='authored-mid-variant-selection-v2'||shot.composition?.verticalVariationMode!=='mirrored-authored-vertical-mid-v1'||shot.composition?.fixedPlacementMode!==expectedFixedPlacementMode||shot.composition?.navigationSafeRelocationMode!==expectedRelocationMode||shot.composition?.navigationConflictFixCount!==10||shot.composition?.relocatedEastRunAnchor?.[0]!==24||shot.composition?.relocatedEastRunAnchor?.[1]!==13||shot.composition?.relocatedFlowerbedNWAnchor?.[0]!==8||shot.composition?.relocatedFlowerbedNWAnchor?.[1]!==8||shot.composition?.relocatedFlowerbedNEAnchor?.[0]!==19||shot.composition?.relocatedFlowerbedNEAnchor?.[1]!==13||shot.composition?.relocatedFlowerbedSWAnchor?.[0]!==8||shot.composition?.relocatedFlowerbedSWAnchor?.[1]!==14||water?.[0]?.[0]!==9||water?.[0]?.[1]!==11||water?.[1]?.[0]!==19||water?.[1]?.[1]!==11||shot.composition?.declaredCellCount!==41||shot.composition?.fixedPlacementCount!==10||shot.composition?.altCenterTileCount!==4||shot.composition?.verticalAltUsageCount!==2)throw new Error(`Composition contract invalid: ${JSON.stringify(shot.composition)}`);
