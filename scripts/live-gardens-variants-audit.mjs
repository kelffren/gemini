...
const expectedCompositionMode='registry-authored-garden-compositions-v6';
...
const expectedRelocationMode='authored-road-clear-east-run-v1';
...
const shot=await page.evaluate(()=>{
  ...
  localPlayer.x=1710;localPlayer.y=2500;camera.x=1710;camera.y=2500;camera.targetX=1710;camera.targetY=2500;render();
...
if(!shot.composition?.ready||shot.composition?.mode!==expectedCompositionMode||shot.composition?.centerVariationMode!=='authored-mid-variant-selection-v2'||shot.composition?.verticalVariationMode!=='mirrored-authored-vertical-mid-v1'||shot.composition?.fixedPlacementMode!==expectedFixedPlacementMode||shot.composition?.navigationSafeRelocationMode!==expectedRelocationMode||shot.composition?.navigationConflictFixCount!==3||shot.composition?.relocatedEastRunAnchor?.[0]!==24||shot.composition?.relocatedEastRunAnchor?.[1]!==13||shot.composition?.fixedPlacementCount!==10||shot.composition?.altCenterTileCount!==4||shot.composition?.verticalAltUsageCount!==2)throw new Error(`Composition contract invalid: ${JSON.stringify(shot.composition)}`);
