...
const expectedCompositionMode='registry-authored-garden-compositions-v14';
...
const expectedJunctionMode='connected-southeast-l-boundary-v1';
...
function valid(state){
  const water=state.composition?.relocatedWaterAnchors,westPlinth=state.composition?.relocatedWestPlinthAnchor,eastPlinth=state.composition?.relocatedEastPlinthAnchor,step=state.composition?.relocatedSteppingStoneAnchor,seCorner=state.composition?.southeastJunctionCorner,seRun=state.composition?.southeastHorizontalRunAnchor;
  return state.joins?.mode===expectedJoinMode&&state.joins?.id==='gardens-joins-v3'&&state.joins?.width===288&&state.joins?.height===32&&state.joins?.columns===9&&state.joins?.tiles?.HEDGE_MID_ALT===6&&state.joins?.tiles?.FLOWER_MID_ALT===7&&state.joins?.tiles?.HEDGE_V_ALT===8&&
    state.composition?.ready&&state.composition?.mode===expectedCompositionMode&&state.composition?.centerVariationMode==='authored-mid-variant-selection-v2'&&state.composition?.verticalVariationMode==='mirrored-authored-vertical-mid-v1'&&state.composition?.junctionMode===expectedJunctionMode&&state.composition?.connectedJunctionCount===1&&seCorner?.[0]===22&&seCorner?.[1]===17&&seRun?.[0]===18&&seRun?.[1]===17&&state.composition?.fixedPlacementMode===expectedFixedPlacementMode&&state.composition?.navigationSafeRelocationMode===expectedRelocationMode&&state.composition?.navigationConflictFixCount===13&&
...
if(!loaded){
  ...
  await browser.close();
  throw new Error(`LIVE never reached Gardens v14 contract: ${JSON.stringify(state)}`);
}
...
if(!valid(finalState))throw new Error(`Final Gardens v14 contract invalid: ${JSON.stringify(finalState)}`);
