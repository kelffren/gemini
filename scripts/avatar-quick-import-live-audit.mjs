...
try{
  ...
  if(direct.transparent.compilerVersion!=='5.0.0'||direct.transparent.source[0]!==3||direct.transparent.source[1]!==4||direct.transparent.compiled[1]!==4||!direct.transparent.canonicalRig)throw new Error(`AVATAR_V5_TRANSPARENT_FAILED:${JSON.stringify(direct.transparent)}`);
  ...
  const used=await page.evaluate(()=>window.__avatarV5Use);if(!used||used.columns!==4||used.rows!==4||used.sourceRects!==16||used.compilerVersion!=='5.0.0'||used.universalAuto!==true)throw new Error(`AVATAR_V5_USE_FAILED:${JSON.stringify(used)}`);
