...
function normalize(snapshot){
  const players={};
  Object.keys(snapshot.players||{}).sort().forEach(id=>{
    const p=snapshot.players[id];
    players[id]={id:p.id,x:+p.x.toFixed(6),y:+p.y.toFixed(6),hp:p.hp,mana:+p.mana.toFixed(6),ackSequence:p.ackSequence,zone:p.zone,cooldowns:p.cooldowns,attackPhase:p.attackPhase,attackKind:p.attackKind,castPhase:p.castPhase,dash:p.dash,basicResource:p.basicResource,specialResource:p.specialResource,comboStep:p.comboStep,dodgeCooldown:+Number(p.dodgeCooldown||0).toFixed(6),statuses:p.statuses};
  });
  return{serverTick:snapshot.serverTick,players,projectiles:(snapshot.projectiles||[]).map(p=>({id:p.id,ownerId:p.ownerId,abilityKey:p.abilityKey,x:+p.x.toFixed(6),y:+p.y.toFixed(6),vx:+Number(p.vx||0).toFixed(6),vy:+Number(p.vy||0).toFixed(6),radius:p.radius}))};
}
...
assert.equal(exported.schema,adapter.STATE_SCHEMA);
assert.ok(exported.actors.length>=2);
assert.ok(exported.serverTick>0);
...
const audit=b.importState(exported,1_000_300);
assert.equal(audit.portable,true);
assert.equal(audit.exportImport,true);
assert.equal(audit.serverTick,exported.serverTick);
...
assert.match(core,/exportState/);
assert.match(core,/importState/);
// Guard executable transport coupling without flagging architecture comments such as
// "NO WebSocket/WebRTC". The portable authority must not instantiate transports,
// import Supabase, or dereference a Supabase client.
assert.doesNotMatch(core,/\b(?:new\s+WebSocket\s*\(|new\s+RTCPeerConnection\s*\(|@supabase\/|supabase\s*\.)/i);
assert.match(worker,/pvp-authority-core\.js/);
assert.doesNotMatch(worker,/setInterval\s*\(/);
assert.doesNotMatch(host,/setInterval\s*\(/);
assert.match(host,/KeloSimulation\.after\('guardian:simulation-host'/);
assert.match(host,/guardian:pvp_state_chunk/);
assert.match(host,/guardian:pvp_state_ack/);
assert.match(host,/durableEconomyAuthority:false/);
assert.match(net,/guardian-webrtc/);
assert.match(net,/server-websocket/);
assert.match(net,/DURABLE_SERVER_OFFLINE/);
assert.match(net,/KeloGuardianSimulationHost/);
assert.match(net,/kelo:guardian-pvp-snapshot/);
assert.match(net,/durableServerFailClosed:true/);
assert.match(ui,/guardian-simulation-host\.js/);
assert.match(ui,/PvP authority/);
assert.match(ui,/durableEconomyAuthority:false/);
