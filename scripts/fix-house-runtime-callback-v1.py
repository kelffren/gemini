...
new="instance.onEnter=async(_participant,runtime)=>window.KELO_INSTANCE_RUNTIME?.enter?.(runtime);instance.onExit=async()=>window.KELO_INSTANCE_RUNTIME?.leave?.();instance.onDestroy=async(runtime,opts)=>{if(opts?.reason!=='crash')await persistRuntime(runtime,false,false);};"
if old not in s:
