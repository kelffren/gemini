...
function valid(s){return s.title===expectedTitle&&s.contract?.version==='1.0.0'&&s.contract?.mode==='data-driven-building-prefabs-v1'&&s.contract?.prefabs?.length===1&&String(s.audit?.version||'').startsWith('generic-prefabs-v1')&&s.audit?.rendererMode==='data-driven-prefabs-v1'&&s.audit?.ready===true&&s.audit?.failed===false&&s.audit?.registeredColliderCount===1&&s.renderer?.ready===true&&s.renderer?.failed===false&&s.architecture?.mode==='generic-prefab-contract-v1'&&s.architecture?.ready===true&&s.architecture?.backLayerRegistered===true&&s.architecture?.frontLayerRegistered===true&&s.luxe?.source==='generic-prefab-contract'&&s.luxe?.ready===true&&s.luxe?.failed===false&&s.layers?.some(x=>x.id==='architecture-prefabs-back'&&x.phase==='props_back'&&x.ready)&&s.layers?.some(x=>x.id==='architecture-prefabs-front'&&x.phase==='props_front'&&x.ready)&&s.liveColliders?.length===1;}
let state=null;
