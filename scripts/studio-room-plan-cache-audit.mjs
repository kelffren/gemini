...
assert.deepEqual(stats,{requests:1,rebuilds:1,reuses:0},'first plan must build one perimeter');
...
assert.deepEqual([measurement.width,measurement.height,measurement.requestedWidth,measurement.deltaWidth],[128,64,128,0],'cached perimeter must still refresh live requested measurement');
assert.equal(second[0].components.buildingPiece.roomId,firstRoomId,'reuse must preserve semantic room identity');
assert.deepEqual(second,first,'same quantized geometry must reuse an equivalent perimeter instead of rebuilding rows');
...
assert.equal(stats.rebuilds,1,'100 same-geometry pointer updates must not rebuild the perimeter');
...
stats=room.getPlannerStats();
assert.equal(changed.length,8,'crossing into a 3x1 modular room must rebuild an eight-wall perimeter');
...
assert.equal(committed.length,12,'cached square plan must persist its full 3x3 perimeter');
...
await kernel.redo();
assert.equal(kernel.document.entities.length,12,'one Redo must restore the cached ROOM commit');
...
console.log(JSON.stringify({ok:true,phase:'5.6',tool:'ROOM',improvement:'quantized-plan-cache',requests:103,rebuildsBeforeModifier:2,reuses:101,redundantRebuildReduction:'100 -> 0',liveMeasurement:true,historyEntries:1,oneUndo:true,oneRedo:true,authorityBypass:false},null,2));
