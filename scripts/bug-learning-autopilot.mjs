 ...
 * purpose: learn bounded risk multipliers, hotspots and prevention gaps from canonical bug history using champion/challenger + holdout
...
const sourceFingerprint=evolutionFingerprint({riskMap,bugs:bugs.map(compactBugForFingerprint),reports:reports.map(compactReportForFingerprint)});
const riskMapFingerprint=evolutionFingerprint(riskMap);

if(previous?.source_fingerprint===sourceFingerprint){
  const summary={ok:true,changed:false,reason:'NO_NEW_BUG_EVIDENCE',sourceFingerprint,champion:previous.champion||null,hotspots:(previous.hotspots||[]).length,preventionGaps:(previous.prevention_gaps||[]).length};
...
const state={
  ...
  training:{examples:examples.length,search_examples:searchExamples.length,holdout_examples:holdoutExamples.length,latest_evidence_at:evidenceAt},
  ...
  safety:{auto_write_scope:['bugs/learning/STATE.json'],arbitrary_source_code_writes:false,bug_state_mutation:false,auto_close:false,requires_holdout_for_policy_promotion:true,max_rule_multiplier:1.75,min_rule_multiplier:.75,max_hotspot_bonus:20},
  updated_at:new Date().toISOString()
};

if(write){fs.mkdirSync(path.dirname(statePath),{recursive:true});fs.writeFileSync(statePath,JSON.stringify(state,null,2)+'\n');}
const summary={ok:true,changed:true,written:write,sourceFingerprint,examples:examples.length,search:searchExamples.length,holdout:holdoutExamples.length,accepted,champion:state.champion,hotspots:hotspots.length,preventionGaps:state.prevention_gaps.length,memory:state.memory_summary};
console.log(jsonOut?JSON.stringify({summary,state},null,2):[`BUG LEARNING — ${accepted?'PROMOTED':'OBSERVED'}`,`evidence=${examples.length} search=${searchExamples.length} holdout=${holdoutExamples.length}`,`champion=${champion.id} score=${championSearch.score}${championHoldout?` holdout=${championHoldout.score}`:''}`,`hotspots=${hotspots.length} prevention_gaps=${state.prevention_gaps.length}`,write?`written=${path.relative(root,statePath)}`:'dry-run (use --write to persist)'].join('\n'));
