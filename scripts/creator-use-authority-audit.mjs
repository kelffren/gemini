...
const probeStart=lazy.indexOf('async function probeCharacterAppearance');
const probeEnd=lazy.indexOf('function onAuthState',probeStart);
const probe=probeStart>=0&&probeEnd>probeStart?lazy.slice(probeStart,probeEnd):'';
...
check('normal session restore is metadata-only and avoids full Creator OS',/useAuthorityModuleUrl/.test(lazy)&&/KeloCreatorUse/.test(lazy)&&/hydrateRuntime:false/.test(probe)&&!/loadPlatform\(\)/.test(probe));
check('appearance stays lazy when authoritative visual loadout is empty',/if\(!bindings\.length\)/.test(probe)&&/ensure\('appearance'\)/.test(probe));
...
check('no polling or second render loop',!/setInterval|requestAnimationFrame/.test(use)&&!/setInterval|requestAnimationFrame/.test(migration)&&!/setInterval|requestAnimationFrame/.test(probe));
