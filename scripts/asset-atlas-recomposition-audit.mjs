 ...
 * keys: ATLAS RECOMPOSITION ALPHA TRIM ORIG ANCHOR MAXRECTS RENDER EXACT
 * purpose: prove every alpha-safe trimmed frame can be recomposed to its original rendered result before atlas promotion
...
const manifest=parseManifest(manifestFile),atlas=manifest.atlas||{},sourcePath=manifest.source?.path||atlas.sourcePath;assert.ok(sourcePath&&fs.existsSync(sourcePath),'atlas source missing');const decoded=decodePngRgba(fs.readFileSync(sourcePath)),plan=planSmartAtlas(atlas.frames,{originalWidth:decoded.ihdr.width,originalHeight:decoded.ihdr.height,padding:2,minAreaSavingPercent:0,rgba:decoded.rgba,atlasWidth:decoded.ihdr.width});assert.ok(plan.maxRects,'MaxRects plan missing');
const packed=Buffer.alloc(plan.maxRects.width*plan.maxRects.height*4),frameReports=[];for(const frame of plan.frames){const placement=plan.maxRects.placements[frame.id];assert.ok(placement,`${frame.id}: placement missing`);copyRect(decoded.rgba,decoded.ihdr.width,{x:frame.sourceRect.x+frame.trim.x,y:frame.sourceRect.y+frame.trim.y,w:frame.trim.w,h:frame.trim.h},packed,plan.maxRects.width,placement.x,placement.y);}
for(const frame of plan.frames){const placement=plan.maxRects.placements[frame.id],original=extract(decoded.rgba,decoded.ihdr.width,frame.sourceRect),reconstructed=Buffer.alloc(frame.orig.w*frame.orig.h*4),packedTrim=extract(packed,plan.maxRects.width,placement);copyRect(packedTrim,placement.w,{x:0,y:0,w:placement.w,h:placement.h},reconstructed,frame.orig.w,frame.trim.x,frame.trim.y);const metrics=evaluatePixelFidelity(original,reconstructed,frame.orig.w,frame.orig.h),verdict=judgePixelFidelity(metrics,'render-exact');assert.equal(verdict.pass,true,`${frame.id}: recomposition changed visible pixels/alpha`);if(frame.originalAnchor&&frame.anchor){assert.equal(frame.anchor.x+frame.trim.x,frame.originalAnchor.x,`${frame.id}: anchor x drift`);assert.equal(frame.anchor.y+frame.trim.y,frame.originalAnchor.y,`${frame.id}: anchor y drift`);}frameReports.push({id:frame.id,renderExact:metrics.renderExactPixels,hiddenTransparentRgbChanges:metrics.hiddenTransparentRgbChangedPixels,anchorPreserved:!frame.originalAnchor||(frame.anchor.x+frame.trim.x===frame.originalAnchor.x&&frame.anchor.y+frame.trim.y===frame.originalAnchor.y),manifestBoundsSafe:frame.manifestContainsPixels});}
console.log(JSON.stringify({status:'ASSET_ATLAS_RECOMPOSITION_OK',frames:frameReports.length,packed:{width:plan.maxRects.width,height:plan.maxRects.height,areaSavingPercent:plan.maxRects.estimatedAreaSavingPercent},renderExactFrames:frameReports.filter(f=>f.renderExact).length,anchorPreserved:frameReports.every(f=>f.anchorPreserved),manifestUnsafeFrames:plan.trim.manifestUnsafeFrameCount,hiddenTransparentRgbChanges:frameReports.reduce((s,f)=>s+f.hiddenTransparentRgbChanges,0)},null,2));
