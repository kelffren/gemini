...
async function runtimeReady(){
  return page.evaluate(expected=>document.title===expected&&!!window.KeloBackpack&&!!window.KeloBackpackUI&&!!window.KeloEquipment&&!!document.getElementById('lx-side-menu'),expectedTitle);
}
...
if(!ready)throw new Error(`LIVE never reached ${expectedTitle} with backpack runtime`);
...
const persisted=await page.evaluate(({destination,key})=>({ready:!!window.KeloBackpack,persistedKey:window.KeloBackpack.getSlots()[destination]?.key||null,expectedKey:key}),{destination:initial.emptyIndex,key:initial.occupiedKey});
...
if(equipmentIndex==null)throw new Error('No equipment item surfaced through backpack');
...
if(persisted.persistedKey!==persisted.expectedKey)throw new Error(`Slot persistence failed: ${JSON.stringify(persisted)}`);
