...
let opened=null;const manifest=createDefinitionWorkspaceManifest({id:'npc',type:'NPC',loader:async()=>({openGenericDefinitionCreator:async context=>{opened=context;return{ok:true};}})});const testRoot={document:{}};const result=await manifest.open({root:testRoot,projects:{},projectId:'creator-project:test'});assert.deepEqual(result,{ok:true});assert.equal(opened.definitionType,'NPC');assert.equal(opened.projectId,'creator-project:test');assert.equal(typeof testRoot.KELO_CREATOR_TEST_BRIDGE?.run,'function');assert.equal(typeof testRoot.KELO_CREATOR_TEST_BRIDGE?.restore,'function');assert.equal(typeof testRoot.KELO_ENVIRONMENT_RUNTIME?.applyTemporary,'function');assert.equal(typeof testRoot.KELO_ENVIRONMENT_RUNTIME?.publish,'function');

console.log(JSON.stringify({ok:true,workspaces:registered.map(x=>x.id),promptInterpreter:true,promptInterpreterV2:true,environmentLivePreview:true,nativeEnvironmentRuntime:true,environmentRuntimeTestBridge:true,reversibleRuntimeTest:true,localApprovedEnvironmentPersistence:true,referenceImageOverlay:true,negationAware:true,bilingualEnvironmentIntent:true,defaultsValidate:true,lazyWorkspaceRouting:true},null,2));
