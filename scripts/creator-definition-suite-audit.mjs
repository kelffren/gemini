...
let opened=null;const manifest=createDefinitionWorkspaceManifest({id:'npc',type:'NPC',loader:async()=>({openGenericDefinitionCreator:async context=>{opened=context;return{ok:true};}})});const testRoot={document:{}};const result=await manifest.open({root:testRoot,projects:{},projectId:'creator-project:test',contentSession:{accessToken:'audit-token'}});assert.deepEqual(result,{ok:true});assert.equal(opened.definitionType,'NPC');assert.equal(opened.projectId,'creator-project:test');assert.equal(typeof testRoot.KELO_CREATOR_TEST_BRIDGE?.run,'function');assert.equal(typeof testRoot.KELO_CREATOR_TEST_BRIDGE?.restore,'function');assert.equal(typeof testRoot.KELO_CREATOR_TEST_BRIDGE?.approve,'function');assert.equal(typeof testRoot.KELO_CREATOR_TEST_BRIDGE?.rollbackLast,'function');assert.equal(typeof testRoot.KELO_ENVIRONMENT_RUNTIME?.applyTemporary,'function');assert.equal(typeof testRoot.KELO_ENVIRONMENT_RUNTIME?.publish,'function');assert.equal(typeof testRoot.KELO_ENVIRONMENT_RUNTIME?.receivePublished,'function');
