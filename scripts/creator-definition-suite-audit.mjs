...
console.log(JSON.stringify({ok:true,workspaces:registered.map(x=>x.id),promptInterpreter:true,promptInterpreterV2:true,environmentLivePreview:true,nativeEnvironmentRuntime:true,environmentRuntimeTestBridge:true,reversibleRuntimeTest:true,worldEnvironmentSync:true,serverPublishedEnvironment:true,realtimeRevisionEnvelope:true,revisionActionMetadata:true,roleGatedWorldHistory:true,boundedWorldHistory:true,historyRollbackControls:true,previewPreservedAcrossRemotePublish:true,roleGatedApplyWorld:true,safeWorldUndo:true,monotonicRollback:true,referenceImageOverlay:true,negationAware:true,bilingualEnvironmentIntent:true,defaultsValidate:true,lazyWorkspaceRouting:true},null,2));
