...
import { createCreatorTestPlan } from '../src/creators/definition/creator-test-bridge.mjs';
import { normalizeEnvironmentState } from '../src/environment/environment-runtime.mjs';
import { rowToWorldEnvironmentEnvelope } from '../src/environment/environment-world-sync.mjs';
...
const syncEnvelope=rowToWorldEnvironmentEnvelope({id:'global',revision:7,schema_version:1,biome:'coast',weather:'rain',time_of_day:'sunset',ambient_density:64,music_mood:'epic',accent:'#4b9cb8',updated_at:'2026-09-16T07:00:00Z',updated_by:null});
...
let opened=null;const manifest=createDefinitionWorkspaceManifest({id:'npc',type:'NPC',loader:async()=>({openGenericDefinitionCreator:async context=>{opened=context;return{ok:true};}})});const testRoot={document:{}};const result=await manifest.open({root:testRoot,projects:{},projectId:'creator-project:test',contentSession:{accessToken:'audit-token'}});assert.deepEqual(result,{ok:true});assert.equal(opened.definitionType,'NPC');assert.equal(opened.projectId,'creator-project:test');assert.equal(typeof testRoot.KELO_CREATOR_TEST_BRIDGE?.run,'function');assert.equal(typeof testRoot.KELO_CREATOR_TEST_BRIDGE?.restore,'function');assert.equal(typeof testRoot.KELO_CREATOR_TEST_BRIDGE?.approve,'function');assert.equal(typeof testRoot.KELO_ENVIRONMENT_RUNTIME?.applyTemporary,'function');assert.equal(typeof testRoot.KELO_ENVIRONMENT_RUNTIME?.publish,'function');assert.equal(typeof testRoot.KELO_ENVIRONMENT_RUNTIME?.receivePublished,'function');
