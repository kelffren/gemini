...
const expectedWorldVersion=process.env.EXPECTED_WORLD_VERSION||'world-v1.10';
...
const report={loaded,title:await page.title(),expectedWorldVersion,expectedGardensPathMode,expectedGardensFramingMode,expectedGardensTransitionMode,shot:shot?{audit:shot.audit,world:shot.world,canvas:shot.canvas,cyan:shot.cyan,ivory:shot.ivory,gold:shot.gold,green:shot.green}:null,consoleErrors,failedRequests,httpErrors};
fs.writeFileSync('artifacts/gardens-report.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));
await browser.close();
if(!loaded)throw new Error(`LIVE never reached Gardens contract ${expectedWorldVersion} / ${expectedGardensPathMode} / ${expectedGardensTransitionMode}`);
