...
const expectedTitle=process.env.EXPECTED_TITLE||'Kelo World — V6.13';
const expectedWorldVersion=process.env.EXPECTED_WORLD_VERSION||'world-v1.13';
...
const expectedGardensFramingMode=process.env.EXPECTED_GARDENS_FRAMING_MODE||'registry-authored-modular-framing-v2';
const expectedGardensCompositionMode=process.env.EXPECTED_GARDENS_COMPOSITION_MODE||'registry-authored-garden-compositions-v1';
...
await page.route(/\/src\/environment\/(world-map|tile-registry|district-decals-registry|gardens-atlas|gardens-compositions)\.js/,route=>{
...
const report={loaded,title:await page.title(),expectedWorldVersion,expectedRegistryVersion,expectedGardensMode,expectedGardensPathMode,expectedGardensFramingMode,expectedGardensCompositionMode,expectedGardensTransitionMode,expectedGardensMarbleMode,expectedGardensGrassRhythmMode,depthAudit,shot:shot?{audit:shot.audit,world:shot.world,composition:shot.composition,registryVersion:shot.registry?.version,canvas:shot.canvas,cyan:shot.cyan,ivory:shot.ivory,gold:shot.gold,green:shot.green}:null,consoleErrors,failedRequests,httpErrors};
...
if(!shot.composition?.ready||shot.composition?.mode!==expectedGardensCompositionMode||shot.composition?.compositionCount<10)throw new Error(`Gardens composition registry invalid: ${JSON.stringify(shot.composition)}`);
