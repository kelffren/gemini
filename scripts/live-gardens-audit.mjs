...
const expectedWorldVersion=process.env.EXPECTED_WORLD_VERSION||'world-v1.18';
...
await page.route(/\/src\/environment\/(world-map|gardens-compositions|gardens-junction-overlay)\.js/,route=>{const u=new URL(route.request().url());u.searchParams.set('audit-bust',`${Date.now()}-${Math.random()}`);route.continue({url:u.toString()});});
...
for(let attempt=1;attempt<=24;attempt++){
  try{
    ...
    lastState=await page.evaluate(()=>({title:document.title,registry:window.KELO_TILE_REGISTRY||null,joins:window.KELO_GARDENS_JOINS||null,composition:window.KELO_GARDENS_COMPOSITION_AUDIT||null,junction:window.KELO_GARDENS_JUNCTION_AUDIT||null,world:window.KELO_WORLD_AUDIT||null}));
    if(valid(lastState)){loaded=true;break;}
  }catch(e){console.log(`attempt ${attempt}: ${e.message}`)}
  await page.waitForTimeout(10000);
}
if(!loaded){fs.writeFileSync('artifacts/gardens-report.json',JSON.stringify({loaded,lastState,consoleErrors,failedRequests,httpErrors,expected:{expectedTitle,expectedWorldVersion,expectedCompositionMode,expectedJoinMode}},null,2));await browser.close();throw new Error(`LIVE never reached current Gardens contract: ${JSON.stringify(lastState)}`);}
...
const finalState={title:await page.title(),registry:shot?.registry,joins:shot?.joins,composition:shot?.composition,junction:shot?.junction,world:shot?.world};
