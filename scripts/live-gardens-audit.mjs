...
const report={loaded,title:await page.title(),expectedWorldVersion,expectedRegistryVersion,expectedGardensPathMode,expectedGardensFramingMode,expectedGardensTransitionMode,expectedGardensMarbleMode,expectedGardensGrassRhythmMode,depthAudit,shot:shot?{audit:shot.audit,world:shot.world,registryVersion:shot.registry?.version,canvas:shot.canvas,cyan:shot.cyan,ivory:shot.ivory,gold:shot.gold,green:shot.green}:null,consoleErrors,failedRequests,httpErrors};
fs.writeFileSync('artifacts/gardens-report.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));
await browser.close();
if(!loaded)throw new Error(`LIVE never reached Gardens contract ${expectedWorldVersion} / ${expectedGardensMarbleMode} / ${expectedGardensGrassRhythmMode}`);
