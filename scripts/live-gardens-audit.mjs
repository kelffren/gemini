...
const expectedGardensFramingMode=process.env.EXPECTED_GARDENS_FRAMING_MODE||'registry-authored-modular-framing-v5';
const expectedGardensCompositionMode=process.env.EXPECTED_GARDENS_COMPOSITION_MODE||'registry-authored-garden-compositions-v13';
...
if(!shot.composition?.ready||shot.composition?.mode!==expectedGardensCompositionMode||shot.composition?.compositionCount<10||shot.composition?.joinMode!==expectedGardensJoinMode||shot.composition?.navigationConflictFixCount<13||shot.composition?.relocatedSteppingStoneAnchor?.[0]!==9||shot.composition?.relocatedSteppingStoneAnchor?.[1]!==16)throw new Error(`Gardens composition registry invalid: ${JSON.stringify(shot.composition)}`);
