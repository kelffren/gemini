 ...
 * keys: OBSERVED BOOT TRANSFER PLAYWRIGHT IPHONE REQUEST RESPONSE CONTENT-LENGTH FIRST PLAYABLE REPEAT STABLE CORE PROP RESIDENCY CAMERA ATLAS PREBOOT INITIATOR IMPERIAL
 * purpose: repeat exact mobile boot observations, separate stable/boundary transfer, hard-gate forbidden preboot assets and capture owner-native prop/camera/atlas state at boot-ready
 * public-api: CLI --url --report --runs --trace-initiators
 * state-owned: report only
 * online: N/A; CI/build-time measurement
 * do-not: mutate runtime, classify requests with coarse Date.now timing, count external-provider traffic as deterministic local payload
 */
...
const traceInitiators=arg('trace-initiators','1')!=='0';
const IPHONE_UA='Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.0 Mobile/15E148 Safari/604.1';
const FORBIDDEN_PREBOOT_PATHS=Object.freeze([
  '/assets/world/imperial-plaza/rosa-de-los-vientos.png',
  '/assets/world/imperial-plaza/banco-imperial.png',
  '/assets/world/imperial-plaza/kiosco-imperial.png',
  '/assets/world/imperial-plaza/jardinera-imperial-curva.png',
  '/assets/world/imperial-plaza/farola-monumental.png'
]);
const forbiddenPrebootSet=new Set(FORBIDDEN_PREBOOT_PATHS);
...
const finiteEpoch=(value,fallback=Date.now())=>Number.isFinite(Number(value))&&Number(value)>0?Number(value):fallback;
function cleanInitiator(initiator){
  if(!initiator)return null;
  const frames=Array.isArray(initiator.stack?.callFrames)?initiator.stack.callFrames.slice(0,10).map(frame=>({functionName:String(frame.functionName||''),url:String(frame.url||''),lineNumber:Number(frame.lineNumber)||0,columnNumber:Number(frame.columnNumber)||0})):[];
  return{type:String(initiator.type||'other'),url:String(initiator.url||''),lineNumber:Number(initiator.lineNumber)||0,columnNumber:Number(initiator.columnNumber)||0,frames};
}
...
async function observeRun(browser,runIndex){
  ...
  const page=await context.newPage(),records=[],byRequest=new Map(),externalOrigins=new Set(),pageErrors=[],targetedNetwork=[];
  let cdp=null;
  if(traceInitiators){
    try{
      ...
      cdp.on('Network.requestWillBeSent',event=>{let parsed;try{parsed=new URL(event?.request?.url||'');}catch{return;}const p=decodeURIComponent(parsed.pathname||'/');if(!forbiddenPrebootSet.has(p))return;targetedNetwork.push({path:p,method:String(event?.request?.method||'GET'),startedAtEpochMs:finiteEpoch(Number(event?.wallTime)*1000),initiator:cleanInitiator(event?.initiator)});});
    }catch(error){pageErrors.push(`initiator-trace:${String(error?.message||error)}`);}
  }
  page.on('pageerror',error=>pageErrors.push(String(error?.stack||error)));
  page.on('request',request=>{let parsed;try{parsed=new URL(request.url());}catch{return;}const timing=request.timing?.(),startedAtEpochMs=finiteEpoch(timing?.startTime);const record={request,path:normalizePath(request.url()),method:request.method(),resourceType:request.resourceType(),startedAtEpochMs,timingSource:Number.isFinite(Number(timing?.startTime))?'playwright-request-timing':'date-now-fallback',sameOrigin:parsed.origin===target.origin,response:null,failed:null};records.push(record);byRequest.set(request,record);if(!record.sameOrigin)externalOrigins.add(parsed.origin);});
  ...
  await page.addInitScript(()=>{try{let current=window.__keloBootReady;Object.defineProperty(window,'__keloBootReady',{configurable:true,enumerable:true,get(){return current;},set(value){current=value;if(value===true&&!window.__keloObservedBootReadyEpochMs){const precise=Number(performance.timeOrigin)+Number(performance.now());window.__keloObservedBootReadyEpochMs=Number.isFinite(precise)?precise:Date.now();}}});}catch(_){}});
  try{
    ...
    const forbiddenPreboot=preboot.filter(record=>forbiddenPrebootSet.has(record.path)).map(record=>({path:record.path,startedAtEpochMs:record.startedAtEpochMs,deltaMs:record.startedAtEpochMs-bootEpoch,resourceType:record.resourceType,timingSource:record.timingSource}));
    const targetedTiming=targetedNetwork.map(record=>({...record,deltaMs:record.startedAtEpochMs-bootEpoch,phase:record.startedAtEpochMs<=bootEpoch?'preboot':'postboot'})).sort((a,b)=>a.startedAtEpochMs-b.startedAtEpochMs);
    return{runIndex,bootReadyMs:Math.max(0,bootEpoch-state.timeOrigin),requestCount:preboot.length,resourceCount:resources.length,totalPayloadBytes,unknownLocalByteResponses,pageErrors,externalOrigins:[...externalOrigins].sort(),forbiddenPreboot,targetedTiming,diagnostics:{camera:state.camera,props:state.props,residency:state.residency,atlas:state.atlas},resources};
  }finally{try{await cdp?.detach?.();}catch{}await context.close();}
}
...
try{
  const runs=[];for(let i=1;i<=runCount;i++){
    ...
    console.log(`OBSERVED_BOOT_RUN index=${i} requests=${run.requestCount} resources=${run.resourceCount} payload=${run.totalPayloadBytes} bootReadyMs=${run.bootReadyMs.toFixed(1)} unknown=${run.unknownLocalByteResponses} pageErrors=${run.pageErrors.length} forbidden=${run.forbiddenPreboot.length}`);
    ...
    for(const row of run.targetedTiming||[])console.log(`OBSERVED_BOOT_TARGETED_TIMING index=${i} phase=${row.phase} deltaMs=${row.deltaMs.toFixed(3)} path=${row.path} initiator=${JSON.stringify(row.initiator)}`);
  }
  const aggregate=new Map(),externalOrigins=new Set();let unknownLocalByteResponses=0;const pageErrors=[],forbiddenPreboot=[];
  for(const run of runs){unknownLocalByteResponses+=run.unknownLocalByteResponses;for(const hit of run.forbiddenPreboot||[])forbiddenPreboot.push({runIndex:run.runIndex,...hit});for(const error of run.pageErrors)pageErrors.push(`run${run.runIndex}:${error}`);for(const origin of run.externalOrigins)externalOrigins.add(origin);for(const item of run.resources){let row=aggregate.get(item.path);if(!row){row={path:item.path,resourceType:item.resourceType,presenceCount:0,requestCount:0,bytesPerResponse:0,totalPayloadBytes:0,statuses:new Set()};aggregate.set(item.path,row);}row.presenceCount+=1;row.requestCount=Math.max(row.requestCount,item.requestCount);row.bytesPerResponse=Math.max(row.bytesPerResponse,item.bytesPerResponse);row.totalPayloadBytes=Math.max(row.totalPayloadBytes,item.totalPayloadBytes);for(const status of item.statuses)row.statuses.add(status);}}
  ...
  const report={schema:'kelo-observed-boot-transfer-v3-precise-boundary',generatedAt:new Date().toISOString(),url:target.origin+'/',runCount,viewport:{width:390,height:844,deviceScaleFactor:2},userAgent:IPHONE_UA,timingBoundary:'performance-epoch-vs-playwright-request-start',forbiddenPrebootPaths:FORBIDDEN_PREBOOT_PATHS,forbiddenPreboot,bootReadyMs:median(runs.map(run=>run.bootReadyMs)),requestCount:Math.round(median(runs.map(run=>run.requestCount))),resourceCount:Math.round(median(runs.map(run=>run.resourceCount))),totalPayloadBytes:Math.round(median(runs.map(run=>run.totalPayloadBytes))),stableRequestCount,stableResourceCount:stableResources.length,stablePayloadBytes,unstableResourceCount:unstableResources.length,unknownLocalByteResponses,pageErrors,externalOrigins:[...externalOrigins].sort(),resources,stableResources,unstableResources,runs};
  fs.mkdirSync(path.dirname(reportPath),{recursive:true});fs.writeFileSync(reportPath,JSON.stringify(report,null,2));
  console.log(`OBSERVED_BOOT_TRANSFER_DONE runs=${runCount} stableRequests=${stableRequestCount} stableResources=${stableResources.length} stablePayload=${stablePayloadBytes} unstable=${unstableResources.length} medianPayload=${report.totalPayloadBytes} medianBootReadyMs=${report.bootReadyMs.toFixed(1)} forbidden=${forbiddenPreboot.length}`);
  ...
  if(forbiddenPreboot.length){for(const hit of forbiddenPreboot)console.error(`OBSERVED_BOOT_FORBIDDEN_PREBOOT run=${hit.runIndex} deltaMs=${hit.deltaMs.toFixed(3)} source=${hit.timingSource} path=${hit.path}`);process.exitCode=4;}
  else if(unknownLocalByteResponses||pageErrors.length)process.exitCode=3;
