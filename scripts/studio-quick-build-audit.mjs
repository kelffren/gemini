...
assert.equal(quick.pieces.length,2,'phase 2 must preserve WALL and FLOOR');
...
assert.equal(preview.components.buildingPiece.version,2,'phase 2 semantic metadata must be versioned');
...
// Locality/performance: populate a large indexed world and require a local query instead of a 5,000-row scan.
...
// Five neighbors in the same local region; resolver should see these, not the other 4,995 entries.
for(let i=0;i<5;i++){
...
console.log(JSON.stringify({ok:true,phase:'2',pieces:['wall','floor'],preview:true,semanticMetadata:true,snapPoints:true,localSnap:true,snapFeedback:true,continuousPlacement:true,desktopPointer:true,mobilePointer:true,undoRedo:true,rotate:true,cancel:true,authorityBypass:false,performance:{entries:perfStats.entries,uniqueCandidates:perfStats.lastQuery.uniqueCandidates,membershipChecks:perfStats.lastQuery.membershipChecks}},null,2));
