 ...
 * keys: CAMERA PVP DEADZONE FOLLOW LOOKAHEAD LANDSCAPE DESKTOP PLAYWRIGHT AIM COMPOSITION
 * purpose: mide en Chromium dead-zone PvP y composición de aim perpendicular durante strafe sin alterar movimiento/aim de gameplay
...
async function enter(){await KeloPvPWorld.ensureCombatReady();KeloPvPWorld.enter();const deadline=performance.now()+5000;while(KeloPvPWorld.state.mode!=='pvp'&&performance.now()<deadline)await new Promise(r=>setTimeout(r,25));if(KeloPvPWorld.state.mode!=='pvp')throw new Error('PVP_ENTER_TIMEOUT');}
const deadzoneFn=async()=>{
  await enter();localPlayer.x=2900;localPlayer.y=720;localPlayer.vx=0;localPlayer.vy=0;input.normX=0;input.normY=0;input.keys.d=false;KeloCamera.setTarget(localPlayer.x,localPlayer.y,{snap:true,source:'pvp-camera-follow-audit'});await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));
...
const aimFn=async()=>{
  await enter();localPlayer.x=2900;localPlayer.y=720;localPlayer.vx=0;localPlayer.vy=0;input.normX=0;input.normY=0;input.keys.d=false;KeloCamera.setTarget(localPlayer.x,localPlayer.y,{snap:true,source:'pvp-camera-aim-audit'});
...
const report={ok:deadzoneOk(landscape)&&deadzoneOk(desktop)&&aimOk(aimLandscape)&&aimOk(aimDesktop),landscape,desktop,aimLandscape,aimDesktop};
