 ...
 * purpose: mide en Chromium dead-zone PvP, composición de aim perpendicular durante strafe y baseline de framing al atacar quieto
...
const enterPvp=async()=>{await KeloPvPWorld.ensureCombatReady();KeloPvPWorld.enter();let deadline=performance.now()+5000;while(KeloPvPWorld.state.mode!=='pvp'&&performance.now()<deadline)await new Promise(r=>setTimeout(r,25));if(KeloPvPWorld.state.mode!=='pvp')throw new Error('PVP_ENTER_TIMEOUT');};
...
const stationaryActionFn=async()=>{
  await enterPvp();
  localPlayer.x=2900;localPlayer.y=720;localPlayer.vx=0;localPlayer.vy=0;input.normX=0;input.normY=0;input.keys.a=false;input.keys.d=false;input.keys.w=false;input.keys.s=false;KeloCamera.setTarget(localPlayer.x,localPlayer.y,{snap:true,source:'pvp-camera-stationary-action-audit'});
  ...
  const before={x:Number(camera.lookOffsetX)||0,y:Number(camera.lookOffsetY)||0,snapshot:KeloCamera.snapshot()};
  const started=KeloPvPWorld.startBasicAttack('camera-audit');const samples=[];let peak=0;const t0=performance.now();
  while(performance.now()-t0<360){await new Promise(r=>requestAnimationFrame(r));const snap=KeloCamera.snapshot();const x=Number(camera.lookOffsetX)||0;peak=Math.max(peak,Math.abs(x));samples.push({t:performance.now()-t0,x,y:Number(camera.lookOffsetY)||0,attack:KeloPvPWorld.state.basicAttack,framing:snap.combatFraming});}
  return{viewport:{w:innerWidth,h:innerHeight},started,before,peakLookX:peak,after:{x:Number(camera.lookOffsetX)||0,y:Number(camera.lookOffsetY)||0},samples:samples.slice(0,16),version:KeloCamera.snapshot().version};
};
...
const report={ok:deadzoneOk(landscape)&&deadzoneOk(desktop)&&aimOk(aimLandscape)&&aimOk(aimDesktop),landscape,desktop,aimLandscape,aimDesktop,stationaryActionBaseline:{mobile:actionLandscape,desktop:actionDesktop}};
