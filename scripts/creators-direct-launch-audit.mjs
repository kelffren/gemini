...
try{
  ...
  const worldState=await page.evaluate(()=>({
    studioVisible:!!document.getElementById('kelo-studio-live'),
    hubGone:!document.getElementById('kelo-creators-hub'),
    lockOwners:window.KeloInputLocks?.snapshot?.().owners||[]
  }));
  if(!worldState.studioVisible||!worldState.hubGone)throw new Error(`CREATORS_DIRECT_WORLD_FAILED:${JSON.stringify(worldState)}`);
  if(pageErrors.length)throw new Error(`CREATORS_DIRECT_PAGE_ERRORS:${pageErrors.join(' | ')}`);

  await page.screenshot({path:'artifacts/creators-direct-world.png',fullPage:true});
  console.log(JSON.stringify({ok:true,url:String(url),viewport:{width:390,height:844,dpr:2},hubState,worldState},null,2));
