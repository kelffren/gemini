/* KELO-INDEX
 * area: QA / GUARDIAN
 * keys: GUARDIAN AUDIT CLIENT-ONLY DONOR HOST LEASE AUTH IOS REGION SCHEDULER REWARD PROOF SUPABASE WEBRTC SIGNAL DATACHANNEL HOT MIRROR FAILOVER CHECKPOINT PORTABLE PVP
 * hace: valida separación cliente/donador, control plane, scheduling, rewards, Hot Mirror y autoridad PvP portable
...
const clientOnly={accountId:'acc-client',roles:['player'],permissions:[]};
...
// Client participation is transport-only: no Guardian workload, witness role, master lease or reward eligibility.
const clientNode={...base,nodeId:'g_client_only_123',capabilities:{...base.capabilities,platform:'desktop',cores:1,memoryGb:0},preferences:{clientOnly:true,idleDonation:false,wifiOnly:false,allowAssets:false,allowRelay:false,allowCompute:false,maxUploadMbps:0,storageMb:0}};
const clientStatus=guardian.enable(clientOnly,clientNode);assert.equal(clientStatus.node.role,'client-only');assert.equal(clientStatus.node.clientOnly,true);assert.equal(clientStatus.node.donationEnabled,false);assert.deepEqual(clientStatus.node.recommendedRoles,[]);assert.equal(clientStatus.network.clientNodes,1);assert.ok(clientStatus.network.p2pNodes>clientStatus.network.activeNodes);
const witnessPlan=guardian.planWorkload({type:'witness',region:'us-east',limit:8});assert.ok(!witnessPlan.candidates.some(c=>c.nodeId===clientNode.nodeId));
assert.throws(()=>guardian.recordVerifiedContribution({accountId:clientOnly.accountId,nodeId:clientNode.nodeId},{type:'availability_seconds',seconds:60,region:'us-east'}),/GUARDIAN_CLIENT_ONLY_NO_REWARD/);
const adminClientNode={...clientNode,nodeId:'g_admin_client_123'};guardian.enable(admin,adminClientNode);assert.throws(()=>guardian.startMaster(admin,adminClientNode),/GUARDIAN_DONATION_REQUIRED/);
...
const relayPlan=guardian.planWorkload({type:'relay',region:'us-east',limit:3});assert.ok(relayPlan.candidates.some(c=>c.nodeId===helpNode.nodeId));assert.ok(!relayPlan.candidates.some(c=>c.nodeId===clientNode.nodeId));
...
assert.equal(guardian.audit().serverAuthorityPreserved,true);assert.equal(guardian.audit().rewardMetricsClientTrusted,false);assert.equal(guardian.audit().kcMintAuthority,false);assert.equal(guardian.audit().clientParticipationSeparate,true);assert.equal(guardian.audit().clientOnlyWorkloadEligible,false);assert.equal(guardian.audit().clientOnlyRewardEligible,false);
...
const simulation=fs.readFileSync(path.join(root,'src/systems/guardian-simulation-host.js'),'utf8');
...
assert.match(simulation,/broadcastDonors/);assert.match(simulation,/safeId\(msg\.actorId\)===actorId\(\)/);assert.match(simulation,/rejectScopedToActor:true/);assert.match(simulation,/durableEconomyAuthority:false/);
...
console.log('GUARDIAN_AUDIT_OK',{...guardian.audit(),supabaseControlPlane:true,webrtcDataPlane:true,hotMirror:true,automaticMasterClaim:true,portablePvpAudit:true,clientDonorSeparation:true,rejectScopedToActor:true,secondLoop:false,clientGameplayAuthority:false});
