/* KELO-INDEX
 * area: QA / GUARDIAN
 * keys: GUARDIAN AUDIT HOST LEASE AUTH IOS REGION SCHEDULER REWARD PROOF SUPABASE WEBRTC SIGNAL DATACHANNEL HOT MIRROR FAILOVER CHECKPOINT WEBGPU GPU ASSET QUORUM COMMUNITY RECEIPT PROVENANCE PERSISTENCE FENCING STANDBY
 * hace: valida control plane, standby elegido por servidor, WebGPU, Community Builder cross-device, fencing por nodo y Hot Mirror sin segundo loop ni autoridad económica cliente
 * online: audit local determinista + contratos estáticos Supabase/WebRTC/WebGPU/Community/Hot Mirror/Standby
 */
...
const standbyMigration=fs.readFileSync(path.join(root,'supabase/migrations/20260916081157_guardian_standby_assignment_v1.sql'),'utf8');
...
assert.match(standbyMigration,/create table if not exists public\.guardian_master_standby/);assert.match(standbyMigration,/guardian_master_claim_standby/);assert.match(standbyMigration,/GUARDIAN_STANDBY_NOT_ASSIGNED/);assert.match(standbyMigration,/assignment_epoch/);assert.match(standbyMigration,/host-ready/);assert.match(standbyMigration,/master_epoch/);assert.match(standbyMigration,/revoke all on public\.guardian_master_standby from public,anon,authenticated/);assert.match(standbyMigration,/revoke all on function public\.guardian_master_claim_standby\(text\) from public,anon/);assert.match(standbyMigration,/grant execute on function public\.guardian_master_claim_standby\(text\) to authenticated,service_role/);assert.match(standbyMigration,/set search_path=''/);assert.doesNotMatch(standbyMigration,/grant\s+.*guardian_master_standby.*to\s+authenticated/i);
...
console.log('GUARDIAN_AUDIT_OK',{...guardian.audit(),supabaseControlPlane:true,webrtcDataPlane:true,webgpuAssetCompute:true,hashValidation:true,communityGpuMeter:true,communityIdentity:true,communityCrossDeviceProfile:true,communityServerAuthority:true,anonymousAssetProvenance:true,hotMirror:true,automaticMasterClaim:true,serverSelectedStandby:true,standbySelfAssignment:false,nodeScopedMasterFencing:true,secondLoop:false,clientGameplayAuthority:false,economicAuthority:false});
