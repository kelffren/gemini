...
11. El presupuesto CI actual es **48/48 scripts externos antes de boot-ready** y **3/3 scripts externos estáticos después**. También se limita el peso fuente determinista: **422,549 / 425,000 bytes críticos** y **21,669 / 22,000 bytes post-boot estáticos**. Subir esos límites requiere evidencia y cambio explícito del contrato, no crecimiento accidental.
...
15. `engine-l.js` y `engine-m.js` registran sus middleware directamente mediante `KeloLegacyAbilityCast`. Ya no existe ningún consumidor runtime directo de `KeloAbilityAim`, y el adapter `KeloAbilityAim.registerCastMiddleware` ya fue **RETIRED**; `KeloAbilityAim` v1.4.1 conserva aim/pointer/range y compatibilidad de cast, pero ya no acepta registros de middleware.
...
- Abilities legacy: `KeloAbilityAim` posee pointer lifecycle global, matemática/range e indicador; `KeloLegacyAbilityCast` posee el registry/dispatcher de middleware. No se añadió un segundo engine ni un script de boot. El antiguo `KeloAbilityAim.registerCastMiddleware` ya fue retirado después de llegar a cero consumidores.
- Consumidores legacy: el audit de Foundation distingue referencias ejecutables de comentarios. Quedan **0 consumidores directos de `KeloAbilityAim`**, **2 consumidores directos de `KeloLegacyAbilityCast`** (`engine-l/m`), **3 consumidores de estado `skillAim`** (`engine-g/l/m`) y **6 consumidores de `triggerStone`**. El siguiente frente seguro es inventariar quién sigue entrando realmente al lifecycle global `beginSkillAim/endSkillAim` y reducir `skillAim` consumidor por consumidor; no tocar dash por similitud nominal.
...
El head runtime `4249e3739e1b2321131b2c4195a0891838ad148d` mantiene `directAimConsumers=[]` y `directCastConsumers=["engine-l.js","engine-m.js"]`; Foundation confirma además que el adapter de registro de `KeloAbilityAim` está retirado y que `KeloLegacyAbilityCast` conserva el único registry/dispatcher. Este gate es evidencia branch-local y **no sustituye QA físico/LIVE**.
...
- No registrar middleware legacy mediante `KeloAbilityAim`: esa API ya no existe. Todo registro pasa exclusivamente por `KeloLegacyAbilityCast.registerMiddleware`.
