...
const {createSpriteAiService,createOpenAiProvider,buildPrompt,buildHuggingFacePrompt}=require('./sprite-ai-service');
...
(async()=>{
  ...
  const hfCalls=[];
  const hf=createHuggingFaceSpriteProvider({space:'kelo/free-sprite-ai',token:'hf-test',connectImpl:async(space,options)=>({predict:async(apiName,payload)=>{hfCalls.push({space,options,apiName,payload});return{data:[dataUrl]};}})});
  ...
  assert.equal(hfOut.provider,'huggingface-zerogpu');assert.equal(hfOut.layout.rows,8);assert.equal(hfCalls.length,1);assert.equal(hfCalls[0].apiName,'/generate');assert.equal(hfCalls[0].payload[2],17);assert(hfCalls[0].payload[0].includes('teal coat warrior'));
  ...
  const failingHf={status:()=>({configured:true,provider:'huggingface-zerogpu',space:'test'}),generate:async()=>{const error=new Error('quota');error.code='SPRITE_AI_HF_UPSTREAM_ERROR';error.status=502;throw error;}};
  ...
  try{
    let r=await fetch(base+'/api/sprite-generate/status',{headers:{Origin:'https://example.test'}});assert.equal(r.status,200);let j=await r.json();assert.equal(j.configured,true);assert.equal(j.provider,'huggingface-zerogpu');assert.equal(r.headers.get('access-control-allow-origin'),'https://example.test');
