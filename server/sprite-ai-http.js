...
const {createSpriteAiResourceGovernor}=require('./sprite-ai-resource-governor');
...
function createRateLimiter({limit=24,windowMs=60*60*1000}={}){
...
function createSpriteAiHttpHandler(options={}){
  ...
  const governor=options.resourceGovernor||createSpriteAiResourceGovernor(options.resourceGovernorOptions||{});
  const maxBodyBytes=Math.max(1024*1024,Number(options.maxBodyBytes||process.env.KELO_SPRITE_AI_MAX_BODY_BYTES||10*1024*1024));
  const rateLimit=Math.max(1,Math.min(120,Number(options.rateLimit||process.env.KELO_SPRITE_AI_RATE_LIMIT_PER_HOUR||24)));
  ...
  return async function handle(req,res){
    ...
    if(path==='/api/sprite-generate/status'){
      ...
      writeJson(res,200,{ok:true,service:'kelo-sprite-ai',...service.status(),auth:'supabase-user',anonymousAllowed:false,rateLimitPerHour:rateLimit,freeTierOptimization:{exactRequestCache:true,inflightDedup:true,userIsolatedCache:true,softBudgetSeconds:Number(process.env.KELO_SPRITE_AI_SOFT_QUOTA_SECONDS||270),paidFallbackDefault:false}},origin);return true;
    }
    ...
    try{
      ...
      const requestBudget=take(user.id);if(!requestBudget.ok)throw serviceError('SPRITE_AI_RATE_LIMITED',429,String(requestBudget.resetAt));
      const input=await readJson(req,maxBodyBytes);
      const result=await governor.run({userId:user.id,input,execute:()=>service.generate(input)});
      writeJson(res,200,{...result,rateLimit:{limit:requestBudget.limit,remaining:requestBudget.remaining,resetAt:requestBudget.resetAt}},origin);return true;
    }catch(error){
