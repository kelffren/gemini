 ...
 * public-api: createSpriteAiService, buildPrompt, buildHuggingFacePrompt
 ...
 * extension-points: provider=auto|huggingface|openai, injected provider adapters for tests
...
function buildHuggingFacePrompt(input={}){
  ...
  return [
    characterPrompt?`Character description: ${characterPrompt}`:'Create a premium dark-fantasy MMORPG character with a clean readable silhouette and restrained gold accents.',
    `Animation: looping ${action}.`,
    styleHint?`Art direction: ${styleHint}`:'',
    'Keep one identical character identity, clothing, equipment, proportions, palette and lighting across every generated view.',
    'Pixel art must remain readable after reduction to a 64x64 game frame. Use transparent background and no scenery, text, labels, borders or cast shadow outside the sprite.',
    retryHint?`Previous Kelo QA feedback: ${retryHint}`:''
  ].filter(Boolean).join('\n');
}
function createOpenAiProvider(options={}){
  ...
  async function generate(input,prompt){
    if(!apiKey)throw serviceError('SPRITE_AI_OPENAI_NOT_CONFIGURED',503);
    const source=parseImageDataUrl(input.sourceImageDataUrl,maxSourceBytes),headers={Authorization:`Bearer ${apiKey}`};let res;
    if(source){
    ...
    const out=await parseResponse(res);return{...out,provider:'openai',model,size,quality,sourceMode:source?'reference-edit':'text-generation'};
  }
...
function createSpriteAiService(options={}){
  ...
  function resolvePrimary(){
    if(requested==='huggingface'||requested==='hf'||requested==='zerogpu')return huggingface;
    if(requested==='openai')return openai;
    if(requested!=='auto')throw serviceError('SPRITE_AI_UNKNOWN_PROVIDER',500,requested);
    if(hfStatus.configured)return huggingface;
    if(openaiStatus.configured)return openai;
    return huggingface;
  }
  function status(){
    const primary=resolvePrimary(),primaryStatus=primary.status();
    return Object.freeze({...primaryStatus,configured:Boolean(primaryStatus.configured),requestedProvider:requested,allowPaidFallback,directions:8,framesPerDirection:4,providers:Object.freeze({huggingface:hfStatus,openai:openaiStatus})});
  }
  async function generate(input={}){
    if(input.sourceImageDataUrl)parseImageDataUrl(input.sourceImageDataUrl,maxSourceBytes);
    const primary=resolvePrimary(),primaryStatus=primary.status();
    if(!primaryStatus.configured)throw serviceError('SPRITE_AI_NOT_CONFIGURED',503,requested==='auto'?'Set KELO_SPRITE_AI_HF_SPACE for free ZeroGPU or OPENAI_API_KEY for paid fallback.':requested);
    let out;
    if(primary===huggingface){
      try{out=await huggingface.generate({prompt:buildHuggingFacePrompt(input),sourceImageDataUrl:input.sourceImageDataUrl||null,seed:input.seed||0,retryHint:input.retryHint||''});}
      catch(error){
        if(!(allowPaidFallback&&openaiStatus.configured))throw error;
        out=await openai.generate(input,buildPrompt(input));out={...out,fallbackFrom:'huggingface-zerogpu'};
      }
    }else out=await openai.generate(input,buildPrompt(input));
    return Object.freeze({ok:true,...out,layout:{columns:4,rows:8,directions:DEFAULT_DIRECTIONS,framesPerDirection:4},generatedAt:Date.now()});
  }
  return Object.freeze({version:'kelo-sprite-ai-service-v2',status,generate,buildPrompt,buildHuggingFacePrompt});
}

module.exports={createSpriteAiService,createOpenAiProvider,buildPrompt,buildHuggingFacePrompt,parseImageDataUrl,serviceError,DEFAULT_DIRECTIONS};
