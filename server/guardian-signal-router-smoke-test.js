...
const guardian={
  ...
  planWorkload(input){
    ...
    const ex=new Set(input.excludeNodeKeys||[]);return{candidates:all.filter(x=>!ex.has(x.nodeKey))};
  }
};
...
clock+=61000;const swept=router.sweep(clock);assert.equal(swept.sessions,0);assert.equal(a.ws.sent.some(x=>x.t==='guardian:peer:closed'&&x.reason==='ttl-expired'),true);
