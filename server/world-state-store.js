...
function normalizeMutation(input={}){
  ...
  const patch=input.patch&&typeof input.patch==='object'&&!Array.isArray(input.patch)?clone(input.patch):null;if(!patch)throw new Error('WORLD_PATCH_INVALID');
...
function createWorldStateStore({bridge=null,clock=Date.now}={}){
  ...
  function memoryMutate(input){
    ...
    const next={cellId:m.cellId,revision:current.revision+1,schemaVersion:1,state:{...current.state,...m.patch},lastEventId:m.eventId,updatedAt:new Date(clock()).toISOString()};
    ...
    snapshots.set(m.cellId,next);events.push(event);const result={cellId:m.cellId,revision:next.revision,schemaVersion:1,state:clone(next.state),eventId:m.eventId,eventType:m.eventType,aggregateId:m.aggregateId};idempotency.set(m.eventId,result);return clone(result);
  }
  ...
  function audit(){return Object.freeze({version:'kelo-world-state-store-v1',source:durable?'supabase-world-ledger':'memory-world-transition',durable,replayable:true,revisionCas:true,idempotentEvents:true,clientWritable:false,loads,mutations,replays,conflicts,lastError});}
  return Object.freeze({version:'kelo-world-state-store-v1',source:durable?'supabase-world-ledger':'memory-world-transition',durable,load,mutate,replay,audit});
}
