...
function createGuardianCoordinator(options={}){
  ...
  function sweepWorkloads(at){for(const [id,workload] of workloads){if(workload.expiresAt>at)continue;const node=nodes.get(workload.nodeKey);if(node&&Array.isArray(node.assignments))node.assignments=node.assignments.filter(row=>row.id!==id);workloads.delete(id);}}
  function networkSummary(at=now()){
    sweepLeaseOnly(at);sweepWorkloads(at);
    let active=0,ios=0,relayReady=0,assetReady=0,computeReady=0,gpuAssetDonors=0,gpuCapacityUnits=0,hostReady=0,assigned=0;const assignedByType={};
    ...
    const demand={};for(const [region,pressure] of regionalDemand)demand[region]=pressure;gpuCapacityUnits=Number(gpuCapacityUnits.toFixed(2));
  ...
  function sweep(at=now()){for(const [key,node] of nodes){if(!fresh(node,at)){for(const row of node.assignments||[])workloads.delete(row.id);nodes.delete(key);}}sweepLeaseOnly(at);sweepWorkloads(at);return networkSummary(at);}
  ...
  function enable(actor,input={}){const at=now(),nodeId=normalizeNodeId(input.nodeId),key=nodeKey(actor.accountId,nodeId),capabilities=sanitizeCapabilities(input.capabilities),preferences=sanitizePreferences(input.preferences),existing=nodes.get(key);const node=ensureNodeState(existing||{key,nodeId,accountId:actor.accountId,createdAt:at});node.capabilities=capabilities;node.preferences=preferences;node.recommendedRoles=recommendedRoles(capabilities,preferences);node.lastHeartbeatAt=at;node.role=masterLease?.nodeKey===key?'master-host':'donor-ready';nodes.set(key,node);return payload(actor,node,at);}
  function heartbeat(actor,input={}){const at=now(),nodeId=normalizeNodeId(input.nodeId),key=nodeKey(actor.accountId,nodeId),node=nodes.get(key);if(!node)throw new Error('GUARDIAN_NODE_NOT_ENABLED');if(input.capabilities)node.capabilities=sanitizeCapabilities(input.capabilities);if(input.preferences)node.preferences=sanitizePreferences(input.preferences);node.recommendedRoles=recommendedRoles(node.capabilities,node.preferences);node.lastHeartbeatAt=at;if(masterLease?.nodeKey===key){if(node.capabilities.visibility==='visible'){masterLease.expiresAt=at+MASTER_LEASE_MS;node.role='master-host';}else{masterLease=null;node.role='donor-ready';}}return payload(actor,node,at);}
  function disable(actor,input={}){const at=now(),nodeId=normalizeNodeId(input.nodeId),key=nodeKey(actor.accountId,nodeId),node=nodes.get(key);if(node)for(const row of node.assignments||[])workloads.delete(row.id);nodes.delete(key);if(masterLease?.nodeKey===key)masterLease=null;return payload(actor,null,at);}
  ...
  function startMaster(actor,input={}){if(!masterEligible(actor))throw new Error('GUARDIAN_MASTER_PERMISSION_DENIED');const at=now(),nodeId=normalizeNodeId(input.nodeId),key=nodeKey(actor.accountId,nodeId);let node=nodes.get(key);if(!node){enable(actor,input);node=nodes.get(key);}if(!node)throw new Error('GUARDIAN_NODE_NOT_ENABLED');if(node.capabilities.visibility!=='visible')throw new Error('GUARDIAN_FOREGROUND_REQUIRED');sweep(at);if(masterLease&&masterLease.nodeKey!==key&&masterLease.accountId!==actor.accountId)throw new Error('GUARDIAN_MASTER_BUSY');if(masterLease&&masterLease.nodeKey!==key){const previous=nodes.get(masterLease.nodeKey);if(previous)previous.role='donor-ready';}masterLease={nodeKey:key,accountId:actor.accountId,nodeId,epoch:++epoch,startedAt:at,expiresAt:at+MASTER_LEASE_MS};node.role='master-host';node.lastHeartbeatAt=at;return payload(actor,node,at);}
  function stopMaster(actor,input={}){if(!masterEligible(actor))throw new Error('GUARDIAN_MASTER_PERMISSION_DENIED');const at=now(),nodeId=normalizeNodeId(input.nodeId),key=nodeKey(actor.accountId,nodeId),node=nodes.get(key);if(masterLease?.nodeKey===key)masterLease=null;if(node)node.role='donor-ready';return payload(actor,node,at);}
  ...
  function workloadCapability(type,node){if(type==='relay')return node.recommendedRoles.includes('relay-ready');if(type==='asset-seeder')return node.recommendedRoles.includes('asset-seeder-ready');if(type==='compute-worker')return node.recommendedRoles.includes('compute-candidate');if(type==='asset-gpu-worker')return node.recommendedRoles.includes('asset-gpu-worker');if(type==='primary-host'||type==='hot-mirror')return node.recommendedRoles.includes('host-ready');return node.recommendedRoles.includes('witness-ready');}
  ...
  function candidateScore(node,input,at){const type=String(input.type||''),region=normalizeRegion(input.region),obs=node.observation||{},cost=WORKLOAD_COST[type]||.2;if(!fresh(node,at)||!workloadCapability(type,node)||loadWeight(node)+cost>1.001)return null;let score=100-loadWeight(node)*55;if(node.capabilities.visibility==='visible')score+=8;if(node.capabilities.charging===true)score+=5;if(node.capabilities.saveData)score-=25;score+=Math.min(20,node.capabilities.cores*1.5)+Math.min(12,node.capabilities.memoryGb||0);if(obs.region===region)score+=30;const rr=obs.rttByRegion&&obs.rttByRegion[region],rtt=Number.isFinite(rr)?rr:obs.region===region?obs.rttMs:null;if(Number.isFinite(rtt))score+=clamp(42-rtt*.35,-35,42,0);if(Number.isFinite(obs.packetLossPct))score-=obs.packetLossPct*4;if(Number.isFinite(obs.cpuLoad))score+=(1-obs.cpuLoad)*30;if(Number.isFinite(obs.tickHz))score+=obs.tickHz>=58?12:obs.tickHz<50?-18:0;if(type==='relay'||type==='asset-seeder')score+=Math.min(45,(Number(obs.uploadMbps)||0)*.7)+Math.min(20,node.preferences.maxUploadMbps*.25);if(type==='primary-host'||type==='hot-mirror'||type==='compute-worker')score+=(1-(Number(obs.cpuLoad)||.35))*25;if(type==='asset-gpu-worker')score+=effectiveGpuUnits(node)*.9+(node.capabilities.gpuTier==='high'?12:node.capabilities.gpuTier==='medium'?6:0);return Number(score.toFixed(3));}
  function planWorkload(input={}){const type=String(input.type||'');if(!WORKLOAD_TYPES.has(type))throw new Error('GUARDIAN_WORKLOAD_TYPE_INVALID');const at=now(),region=normalizeRegion(input.region),limit=Math.round(clamp(input.limit,1,8,3)),exclude=new Set((input.excludeNodeKeys||[]).map(String)),rows=[];sweep(at);for(const node of nodes.values()){if(exclude.has(node.key))continue;const score=candidateScore(node,{...input,type,region},at);if(score==null)continue;rows.push({nodeKey:node.key,nodeId:node.nodeId,accountId:node.accountId,score,region:node.observation?.region||'unknown',rttMs:node.observation?.rttByRegion?.[region]??node.observation?.rttMs??null,load:Number(loadWeight(node).toFixed(3)),gpuCapacityUnits:type==='asset-gpu-worker'?effectiveGpuUnits(node):undefined});}rows.sort((a,b)=>b.score-a.score||String(a.nodeKey).localeCompare(String(b.nodeKey)));return Object.freeze({type,region,candidates:Object.freeze(rows.slice(0,limit).map(row=>Object.freeze(row))),demandMultiplier:Number(demandMultiplier(region).toFixed(4))});}
  ...
  function planSupport(input={}){const region=normalizeRegion(input.region),primaryKey=String(input.primaryNodeKey||''),exclude=primaryKey?[primaryKey]:[],cpu=clamp(input.cpuLoad,0,1,0),upload=clamp(input.uploadPressure,0,1,0),loss=clamp(input.packetLossPct,0,100,0),tick=clamp(input.tickHz,0,240,60),regional=clamp(input.regionalPressure,0,1,0),needs=[];if(upload>=.75||loss>=3)needs.push(Object.freeze({reason:'NETWORK_PRESSURE',plan:planWorkload({type:'relay',region,limit:2,excludeNodeKeys:exclude})}));if(cpu>=.8||tick<55)needs.push(Object.freeze({reason:'SIMULATION_PRESSURE',plan:planWorkload({type:'primary-host',region,limit:2,excludeNodeKeys:exclude})}),Object.freeze({reason:'FAILOVER_SAFETY',plan:planWorkload({type:'hot-mirror',region,limit:2,excludeNodeKeys:exclude})}));if(regional>=.7)needs.push(Object.freeze({reason:'REGIONAL_LATENCY',plan:planWorkload({type:'primary-host',region,limit:3,excludeNodeKeys:exclude})}));return Object.freeze({region,needs:Object.freeze(needs)});}
  ...
  function recordVerifiedContribution(ref,proof={}){const node=resolveNode(ref);if(!node||!fresh(node,now()))throw new Error('GUARDIAN_NODE_NOT_ENABLED');const type=String(proof.type||''),rate=PROOF_RATES[type];if(!rate)throw new Error('GUARDIAN_PROOF_TYPE_INVALID');let quantity=0;if(type.endsWith('_seconds'))quantity=clamp(proof.seconds,0,3600,0);else quantity=clamp(proof.megabytes,0,10240,0);if(quantity<=0)throw new Error('GUARDIAN_PROOF_EMPTY');const rawUnits=quantity*rate,region=normalizeRegion(proof.region||node.observation?.region),demand=type==='availability_seconds'?1:demandMultiplier(region),service=serviceState(node),rank=rankFor(service.verifiedUnits+rawUnits),rewarded=rawUnits*demand*rank.multiplier;node.service.verifiedUnits=Number((service.verifiedUnits+rawUnits).toFixed(6));node.service.rewardedUnits=Number((service.rewardedUnits+rewarded).toFixed(6));node.service.proofCount++;if(type==='availability_seconds')node.service.availabilitySeconds+=quantity;else if(type==='host_seconds'||type==='mirror_seconds'||type==='compute_seconds'||type==='asset_gpu_seconds')node.service.usefulSeconds+=quantity;if(type==='asset_gpu_seconds')node.service.gpuAssetSeconds+=quantity;if(type==='relay_megabytes')node.service.bytesRelayed+=quantity*1024*1024;if(type==='asset_megabytes')node.service.bytesAssets+=quantity*1024*1024;return Object.freeze({accepted:true,type,quantity,region,rawUnits:Number(rawUnits.toFixed(6)),demandMultiplier:Number(demand.toFixed(4)),rank:Object.freeze({...rank}),weightedUnits:Number(rewarded.toFixed(6)),service:serviceState(node),kcMinted:0});}

  async function authenticate(req){const token=bearer(req);if(!token)throw new Error('AUTH_TOKEN_REQUIRED');const user=await identity.verifyAccessToken(token),access=await identity.getAccountAccess(token);if(access.status==='banned')throw new Error('ACCOUNT_BANNED');if(access.status==='suspended')throw new Error('ACCOUNT_SUSPENDED');return{accountId:user.id,roles:access.roles||[],permissions:access.permissions||[]};}
  async function handleHttp(req,res){const url=new URL(String(req.url||'/'),'http://guardian.local');if(!url.pathname.startsWith('/api/guardian'))return false;if(cors(req,res))return true;try{const actor=await authenticate(req);let body={};if(req.method==='POST')body=await readJson(req);let result;if(req.method==='GET'&&url.pathname==='/api/guardian/status')result=status(actor,{nodeId:url.searchParams.get('nodeId')});else if(req.method==='POST'&&url.pathname==='/api/guardian/enable')result=enable(actor,body);else if(req.method==='POST'&&url.pathname==='/api/guardian/heartbeat')result=heartbeat(actor,body);else if(req.method==='POST'&&url.pathname==='/api/guardian/disable')result=disable(actor,body);else if(req.method==='POST'&&url.pathname==='/api/guardian/master/start')result=startMaster(actor,body);else if(req.method==='POST'&&url.pathname==='/api/guardian/master/stop')result=stopMaster(actor,body);else{json(res,404,{ok:false,error:'GUARDIAN_NOT_FOUND'});return true;}json(res,200,result);return true;}catch(error){const code=errorCode(error),status=code==='AUTH_TOKEN_REQUIRED'?401:code==='GUARDIAN_MASTER_PERMISSION_DENIED'?403:code==='GUARDIAN_MASTER_BUSY'?409:code==='GUARDIAN_SERVER_ERROR'?500:400;json(res,status,{ok:false,error:code});return true;}}
