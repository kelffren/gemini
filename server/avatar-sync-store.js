...
function defaultCommunityProfile(slot){
  const base={mode:'socket',layer:'front',socket:'center',anchor:{x:.5,y:.5},scale:1,rotation:0,columns:1,rows:1,frameMs:140,faceRows:{down:0,left:0,right:0,up:0},offsets:{},pixelated:false,heightScale:.5,widthScale:1};
  if(slot==='body'||slot==='outfit')Object.assign(base,{socket:'foot',anchor:{x:.5,y:1},heightScale:1});
  else if(slot==='head'||slot==='hair'||slot==='face')Object.assign(base,{socket:'head',anchor:{x:.5,y:.55},heightScale:.38});
  else if(slot==='weapon')Object.assign(base,{socket:'handR',anchor:{x:.5,y:.72},heightScale:.5});
  else if(slot==='offhand')Object.assign(base,{socket:'handL',anchor:{x:.5,y:.65},heightScale:.42});
  else if(slot==='back')Object.assign(base,{socket:'center',layer:'back',anchor:{x:.5,y:.55},heightScale:.72});
  else if(slot==='aura')Object.assign(base,{socket:'foot',layer:'back',anchor:{x:.5,y:.72},heightScale:1.25});
  else if(slot==='effect')Object.assign(base,{socket:'center',anchor:{x:.5,y:.5},heightScale:1.05});
  else if(slot==='pet')Object.assign(base,{socket:'foot',anchor:{x:.5,y:1},heightScale:.44,offsets:{default:{x:42,y:2}}});
  else if(slot==='mount')Object.assign(base,{socket:'foot',layer:'back',anchor:{x:.5,y:.78},heightScale:1.35,offsets:{default:{x:0,y:12}}});
  return base;
}
function communityRenderProfile(raw,slot){
  const fallback=defaultCommunityProfile(slot),source=raw&&typeof raw==='object'?raw:{},mode=source.mode==='sheet'?'sheet':'socket',layer=source.layer==='back'?'back':'front',socket=COMMUNITY_SOCKETS.has(String(source.socket))?String(source.socket):fallback.socket,anchorSource=source.anchor&&typeof source.anchor==='object'?source.anchor:fallback.anchor;
  ...
  return Object.freeze({mode,layer,socket,anchor:Object.freeze({x:num(anchorSource.x,0,1,.5),y:num(anchorSource.y,0,1,.5)}),scale:num(source.scale,.1,4,1),rotation:num(source.rotation,-360,360,0),columns,rows,frameMs:int(source.frameMs,50,2000,140),idleFrame:int(source.idleFrame,0,Math.max(0,columns-1),0),faceRows:Object.freeze(faceRows),heightScale:num(source.heightScale,.08,4,fallback.heightScale),widthScale:num(source.widthScale,.08,4,1),offsets:Object.freeze(offsets),pixelated:source.pixelated===true});
}
