...
function connectionsForPlayerKey(playerKey) {
  const rows=[];players.forEach(p=>{if(p.playerKey===String(playerKey))rows.push(p);});return rows;
}
const commerce = createCommerceService({
  economyStore: economy,
  resolvePlayerName: playerKey => connectionsForPlayerKey(playerKey)[0]?.name || String(playerKey),
...
function notifyCommerce(playerKeys, reason, exceptPlayerKey) {
  [...new Set((playerKeys || []).filter(Boolean).map(String))].forEach(playerKey => {
    if (playerKey === exceptPlayerKey) return;
    const snapshot=commerce.snapshot(playerKey);
    connectionsForPlayerKey(playerKey).forEach(p=>send(p.ws,{t:'commerce:event',reason:reason||null,snapshot,source:'server-authoritative'}));
  });
}
  ...
  ws.on('message', async (buf) => {
    ...
    try {
      ...
      if (msg.t === 'pose') {
        ...
        if (['plaza','cafe','open-world','market'].includes(msg.zone)) me.zone = msg.zone;
        return;
      }
      ...
      if (msg.t === 'commerce:request') {
        const op=String(msg.op||'').slice(0,64),payload=msg.payload&&typeof msg.payload==='object'?msg.payload:{};
        const result=await commerce.handle(me.playerKey,op,payload,msg.requestId);
        const notifyIds=Array.isArray(result.notifyPlayerIds)?result.notifyPlayerIds.slice():[];
        const response={...result};delete response.notifyPlayerIds;
        send(ws,{t:'commerce:result',requestId:msg.requestId||null,...response,source:'server-authoritative'});
        notifyCommerce(notifyIds,op,me.playerKey);
        return;
      }
